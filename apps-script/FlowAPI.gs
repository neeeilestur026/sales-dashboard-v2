...
function createQuotation(p) {
  ...
  var initialStatus = p.status ||
    (_isMgmtTier(creatorRole) ? 'Approved' : (_isAdminTier(creatorRole) ? 'Pending Director' : 'Pending Admin'));
...
function submitQuotationApproval(p) {
  ...
  // A267 — admin-created quotations enter at the director; sales-created go to admin first.
  var role = String(q['Created By Role'] || p.actorRole || 'sales');
  var next = _isAdminTier(role) ? 'Pending Director' : 'Pending Admin';
...
function approveQuotation(p) {
  ...
  if (st === 'Pending Admin') {
    ...
    _setQuotationCells(p.quotationNo, { 'Status': 'Pending Director' });
    return { success: true, quotationNo: p.quotationNo, status: 'Pending Director', message: 'Admin approved; forwarded to the director.' };
  }
  /* A267 — the director reviews BEFORE management, against the pricing management set on the
     purchase request. Management is the last signature, so a director approval no longer closes
     the quotation. */
  if (st === 'Pending Director') {
    if (!_isDirectorRole(role)) return { success: false, message: 'Only the director can approve at this stage.' };
    _setQuotationCells(p.quotationNo, { 'Status': 'Pending Management' });
    return { success: true, quotationNo: p.quotationNo, status: 'Pending Management', message: 'Director approved; forwarded to management.' };
  }
...
function rejectQuotation(p) {
  ...
  var canReject = (st === 'Pending Admin' && _isAdminTier(role))
    || (st === 'Pending Director' && _isDirectorRole(role))          // A267
    || (st === 'Pending Management' && _isMgmtOnly(role));
