...
var SCHEMA = {
  ...
  SalesCalls: ['Call No', 'Date', 'User', 'Contact', 'Company', 'Outcome', 'Notes', 'Created At', 'Kind', 'Contact No'],
...
var LEADGEN = {
  ...
  touches:  { sheet: 'LgTouches',    prefix: 'TCH', internal: true }   // written only by the fan-outs below
};
...
var _LG_COUNT_KEYS = ['plants', 'contacts', 'introEmails', 'followupEmails', 'coldCalls', 'followupCalls', 'leads', 'meetings'];
var _LG_QUOTA_KEY = { plants: 'lgQuotaPlants', contacts: 'lgQuotaContacts', introEmails: 'lgQuotaIntroEmails',
  followupEmails: 'lgQuotaFollowupEmails', coldCalls: 'lgQuotaColdCalls', followupCalls: 'lgQuotaFollowupCalls',
  leads: 'lgQuotaLeads', meetings: 'lgQuotaMeetings' };
...
function getLeadgenDay(p) {
  ...
  var calls = _rows('SalesCalls').filter(function (r) { return r['Kind']; }).filter(mine('User'))
    .filter(function (r) { return _lgDayOf(r['Date']) === date; }).map(function (r) {
      ...
      return { callNo: r['Call No'], kind: r['Kind'], contact: r['Contact'], company: r['Company'] || (pl ? pl['Company'] : ''),
               ...
               outcome: r['Outcome'], notes: r['Notes'], createdAt: r['Created At'] || '', contactNo: r['Contact No'] || '' };
    });
  ...
  return {
    ...
    plants: plants, contacts: contacts,
...
function saveLeadgenRecord(p) {
  ...
  var settings = null, plant = null, fanout = null, handoff = null;
  ...
  var msg = entity === 'batches' ? _lgMsg(entity, rec, stamp['Count'])
...
function getLeadgenCounts(p) {
  ...
  var plants = _lgRows('plants').filter(mine('Created By')), contacts = _lgRows('contacts').filter(mine('Created By')),
      batches = _lgRows('batches').filter(mine('Created By')), leads = _lgRows('leads').filter(mine('Created By')),
  ...
  var sumCount = function (rows) { return rows.reduce(function (s, r) { return s + _num(r['Count']); }, 0); };
  var dayCounts = function (d) {
    var b = on(batches, 'Sent On', d), c = on(calls, 'Date', d);
    return {
      plants: on(plants, 'Researched On', d).length,
      contacts: on(contacts, 'Verified On', d).length,
      introEmails: sumCount(b.filter(function (r) { return r['Kind'] === 'Intro'; })),
      followupEmails: sumCount(b.filter(function (r) { return r['Kind'] === 'Follow-up'; })),
      coldCalls: c.filter(function (r) { return r['Kind'] === 'Cold'; }).length,
      followupCalls: c.filter(function (r) { return r['Kind'] === 'Follow-up'; }).length,
  ...
  var quotas = {};
  _LG_COUNT_KEYS.forEach(function (k) { quotas[k] = _num(settings[_LG_QUOTA_KEY[k]]); });
  var days = [], byDay = {}, working = [], totals = { replies: 0 };
  _LG_COUNT_KEYS.forEach(function (k) { totals[k] = 0; });
  for (var i = 0; i < 7; i++) {
    ...
    working.push(d);
    _LG_COUNT_KEYS.forEach(function (k) { totals[k] += c[k]; });
    totals.replies += c.replies;
  }
  var targets = {};
  _LG_COUNT_KEYS.forEach(function (k) { targets[k] = quotas[k] * working.length; });
  var day = dayCounts(date);
  return {
    success: true, today: today, hour: _lgHHmm(), date: date, day: day, quotas: quotas,
    week: { start: weekStart, end: days[6], days: days, workingDays: working, targets: targets, totals: totals,
            byDay: byDay, replies: totals.replies,
...
function getSalesCalls(p) {
  ...
  return { success: true, data: rows.map(function (r) {
    return {
      ...
      kind: r['Kind'] || '', contactNo: r['Contact No'] || '',
      rowIndex: r.rowIndex
    };
  }) };
}
...
function logSalesCall(p) {
  ...
  _append('SalesCalls', [no, date, p.actorName || '', contact, company, p.outcome || '',
    p.notes || '', _now(), kind, contactNo]);                                             // 10 values
  if (lg) {
...
var _FLOW_SETTING_DEFAULTS = {
  ...
  lgQuotaPlants: 15, lgQuotaContacts: 20, lgQuotaIntroEmails: 40, lgQuotaFollowupEmails: 30,
  lgQuotaColdCalls: 20, lgQuotaFollowupCalls: 10, lgQuotaLeads: 3, lgQuotaMeetings: 1,
