...
function _writeMigratedRecordsForSO(cd, force) {
  ...
  if (force || (!hasReal && !hasMigInv)) {
    ...
    if (force && existing) {
      ...
      itSh.appendRow([eNo, '(migrated)', 'Migrated legacy sales', 1, sales, sales, cogs, cogs, '']);
      wroteInv = true;
    } else {
