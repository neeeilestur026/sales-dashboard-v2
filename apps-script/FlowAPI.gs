...
var SCHEMA = {
  ...
  Quotations:     ['Quotation No', 'Date', 'Customer', 'Status', 'Total', 'Created By', 'Created At', 'PDF Link',
                   ...
                   // A215. 'Sent At Basis' says HOW the send date was arrived at. Blank means it was
                   // recorded as it happened. Anything else means it was ESTIMATED by the backfill,
                   // and every surface that shows the date must say so — 60 of 85 live quotations
                   // were marked Sent before the stamp existed, and an inferred date that looks
                   // identical to a real one is how a follow-up clock starts lying.
  ...
  //    A278: 'VAT Rate' and 'VAT' appended at the END. 'Total Sales' is UNCHANGED and still NET —
  //    it is the revenue figure ~15 P&L/KPI readers take it for, and moving it would have traded an
  //    AR bug for a worse one. The VAT AMOUNT is stored because it is the number the journal posted,
  //    to the centavo; a reader re-deriving it can round differently and then the ledger and the
  //    invoice disagree with no way to tell which is right. The RATE is stored because it prints on
  //    the document, it keeps a 2026 invoice reproducible after the rate moves, and it cannot be
  //    back-solved (net is 0 on a pure-deposit invoice). 'Total Due' is deliberately NOT stored: it
  //    is Total Sales + VAT + Total Deposit, three cells on one row, so getInvoices derives it.
  //    Legacy rows read blank → _num('') === 0 → exactly their old behaviour, everywhere.
  Invoices:     ['INV No', 'SO No', 'Date', 'Customer', 'Total Sales', 'Total COGS', 'Created By', 'Created At',
                 'Voided', 'Void Reason', 'Total Deposit', 'VAT Rate', 'VAT'],
  /* A282 — the last three are the hire shape, appended at the END (house convention). A goods line
     leaves all three blank and reads exactly as it always has. They are stored because 'Line Sales'
     alone cannot be checked: 2 wrenches x 11,015.61 x 7 days is a number no reader can verify from
     Qty and Selling Price, and the invoice screen re-reads these rows. */
  InvoiceItems: ['INV No', 'Item No', 'Item Name', 'Qty', 'Selling Price', 'Line Sales', 'Landed Cost/Unit', 'Line COGS',
                 'Item ID', 'Charge Kind', 'Rate Basis', 'Duration'],
  ...
  /*    A242: 'Quoted On' — WHICH QUOTATION THIS LINE WENT OUT ON, blank until it does. It is what
        makes a partial quotation possible: quote the 3 lines management has priced, leave the other
        2 on the request, quote them later on a second quotation. Before this there was no per-line
        state at all beyond `Included`, so "later" could only be expressed as "never".
        APPENDED AT THE END, and like Salesperson above that is not style: updatePRSourcing writes a
        6-wide positional block at col 8 and rejectMgmtPricing writes one at col 9, so an INSERT
        would shift every live row and land those blocks in the wrong cells. Written only through a
        targeted SCHEMA.indexOf()+1 single-cell write, the same way 'Supplier Price VAT' is.
        NEVER CLEARED. "Can this line be quoted" is DERIVED — blank, or the quotation it names no
        longer exists or was Cancelled — so a crash between creating a quotation and stamping its
        lines heals itself instead of leaving a line nobody can quote. */
  PricingRequestItems: ['PR No', 'Line', 'Item No', 'Item Name', 'Qty', 'UOM', 'Remarks', 'Included',
                        ...
                        'Quoted On'],   // A242: the quotation number this line was quoted on
  ...
  // A277 — 'Stage' APPENDED: 'Prospect' on a row the lead-gen hand-off created, blank on a real
  // customer. saveClient writes this sheet as a LITERAL of SCHEMA.Clients.length values, so any
  // further column goes at the end and that literal grows with it, or saveClient throws.
  Clients:   ['Customer', 'Address', 'Contact Person', 'Designation', 'Email', 'Phone', 'RFQ Ref',
              'Payment Terms', 'Notes', 'Updated By', 'Updated At', 'Stage'],
  ...
  // ── A277 Lead generation ──
  // Every counted thing is a server-stamped Manila date on its own row ('… On'); the last three
  // columns are the audit trio. 'Deleted On' is the soft delete. See the LEADGEN block.
  LgPlants:     ['Plant No', 'Company', 'Plant / Site', 'Sector', 'Province', 'Territory', 'Equipment',
                 'PhilGEPS', 'Source', 'Status', 'Next Action', 'Next Action Date', 'Notes',
                 'Researched On', 'Deleted On', 'Created By', 'Created At', 'Updated At',
                 'Intel Updated On'],   // A279 — stamped when an EXISTING account's research is updated
  LgContacts:   ['Contact No', 'Plant No', 'Name', 'Role', 'Email', 'Mobile', 'LinkedIn', 'Email Verified',
                 'Status', 'Intro Sent', 'Replied On', 'Verified On', 'Notes', 'Deleted On',
                 'Created By', 'Created At', 'Updated At',
                 'Next Call Date', 'Scheduled On'],   // A279 — a follow-up call booked for a coming day, stamped when set
  // One row per contact per email batch or logged call. 'Ref No' is the batch or call it came
  // from, so removing that record takes its touches out of the follow-up maths.
  LgTouches:    ['Touch No', 'Contact No', 'Channel', 'Kind', 'Ref No', 'Sent On', 'Created By', 'Created At'],
  // 'Count' is DERIVED from 'Contact Nos' by the server; a client-sent count is discarded.
  LgEmailBatch: ['Batch No', 'Date', 'Time Slot', 'Kind', 'Sector', 'Template', 'Contact Nos', 'Count',
                 'Notes', 'Sent On', 'Deleted On', 'Created By', 'Created At', 'Updated At'],
  LgLeads:      ['Lead No', 'Plant No', 'Contact No', 'Territory', 'Handed To', 'Right Person',
                 'Own Maintenance', 'Flanged Or Hydraulic', 'Said Yes', 'Pain', 'What They Said',
                 'Next Step', 'Next Step Date', 'Presentation Date', 'Attendees', 'Status', 'Client Created',
                 'Handed Off On', 'Rehanded On', 'Returned On', 'Return Reason', 'Booked On',
                 'Notes', 'Deleted On', 'Created By', 'Created At', 'Updated At'],
  LgAccred:     ['Accred No', 'Plant No', 'Company', 'Status', 'Submitted', 'Approved', 'Expiry',
                 'Docs Sent', 'Notes', 'Started On', 'Deleted On', 'Created By', 'Created At', 'Updated At'],
  // A279 — local suppliers researched for local procurement. Handing one off fills the Suppliers
  // master (fill-blanks only), the mirror of a lead becoming a Prospect client.
  LgSuppliers:  ['Supplier No', 'Company', 'Category', 'Location', 'Contact', 'Email', 'Mobile', 'Website',
                 'Status', 'Notes', 'Researched On', 'Qualified On', 'Handed Off On', 'Deleted On',
                 'Created By', 'Created At', 'Updated At'],

  // ── Sales call log (per rep, per day) ──
  // A277 — 'Kind' (Cold / Follow-up) and 'Contact No' APPENDED for the lead-gen call logger, so
  // the two call quotas count separately. A rep's ordinary call leaves both blank and is untouched.
  // A279 — 'Reached' APPENDED: who actually answered. A "real conversation" is one that reached the
  // decision-maker, and the outcome alone could not say (a gatekeeper can be "Interested" too).
  SalesCalls: ['Call No', 'Date', 'User', 'Contact', 'Company', 'Outcome', 'Notes', 'Created At', 'Kind', 'Contact No', 'Reached'],
...
var COA = [
  ...
  /* A278 — OUTPUT VAT, the sales side. 1500 above is the PURCHASE side (Input VAT, debited by
     createReceiving) and the two are never netted in this ledger. Until now the sales side booked
     nothing at all: the 12% was added only when the quotation PDF rendered and was never persisted,
     so 1200 was debited NET at invoice and credited GROSS at collection and the difference simply
     accumulated as an unexplained balance on the receivable account.
     LISTED HERE, not merely posted to — the same trap the 2100/4100 note above is about.
     NOTHING DEBITS 2200. There is no VAT-remittance action anywhere in this system, so the balance
     accrues from here to whenever one is built. That is the honest reading of what exists and it
     makes the gap visible on the trial balance instead of hiding it inside 1200. */
  ['2200', 'Output VAT Payable', 'Liability', 'Credit'],
  ['4100', 'Service Revenue', 'Revenue', 'Credit'],
  ['5000', 'Cost of Goods Sold', 'Expense', 'Debit']
];
var ACC = { CASH: '1010', AR: '1200', INV: '1300', CLEARING: '1400', INPUT_VAT: '1500', CWT: '1600', AP: '2010', SALES: '4000', COGS: '5000', DEPOSITS: '2100', SERVICE: '4100', OUTPUT_VAT: '2200' };
...
/* A270 — getShipments took 36.7 SECONDS for 16 rows. _rows() has no cache and re-reads the whole
   sheet on every call, and getShipments derives each shipment's supplier kind and full 25-stage
   timeline per row: _soSupplierKind reads 2 sheets and _shipAutoDerive reads 6, so 16 shipments
   meant ~128 full-sheet reads of Invoices, ARAging, MaterialsReceiving, SalesOrders and the rest.
   The page timed out.

   This memo is deliberately OPT-IN and short-lived rather than a cache inside _rows(): plenty of
   write paths append a row and then re-read to see it, and a always-on cache would hand them stale
   data. It is opened around one read-only call and closed in a finally, so everywhere else _rowsMemo
   falls straight through to _rows and behaves exactly as before. */
var _ROWS_MEMO = null;

function _rowsMemo(name) {
  if (!_ROWS_MEMO) return _rows(name);
  if (!_ROWS_MEMO[name]) _ROWS_MEMO[name] = _rows(name);
  return _ROWS_MEMO[name];
}
...
var _SECURED = {
  /* A277 — the lead-gen rows are one person's performance record and every count is an event the
     server stamps against `Created By`, so the actor has to be the login session and not a field
     the browser filled in. These two are NEW actions: nothing deployed calls them, so securing
     them costs nothing. The reads stay open.

     A277-3 — logSalesCall and deleteSalesCall were here too, and must NOT come back without the
     deploy order below. They are EXISTING actions: every sales rep's report.html has logged calls
     through them for months. Adding an existing action to this list breaks it for everyone the
     instant this file is pasted, because the browser decides whether to route through Flask from
     its OWN FLOW_SECURED_ACTIONS — which is only updated by a Render deploy. That is what took the
     call log down live.

     TO SECURE AN EXISTING ACTION: ship dashboard/js/flow-api.js + blueprints/flow.py to Render
     FIRST (the client then routes it through /flow/secure, and an old Apps Script simply ignores
     the extra flowSecret), confirm it works, and only then paste this file. Never the other way
     round, and never both in the same breath. */
  saveLeadgenRecord: 1, deleteLeadgenRecord: 1,
  ...
  /* A215 — runQuotationSentAtBackfill decides who may rewrite the send date on up to 60 quotations
  ...
  // A243 — the quotation twin of setPricingRequestSalesperson, secured for the same reason it is.
  setQuotationSalesperson: 1,
  ...
  /* A249 — backfillMissingAR CREATES RECEIVABLES, off a browser-supplied call, with no role check of
     its own. Every other action that decides or moves money already has to arrive through Flask with
     a validated login session; this one minted debt on the strength of knowing the /exec URL. */
  backfillMissingAR: 1,
  // A278 — moves receivables, off a browser-supplied call with no role check of its own.
  applyInvoiceVatRepair: 1,
...
/* A209 — commission requests are built but NOT open to everyone yet.
   ...
   A233 — LAUNCHED. All three roles, on the director's instruction. This is the half that actually
   lets an action through: _featureBlocked below reads it, so until this file is pasted AND redeployed
   as a NEW VERSION, every commission action is still refused no matter what the browser shows.

   Still not the security boundary. For an unsecured action `actorRole` is whatever the browser said,
   so this list decides ROLLOUT only — _SECURED and _commMayActOn are what protect the money, and
   both are unchanged.

   TO CLOSE AGAIN: empty this and FLOW_COMMISSIONS_ROLES in dashboard/js/flow-api.js. Unlike the
   pre-launch hold, closing now strands reps' in-flight requests rather than costing nothing. */
var _COMM_ROLES = ['director', 'management', 'sales'];

var _COMM_ACTIONS = {
  getCommissionRequests: 1, getCommissionClaimable: 1, getCommissionPreview: 1,
  previewCommissionAttribution: 1,          // A239 — a commission read, so the same hold covers it
...
function getQuotations(p) {
   ...
   *
   * A246 — ...but OWNER ALONE hides a rep's own typing from them, which is the mirror of the bug
   * A218 fixed and turned out to be just as bad. A218's own note records that one person typed 46 of
   * 85 quotations while owning 27: those ~19 were unfindable by the only person who could correct
   * them. It surfaced as something that looked like a different bug entirely — a rep could not find
   * quotation 2026-457, so she retyped it and got "Quotation No already exists", with nothing on
   * screen explaining why she could not see a record she had created herself. Both rows carried an
   * owner of "Neil", a bare first name matching no account, so they belonged to nobody and appeared
   * for nobody.
   *
   * So the scope is owner OR creator. It widens nothing dangerous — a person only ever gains sight of
   * rows they typed themselves — and it makes "I made this, so I can find it" true, which is the
   * assumption every rep was already working under. The owner still decides attribution everywhere
   * else: whose tracker it counts in, whose commission it feeds, what the board reports. */
  if (p && p.createdBy) headers = headers.filter(function (q) {
    return _quoOwner(q) === String(p.createdBy)
        || String(q['Created By'] || '').trim() === String(p.createdBy).trim();
  });
  ...
  return { success: true, data: headers.map(function (q) {
    ...
    return {
      ...
      // A215 — blank basis means the date was recorded as it happened. Anything else means it was
      // ESTIMATED, and every surface showing the date has to say so.
...
/* ── A282 · A HIRE HAS TWO MULTIPLIERS ───────────────────────────────────────────────────────
 *
 * A276 modelled a rental line as a rate over a duration and put the DURATION in the quantity
 * column: seven days of a wrench was "qty 7". It reads correctly for one tool and cannot express
 * two — quoting a pair of wrenches for a week had no honest spelling at all, and 'Duration' was
 * stored beside it as a copy of qty that nothing ever multiplied by.
 *
 * So a hire line now carries BOTH: Qty is how many tools, Duration is for how long, and the amount
 * is qty x rate x duration. A supply line is unchanged in every respect — it has no duration, the
 * span below is 1, and qty x price comes out of the same expression it always did.
 *
 * WHICH LINES GET A SPAN is decided by the RATE BASIS, not by the charge kind, and it is the same
 * rule the PDF already used to decide whether a rate prints "/ DAY" beside it: a rate per unit of
 * TIME is multiplied by a duration; a flat charge per LOT or per PC is not. That matters for the
 * two lines where getting it wrong costs money — a mobilization is one flat fee however long the
 * tool is out, and a refundable DEPOSIT is per tool, not per day. Neither has a time basis, so
 * neither can be multiplied by one, whatever a caller sends in the duration field.
 */
var _RATE_TIME_UNITS = { DAY: 1, DAYS: 1, WEEK: 1, WEEKS: 1, MONTH: 1, MONTHS: 1, HOUR: 1, HOURS: 1,
                         MANDAY: 1, MANDAYS: 1, SHIFT: 1, SHIFTS: 1 };

/** True when this line's rate is per unit of TIME, and so is multiplied by a duration. */
function _isTimeBasis(basis) {
  return _RATE_TIME_UNITS[String(basis || '').trim().toUpperCase().replace(/\.$/, '')] === 1;
}

/** How many rate-units this line is charged for: its duration on a time-based rate, otherwise 1. */
function _lineSpan(it) {
  if (!it) return 1;
  /* THE RATE BASIS, and never the UOM. They hold the same word on a hire line (the form writes
     both from one control), but a SUPPLY line has a UOM and no basis — and a supply line whose unit
     happened to read "DAYS" would otherwise be multiplied by a duration field it never meant to
     fill. Basis is set by the service form and by nothing else, so reading it alone is what makes
     "a supply quotation never spans" true by construction rather than by the caller's good manners. */
  var basis = it.rateBasis !== undefined ? it.rateBasis : it['Rate Basis'];
  if (!_isTimeBasis(basis)) return 1;
  var d = _num(it.duration !== undefined ? it.duration : it['Duration']);
  return d > 0 ? d : 1;      // a time rate with no duration is one unit, never zero
}

/** THE line amount. Every total in this file goes through here. */
function _lineAmount(it) {
  return _num(it.qty) * _num(it.price) * _lineSpan(it);
}
...
function _quotationRecommended(items, recommended) {
  ...
  (items || []).forEach(function (it) {
    ...
    if (!k) return;
    groups[k] = (groups[k] || 0) + _lineAmount(it);
  });
...
function createQuotation(p) {
  ...
  if (p.quotationNo) {
    ...
    /* A246 — say WHOSE it is and what state it is in. "already exists" alone is a dead end when the
       reason you are retyping it is that you could not see it: the rep has no way to tell whether
       they typo'd a number, whether it is theirs, or who to ask. Naming the owner and status turns it
       into something actionable in one read. */
    if (clash) {
      var clashRow = _rows('Quotations').filter(function (r) {
        return String(r['Quotation No']).toLowerCase() === wanted; })[0] || {};
      var clashWho = _quoOwner(clashRow) || String(clashRow['Created By'] || '').trim();
      return { success: false, existing: String(clashRow['Quotation No'] || p.quotationNo),
        message: 'Quotation No "' + p.quotationNo + '" already exists' +
          (clashWho ? ' — it belongs to ' + clashWho : '') +
          (clashRow['Status'] ? ' and is ' + clashRow['Status'] : '') +
          '. Open that quotation and edit it rather than creating a second one with the same number.' };
    }
  }
  ...
  _writeItems('QuotationItems', 'Quotation No', no, items, function (it) {
    return [no, it.itemNo, it.itemName, _num(it.qty), _num(it.price), _lineAmount(it),
...
function updateQuotation(p) {
  ...
  /* A281 — SUPPLY or SERVICE, on the edit path too. createQuotation has written these two since
     A276 and updateQuotation never touched them, so a quotation's type was fixed for ever at
     creation: a rep who typed a hire as a sale (or the reverse) could correct the lines, the
     charge kinds and the document, and the RECORD would still say the wrong thing — which is what
     createSalesOrder and createInvoice read to decide whether a tool leaves stock.
     A174 rule: an unsent field is left alone. Sending quoteType='' is a real value (a supply
     quotation) and clears it on purpose; not sending it at all leaves the column untouched. */
  if (p.quoteType !== undefined) {
    _setCellByKey('Quotations', 'Quotation No', newNo, 'Type', String(p.quoteType || '').trim());
  }
  if (p.serviceKind !== undefined) {
    _setCellByKey('Quotations', 'Quotation No', newNo, 'Service Kind', String(p.serviceKind || '').trim());
  }
  ...
  if (p.items !== undefined || newNo !== String(no)) _writeItems('QuotationItems', 'Quotation No', no, items, function (it) {
    ...
    return [newNo, it.itemNo, it.itemName, _num(it.qty), _num(it.price), _lineAmount(it),
...
function createSalesOrder(p) {
  ...
  var total = 0;
  items.forEach(function (it) { total += _lineAmount(it); });
  ...
  incoming.forEach(function (c) {
    try {
      ...
      if (recv > 0) {
        ...
        /* A265 — Method, Reference No and Notes were hard-coded to '' / 'Migrated (legacy)', so a
           reconciliation that KNOWS how each payment arrived (bank transfer vs cheque, and its
           reference) threw that away on import and there was no unsecured path to put it back.
           Absent fields keep the old behaviour exactly, per A174. */
        _append('Collections', [colNo, arNo, invNo, c.soNo || '', customer,
          c.dateCollected || c.date || _dateStr(_now()), recv, c.method || '', c.ref || '',
          c.notes || 'Migrated (legacy)', _now(),
...
function correctCollection(p) {
  ...
  /* A248 — a VOIDED sibling is not money and must not count here. Every other over-collect
     comparison in this file filters it; this one did not, so a receivable that had been corrected
     once already would refuse the next legitimate re-split with "that would collect X against an
     amount due of Y" — a figure that included a payment somebody had reversed. */
  var others = rows.filter(function (r) {
    return String(r['AR No']) === arNo && String(r['Collection No']) !== String(p.collectionNo)
      && String(r['Voided'] || '') !== 'true';
  }).reduce(function (s, r) { return s + _num(r['Amount (PHP)']); }, 0);
  var amt = _num(ar['Amount (PHP)']);
...
/* A252 — RENAME AN INVOICE NUMBER. The business issues its own invoice numbers on its own paper;
   the sequence this system mints is an internal placeholder, and until now there was no way to put
   the real number on the record. Every downstream money row keys on the string, so this is a re-key,
   not an edit: ARAging, Collections, InvoiceItems, CommissionRequestItems, the filed documents and
   the GL entry all have to move together or the invoice fragments.

   Modelled on renameSalesOrder, including its two hard rules: the collision test is
   case-INSENSITIVE (two invoices differing only in case are the same number to a human reading a
   statement), and a claim already in flight FREEZES the number, because _commPriorClaimed matches by
   string and a half-moved rename would let the same collection be claimed twice.

   The sweep is schema-driven rather than a hand-kept list, so a sheet that gains an 'INV No' column
   later cannot be forgotten here. */
function renameInvoice(p) {
  if (!p || !p.invNo) return { success: false, message: 'invNo required.' };
  var oldNo = String(p.invNo).trim();
  var newNo = String(p.newInvNo == null ? '' : p.newInvNo).trim();
  if (!newNo) return { success: false, message: 'newInvNo required.' };
  if (newNo === oldNo) return { success: true, invNo: oldNo, renamed: false, message: 'The number is unchanged.' };

  var inv = _rows('Invoices').filter(function (r) { return String(r['INV No']) === oldNo; })[0];
  if (!inv) return { success: false, message: 'Invoice ' + oldNo + ' not found.' };

  var lower = newNo.toLowerCase();
  var clash = _rows('Invoices').some(function (r) {
    return String(r['INV No']).trim().toLowerCase() === lower && String(r['INV No']) !== oldNo;
  });
  if (clash) return { success: false, message: 'Invoice No "' + newNo + '" already exists.' };

  /* A live commission claim freezes the number onto a payable — same reasoning as renameSalesOrder. */
  var held = {};
  _rows('CommissionRequests').forEach(function (r) {
    if (_COMM_LOCKING[String(r['Status'] || '')]) held[String(r['Comm No'])] = 1;
  });
  var locked = _rows('CommissionRequestItems').filter(function (r) {
    return String(r['INV No']) === oldNo && held[String(r['Comm No'])];
  });
  if (locked.length) {
    return { success: false, message: 'Cannot rename ' + oldNo + ' — ' + locked.length
      + ' commission claim line(s) are in flight against it ('
      + locked.map(function (r) { return r['Comm No']; }).join(', ') + '). Settle or withdraw them first.' };
  }

  /* Documents already filed follow the rename. Said before the write, not after, so the person
     pressing the button knows what moves with it. */
  var docs = _rows('Documents').filter(function (d) {
    return String(d['Module']) === 'Invoice' && String(d['Ref No']) === oldNo;
  });
  if (docs.length && !p.confirmDocs) {
    return { success: false, needsConfirm: 'renameDocs', invNo: oldNo, newInvNo: newNo,
      docCount: docs.length,
      message: docs.length + ' document(s) are filed against ' + oldNo + '. They will be re-filed under '
        + newNo + ' and their links keep working. Confirm to proceed.' };
  }

  // ── the re-key: every sheet carrying an 'INV No', found from the schema ──────────────────────
  var moved = {};
  Object.keys(SCHEMA).forEach(function (name) {
    var col = SCHEMA[name].indexOf('INV No') + 1;
    if (col < 1) return;
    var sh = _sheet(name), n = 0;
    _rows(name).forEach(function (r) {
      if (String(r['INV No']) === oldNo) { sh.getRange(r.rowIndex, col, 1, 1).setValues([[newNo]]); n++; }
    });
    if (n) moved[name] = n;
  });

  // Documents are polymorphic: Module + Ref No, where Module 'Invoice' means Ref No IS the INV No.
  var docSh = _sheet('Documents');
  var refCol = SCHEMA.Documents.indexOf('Ref No') + 1;
  var docsMoved = 0;
  ...
  /* The GL. _postJournal keys an entry as 'JE-' + source + '-' + sourceNo and repeats the number in
     each memo, so three things move: the entry number, the source number, and the number printed in
     the memo a person actually reads in the ledger. The memo is rewritten by exact-substring swap —
     never a loose replace, which would also rewrite a number that merely contains this one. */
  var jSh = _sheet('Journal');
  var jEntry = SCHEMA.Journal.indexOf('Entry No') + 1;
  var jSrcNo = SCHEMA.Journal.indexOf('Source No') + 1;
  var jMemo  = SCHEMA.Journal.indexOf('Memo') + 1;
  var jMoved = 0;
  _rows('Journal').forEach(function (r) {
    if (String(r['Source']) !== 'INV' || String(r['Source No']) !== oldNo) return;
    jSh.getRange(r.rowIndex, jEntry, 1, 1).setValues([['JE-INV-' + newNo]]);
    jSh.getRange(r.rowIndex, jSrcNo, 1, 1).setValues([[newNo]]);
    var memo = String(r['Memo'] || '');
    if (memo.indexOf(oldNo) >= 0) {
      jSh.getRange(r.rowIndex, jMemo, 1, 1).setValues([[memo.split(oldNo).join(newNo)]]);
    }
    jMoved++;
  });
  ...
  return { success: true, invNo: newNo, previousInvNo: oldNo, renamed: true, moved: moved,
    message: 'Invoice ' + oldNo + ' is now ' + newNo + '. '
      + Object.keys(moved).map(function (k) { return moved[k] + ' ' + k; }).join(', ') + ' re-keyed.' };
}

/* A265 — AN AR ROW OR COLLECTION COULD NEVER BE ATTACHED TO ITS SALES ORDER. updateARAging writes
   only Due Date, Notes and Status, and nothing anywhere else in the system sets 'SO No' on an
   existing money record. 47 migrated rows — receivables and payments that ARE recorded, just not
   linked — were therefore permanently orphaned: they do not roll up to any order, so the order looks
   uncollected while the cash sits in the book. Verified by hand against the live ledger before this
   was written.

   Deliberately narrow: it sets ONE cell, on one of two sheets, and only to a sales order that
   actually exists. Re-pointing a row that already carries a different SO needs confirmReattach, so a
   wrong sweep cannot silently move money between orders. */
function attachOrphanToSO(p) {
  var type = String(p.type || '').trim().toLowerCase();
  var recordNo = String(p.recordNo || '').trim();
  var soNo = String(p.soNo || '').trim();
  if (!recordNo || !soNo) return { success: false, message: 'recordNo and soNo are required.' };

  var sheet, keyCol;
  if (type === 'ar') { sheet = 'ARAging'; keyCol = 'AR No'; }
  else if (type === 'collection') { sheet = 'Collections'; keyCol = 'Collection No'; }
  else return { success: false, message: 'type must be "ar" or "collection".' };

  var soOk = _rows('SalesOrders').some(function (r) { return String(r['SO No']) === soNo; });
  if (!soOk) return { success: false, message: 'Sales order "' + soNo + '" not found.' };

  var row = _rows(sheet).filter(function (r) { return String(r[keyCol]) === recordNo; })[0];
  if (!row) return { success: false, message: recordNo + ' not found in ' + sheet + '.' };

  var was = String(row['SO No'] || '');
  if (was === soNo) {
    return { success: true, recordNo: recordNo, soNo: soNo, previous: was, unchanged: true,
      message: recordNo + ' is already attached to ' + soNo + '.' };
  }
  if (was && !p.confirmReattach) {
    return { success: false, needsConfirm: 'reattach', current: was,
      message: recordNo + ' is already attached to ' + was + '. Re-attach it to ' + soNo + '?' };
  }
  _setCellByKey(sheet, keyCol, recordNo, 'SO No', soNo);
  if (sheet === 'ARAging') _setCellByKey(sheet, keyCol, recordNo, 'Updated At', _now());
  return { success: true, recordNo: recordNo, soNo: soNo, previous: was,
    message: recordNo + ' attached to ' + soNo + '.' };
}

/* A265 — the companion to the importCollections fix above. That fix stops Method / Reference /
   Notes being discarded on FUTURE imports; this one can put them back on a payment already
   imported without them. Metadata only: it cannot touch AR No, SO No, Date, Amount or EWT, so it
   can never move money or change what a receivable is owed. */
function setCollectionMeta(p) {
  var no = String(p.collectionNo || '').trim();
  if (!no) return { success: false, message: 'collectionNo required.' };
  var row = _rows('Collections').filter(function (r) { return String(r['Collection No']) === no; })[0];
  if (!row) return { success: false, message: no + ' not found.' };
  if (String(row['Voided'] || '') === 'true') {
    return { success: false, message: no + ' is voided.' };
  }
  var wrote = [];
  // Per A174: a field that is not sent is LEFT ALONE, never blanked.
  if (p.method !== undefined && String(p.method).trim()) {
    _setCellByKey('Collections', 'Collection No', no, 'Method', String(p.method).trim());
    wrote.push('Method');
  }
  if (p.ref !== undefined && String(p.ref).trim()) {
    _setCellByKey('Collections', 'Collection No', no, 'Reference No', String(p.ref).trim());
    wrote.push('Reference No');
  }
  if (p.notes !== undefined && String(p.notes).trim()) {
    _setCellByKey('Collections', 'Collection No', no, 'Notes', String(p.notes).trim());
    wrote.push('Notes');
  }
  if (!wrote.length) return { success: true, collectionNo: no, message: 'Nothing to change.' };
  return { success: true, collectionNo: no, updated: wrote,
    message: no + ' updated (' + wrote.join(', ') + ').' };
}
...
var _DOC_TYPE_ALIASES = {
  ...
  /* A251 — the wordings the bank's own paperwork actually carries. 'Telegraphic Transfer Form' is
     the one PRF-2026-93 was filed under; the rest are the same instruction under its other trade
     names. They are the TT the pay gate asks for, not the debit memo the receive gate asks for —
     that one is the bank's confirmation AFTER the money moved, and stays 'debit_memo'. */
  'telegraphic transfer form': 'tt_sent', 'telegraphic transfer application form': 'tt_sent',
  'tt application form': 'tt_sent', 'tt form': 'tt_sent', 'tt slip': 'tt_sent',
  'bank remittance slip': 'tt_sent', 'remittance slip': 'tt_sent',
  'outward remittance': 'tt_sent', 'wire transfer': 'tt_sent', 'swift copy': 'tt_sent',
  'signed delivery receipt': 'delivered_client', 'customer delivery receipt': 'delivered_client'
};

/* ── A251 · the fallback pass ────────────────────────────────────────────────────────────────────
 * The table above matches the WHOLE string, exactly. So "Telegraphic Transfer Form" walked straight
 * past the 'telegraphic transfer' entry sitting next to it, stayed free text, matched no rule, and
 * PRF-2026-93 was refused for a TT slip that was attached and named on screen. Listing every wording
 * by hand only moves the next miss one word further out — "TT Form (signed)", "TT application copy".
 *
 * So: after the exact table misses, strip the words that decorate a document's name without changing
 * WHICH document it is, and look again. "TT", "TT form" and "Telegraphic Transfer Application Form"
 * all reduce to the same thing.
 *
 * Two deliberate choices:
 *   • only '/' splits a name, not ' and ' / ' or '. A slash means one document written two ways —
 *     "TT form / bank remittance slip" — so either half identifies it. "and" and "or" join names
 *     that may be two different documents, and deciding that one of them is enough is a policy call
 *     for _DOC_RULES to make, not for a string function to make on its behalf.
 *   • the rule LABELS are folded in as aliases automatically. A refusal message quotes the label, so
 *     the label is what people type back into the Doc Type box — this is the exact round trip that
 *     failed here. Deriving them means the vocabulary cannot drift from _DOC_RULES.
 * Nothing here can widen a match to a DIFFERENT document: every candidate must still land on a key
 * that some rule or alias already owns, or the original free text is returned unchanged. */
var _DOC_TYPE_NOISE = /\b(?:form|forms|application|slip|copy|copies|document|documents|doc|docs|file|scan|scanned|pdf|attachment|attached|the|a|an)\b/g;

function _docTypeNorm(s) {
  return String(s || '')
    .toLowerCase()
    .replace(/\([^()]*\)/g, ' ')       // "(stamped)", "(goods received at our office)"
    .replace(/[^a-z0-9 ]+/g, ' ')      // punctuation, apostrophes, the '_' in stage keys
    .replace(_DOC_TYPE_NOISE, ' ')
    .replace(/\s+/g, ' ')
    .trim();
}

var _DOC_TYPE_NORM_INDEX = null;
function _docTypeNormIndex() {
  if (_DOC_TYPE_NORM_INDEX) return _DOC_TYPE_NORM_INDEX;
  var idx = {};
  // First writer wins, so the order below is the precedence order.
  var put = function (text, target) {
    var n = _docTypeNorm(text);
    if (n && !idx[n]) idx[n] = target;
  };
  _DOC_RULES.forEach(function (r) { put(r.type, r.type); });          // a type resolves to itself
  Object.keys(_DOC_TYPE_ALIASES).forEach(function (k) { put(k, _DOC_TYPE_ALIASES[k]); });
  _DOC_RULES.forEach(function (r) {                                   // then the labels
    put(r.label, r.type);
    String(r.label || '').split('/').forEach(function (part) { put(part, r.type); });
  });
  _DOC_TYPE_NORM_INDEX = idx;
  return idx;
}
...
function _docTypeKey(s) {
  ...
  t = t.replace(/\s+/g, ' ');
  if (_DOC_TYPE_ALIASES[t]) return _DOC_TYPE_ALIASES[t];
  var idx = _docTypeNormIndex();
  var n = _docTypeNorm(t);
  if (n && idx[n]) return idx[n];
  var parts = t.split('/');
  for (var i = 0; i < parts.length; i++) {
    var p = _docTypeNorm(parts[i]);
    if (p && idx[p]) return idx[p];
  }
  return t;                                                           // unknown: still free text
}
...
function _soSupplierKind(soNo) {
  var so = _rowsMemo('SalesOrders').filter(function (r) { return String(r['SO No']) === String(soNo); })[0];
  var t = so ? String(so['Supplier Type'] || '').trim().toLowerCase() : '';
  if (t === 'international') return 'intl';
  if (t === 'local') return 'local';
  try {
    var cd = _rowsMemo('SOCostDetails').filter(function (r) { return String(r['SO No']) === String(soNo); })[0];
...
function getInvoices(p) {
  ...
  return { success: true, data: invRows.map(function (v) {
    ...
    return {
      ...
      /* A278 — totalDeposit was never emitted at all (an A276 gap), so nothing downstream could see
         a deposit. totalDue is DERIVED here rather than stored: three cells on one row. */
      totalDeposit: _num(v['Total Deposit']), vatRate: _num(v['VAT Rate']), vat: _num(v['VAT']),
      totalDue: _num(v['Total Sales']) + _num(v['VAT']) + _num(v['Total Deposit']),
      ...
      items: its.map(function (r) { return {
        ...
        // A282 — the hire shape, so the invoice screen can show 2 x 7 days rather than a line
        // total no reader can arrive at from the two columns beside it.
        chargeKind: String(r['Charge Kind'] || '').trim(), rateBasis: String(r['Rate Basis'] || '').trim(),
        duration: _num(r['Duration']) }; })
    };
  }) };
}
...
function createInvoice(p) {
  ...
  /* A252 — a caller-supplied number was taken on trust. Now that the business types its own numbers
     that is a live hazard: _postJournal opens with _removeJournal(source, sourceNo), so re-using an
     existing number would have DELETED the first invoice's GL entry while leaving two rows sharing
     one number. Case-insensitive, matching renameInvoice. */
  if (p.invNo) {
    var wanted = String(p.invNo).trim().toLowerCase();
    var taken = _rows('Invoices').some(function (r) {
      return String(r['INV No']).trim().toLowerCase() === wanted;
    });
  ...
  /* A278 — the rate CHARGED on this invoice, stored so an exempt or zero-rated sale is a recorded
     fact rather than an absence, and so a historical invoice stays reproducible if the rate moves.
     THE DEFAULT IS 12, UNCONDITIONALLY. "Default to 0 when the caller sends no rate" would be a
     silent mode switch keyed on absence that never turns itself off: a page that simply forgot the
     field would quietly issue zero-VAT invoices and nothing would fail. */
  var vatRate = (p.vatRate === undefined || p.vatRate === null || String(p.vatRate).trim() === '')
                ? 12 : _num(p.vatRate);
  ...
  var lines = items.map(function (it) {
    ...
    /* A282 — qty x rate x DURATION. A hire line carries two multipliers: how many tools, and for
       how long. Only a rate per unit of TIME is spanned (see _lineSpan) — a mobilization per LOT is
       one flat fee however long the tool is out, and a deposit is per tool, not per day. A goods
       line has no basis and no duration, so span is 1 and lineSales is qty x price exactly as
       before; the COGS below is deliberately NOT spanned, because it costs the units issued. */
    var span = _lineSpan(it);
    var lineSales = qty * price * span;
    ...
    if (kind !== 'goods') {
      ...
      return [no, it.itemNo, it.itemName, qty, price, lineSales, 0, 0, it.itemId || '',
              String(it.chargeKind || ''), String(it.rateBasis || ''), span > 1 ? span : (_num(it.duration) || '')];
    }
    ...
    totalCOGS += lineCOGS; goodsSales += lineSales;
    return [no, it.itemNo, it.itemName, qty, price, lineSales, landed, lineCOGS,
            ...
            '', '', ''];                                   // A282: a goods line has no hire shape
  });
  /* A278 — ONE sum, in the SAME addend order the journal credits use. `totalSales` used to be
     accumulated inside the map while goodsSales/serviceSales accumulated separately, so on a mixed
     invoice the two sums added the same terms in a different order and floating point could put
     them a bit apart — on a journal whose debit has to equal its credits exactly. */
  var revenue = goodsSales + serviceSales;
  var totalSales = revenue;                 // the sheet column: NET, unchanged, still the P&L figure
  /* Rounded HERE AND ONLY HERE. This one value is written to the sheet, added into the receivable
     and credited in the journal, so all three agree to the centavo and nothing downstream re-rounds. */
  var vat = Math.round(revenue * (vatRate / 100) * 100) / 100;
  var totalDue = revenue + vat + depositTotal;
  _append('Invoices', [no, p.soNo || '', p.date || _now(), p.customer, totalSales, totalCOGS, p.createdBy || '', _now(),
    '', '', depositTotal, vatRate, vat]);   // A158 trailing: Voided / Void Reason. A276: + Total Deposit. A278: + VAT Rate, VAT
  ...
  /* A278 — the receivable is the GROSS the client must remit, and the credits split it into what we
     earned (net), the tax we collected on the state's behalf, and what we are merely holding. The
     debit is written as the same four addends in the same order as the four credit lines, so
     Dr 1200 === Cr 4000 + Cr 4100 + Cr 2200 + Cr 2100 bit-for-bit rather than within a tolerance.
     A ZERO-RATED INVOICE POSTS A BYTE-IDENTICAL ENTRY TO PRE-A278: _postJournal drops any line that
     is zero on both sides, so with vatRate 0 the 2200 line simply does not exist. */
  _postJournal('INV', no, p.date || _now(), 'PHP', [
    { account: ACC.AR, debit: goodsSales + serviceSales + vat + depositTotal, memo: 'Invoice ' + no + ' — ' + p.customer },
    { account: ACC.SALES, credit: goodsSales, memo: 'Sales ' + no },
    { account: ACC.SERVICE, credit: serviceSales, memo: 'Service revenue ' + no },
    { account: ACC.OUTPUT_VAT, credit: vat, memo: 'Output VAT ' + vatRate + '% on ' + no },
  ...
  return { success: true, invNo: no, arNo: arNo, zeroCogsLines: zeroCogsLines,
    ...
    /* A278 — the figures are SPELLED OUT deliberately. A browser tab left open from before this
       shipped posts no vatRate and gets the 12% default, which is correct but would otherwise be
       silent; this tells whoever pressed the button exactly what was charged. */
    message: 'Invoice issued for ' + totalDue.toFixed(2) + ' (net ' + totalSales.toFixed(2) +
      ' + VAT ' + vatRate + '% ' + vat.toFixed(2) +
      (depositTotal > 0 ? ' + deposit ' + depositTotal.toFixed(2) : '') +
      '); AR entry created, inventory deducted and journal posted.' +
...
function backfillMissingAR(p) {
  p = p || {};
  /* A249 — SCOPE. This took a parameter and never read it, so it was all-or-nothing across every
     invoice on the book: the only way to create the 9 receivables that are genuinely missing was to
     create 65, most of which are pre-baseline history the architecture excludes on purpose. An
     explicit list makes it a targeted correction instead of a mass event.
     Absent, it behaves exactly as before — a bare re-run is unchanged. */
  var only = null;
  if (p.invNos !== undefined && p.invNos !== null && p.invNos !== '') {
    try {
      var _l = JSON.parse(typeof p.invNos === 'string' ? p.invNos : JSON.stringify(p.invNos)) || [];
      only = {};
      _l.forEach(function (x) { only[String(x).trim()] = 1; });
    } catch (e) { return { success: false, message: 'Could not read the list of invoices to backfill.' }; }
    if (!Object.keys(only).length) return { success: false, message: 'No invoices were selected.' };
  }
  var hasAR = {};
  ...
  _rows('Invoices').forEach(function (v) {
    ...
    if (only && !only[invNo]) { skipped++; return; }        // A249 — outside the requested scope
    /* A248 — A VOIDED INVOICE IS SUPPOSED TO HAVE NO AR ROW.
       voidInvoice DELETES the receivable (ARAging has no 'Voided' column, unlike Invoices and
       Collections which are both flagged so the reversal stays auditable). So a voided invoice fits
       this function's definition of "missing its AR row" exactly — and without this line, pressing
       "Backfill missing AR rows" recreated a full-value Unpaid receivable for every invoice ever
       voided, with no journal behind it. The customer owed it again. */
    if (String(v['Voided'] || '') === 'true') { skipped++; return; }
    /* A278 — the same receivable createInvoice would have written. Two terms were missing: the VAT
       (new) and the DEPOSIT, which has been absent since A276 — backfilling an invoice carrying a
       deposit minted a receivable short by it. Both read blank → 0 on every legacy row, so a
       pre-A278 invoice backfills to exactly the number it always did. */
    var amt = _num(v['Total Sales']) + _num(v['VAT']) + _num(v['Total Deposit']);
    var recv = 0;
    _rows('Collections').forEach(function (c) {
      ...
      // A248 — a voided collection is history, not money. Every other reader filters it
      // (_arRecomputeFromCollections, recordCollection, voidInvoice, updateARAging); this one did
      // not, so a reversed payment inflated the backfilled Collected (PHP) and could mark a
      // receivable Paid that nobody had paid.
  ...
  var _missed = only ? Object.keys(only).filter(function (x) { return !hasAR[x]; }) : [];
  return { success: true, created: created, skipped: skipped, notFound: _missed,
    message: 'Created ' + created + ' AR row(s); ' + skipped + ' skipped.' +
      (_missed.length ? ' Not found or already had one: ' + _missed.join(', ') + '.' : '') };
}

/* ════════════════════════════════════════════════════════════════════════════════════════════════
   A278 — REPAIRING THE RECEIVABLES RAISED BEFORE OUTPUT VAT EXISTED.

   Every invoice issued before v153 booked its receivable NET while the client was billed GROSS. This
   pair corrects them: preview lists what would change and writes nothing; apply corrects an EXPLICIT
   list of invoices and nothing else.

   IT IMPUTES. Those invoices never recorded a rate, so there is no stored figure to recover — 12% is
   an assumption, stated per row in the preview so the person approving sees it line by line rather
   than once in a header. A batch that was genuinely exempt is run with rate 0, which stamps the fact
   and leaves the receivable alone.

   WHAT MAKES A ROW SAFE TO TOUCH is five stacked tests (_lgVatCandidates below). The decisive one is
   arithmetic: a native pre-A278 receivable is EXACTLY its invoice's net + deposit, because that is
   what createInvoice wrote. A migrated receivable is the gross less 1% withholding — about 11% away,
   which is four orders of magnitude outside the 0.005 tolerance. Notes are corroboration only: the
   AR notes field is a free-text input on the aging page, so anybody can overwrite it.
   ════════════════════════════════════════════════════════════════════════════════════════════════ */

/** The raw cell test. `_num(0) === _num('')`, so _num cannot tell a stamped zero-rated invoice from a
 *  legacy row that never carried a rate at all — and re-VATing the first would be a real error. */
function _invVatStamped(v) {
  return String(v['VAT Rate'] == null ? '' : v['VAT Rate']).trim() !== '' ||
         String(v['VAT'] == null ? '' : v['VAT']).trim() !== '';
}

/** Every AR row that a VAT repair could act on, each already classified. Pure: reads, never writes. */
function _invVatCandidates(rate) {
  var invByNo = {}, invBySo = {};
  _rows('Invoices').forEach(function (v) {
    var n = String(v['INV No'] || ''); if (!n) return;
    invByNo[n] = v;
    if (String(v['Voided'] || '') !== 'true') {
      var so = String(v['SO No'] || '');
      if (so) (invBySo[so] = invBySo[so] || []).push(n);
    }
  });
  var arByInv = {};
  _rows('ARAging').forEach(function (r) {
    var n = String(r['INV No'] || ''); if (n) (arByInv[n] = arByInv[n] || []).push(r);
  });
  var rows = [], skipped = [];
  var skip = function (invNo, arNo, why) { skipped.push({ invNo: invNo, arNo: arNo, reason: why }); };

  _rows('ARAging').forEach(function (ar) {
    var invNo = String(ar['INV No'] || ''), arNo = String(ar['AR No'] || '');
    if (!invNo) return skip('', arNo, 'no invoice number on the receivable — a migrated or hand-made row');
    var v = invByNo[invNo];
    if (!v) return skip(invNo, arNo, 'no invoice by that number — an imported legacy receivable');
    if (String(v['Created By']) === 'Migrated (legacy)') return skip(invNo, arNo, 'migrated (legacy) invoice — its receivable is already gross');
    if (String(v['Voided'] || '') === 'true') return skip(invNo, arNo, 'the invoice is voided');
    if (_invVatStamped(v)) return skip(invNo, arNo, 'the invoice already records a VAT rate — nothing to impute');
    var net = _num(v['Total Sales']), dep = _num(v['Total Deposit']);
    var amt = _num(ar['Amount (PHP)']);
    if (Math.abs(amt - (net + dep)) > 0.005) {
      return skip(invNo, arNo, 'the receivable (' + amt.toFixed(2) + ') is not this invoice\'s net + deposit (' +
        (net + dep).toFixed(2) + ') — it was not written by createInvoice, so its basis is unknown');
    }
    if (net <= 0) return skip(invNo, arNo, 'nothing to charge VAT on');

    var vat = Math.round(net * (_num(rate) / 100) * 100) / 100;
    var collected = _num(ar['Collected (PHP)']);
    var cat = collected <= 0.005 ? 'unpaid'
            : (collected > amt + 0.005 ? 'overCollected' : 'partial');
    var why = [];
    if ((invBySo[String(v['SO No'] || '')] || []).length > 1) why.push('this sales order carries more than one live invoice');
    if ((arByInv[invNo] || []).length > 1) why.push('this invoice has more than one receivable');
    rows.push({
      invNo: invNo, arNo: arNo, soNo: String(v['SO No'] || ''), customer: String(v['Customer'] || ''),
      date: _dateStr(v['Date']), category: cat, ambiguous: why.length > 0, ambiguousWhy: why,
      totalSales: net, totalDeposit: dep, imputedRate: _num(rate), imputedVat: vat,
      amountNow: amt, amountAfter: Math.round((amt + vat) * 100) / 100,
      collected: collected, outstandingNow: Math.round((amt - collected) * 100) / 100,
      outstandingAfter: Math.round((amt + vat - collected) * 100) / 100,
...
/** previewInvoiceVatRepair({rate?}) — WRITES NOTHING. What a repair would do, row by row. */
function previewInvoiceVatRepair(p) {
  p = p || {};
  var rate = (p.rate === undefined || p.rate === null || String(p.rate).trim() === '') ? 12 : _num(p.rate);
  if (!(rate >= 0) || rate > 100) return { success: false, message: 'The rate must be between 0 and 100.' };
  var c = _invVatCandidates(rate);
  var by = { unpaid: 0, overCollected: 0, partial: 0 }, vatTotal = 0, before = 0, after = 0, amb = 0;
  c.rows.forEach(function (r) {
    by[r.category] = (by[r.category] || 0) + 1;
    vatTotal += r.imputedVat; before += r.amountNow; after += r.amountAfter;
    if (r.ambiguous) amb++;
  });
  return {
    success: true, imputed: true, rate: rate,
    rows: c.rows, skipped: c.skipped, counts: by, ambiguous: amb,
    vatTotal: Math.round(vatTotal * 100) / 100,
    amountBefore: Math.round(before * 100) / 100, amountAfter: Math.round(after * 100) / 100,
    message: 'These invoices carry NO recorded VAT. This repair IMPUTES ' + rate +
      '% — it does not recover a stored figure. Nothing has been written. ' + c.rows.length +
      ' receivable(s) would rise by ' + (Math.round(vatTotal * 100) / 100).toFixed(2) + ' in total; ' +
      c.skipped.length + ' row(s) are out of scope.'
  };
}

/** applyInvoiceVatRepair({invNos, rate?, confirmAmbiguous?, skipJournal?}) — corrects the named
 *  invoices only, and only ones the preview itself produced. */
function applyInvoiceVatRepair(p) {
  p = p || {};
  var rate = (p.rate === undefined || p.rate === null || String(p.rate).trim() === '') ? 12 : _num(p.rate);
  if (!(rate >= 0) || rate > 100) return { success: false, message: 'The rate must be between 0 and 100.' };
  /* Scoped exactly like backfillMissingAR (A249), for the same reason: "fix everything" on a money
     ledger is not a button anybody should have. */
  if (p.invNos === undefined || p.invNos === null || p.invNos === '') {
    return { success: false, message: 'Name the invoices to repair — this never fixes everything at once.' };
  }
  var want = {};
  try {
    var _l = JSON.parse(typeof p.invNos === 'string' ? p.invNos : JSON.stringify(p.invNos)) || [];
    _l.forEach(function (x) { var k = String(x).trim(); if (k) want[k] = 1; });
  } catch (e) { return { success: false, message: 'Could not read the list of invoices to repair.' }; }
  if (!Object.keys(want).length) return { success: false, message: 'No invoices were selected.' };

  var c = _invVatCandidates(rate);
  var byInv = {}; c.rows.forEach(function (r) { byInv[r.invNo] = r; });
  var skipWhy = {}; c.skipped.forEach(function (s) { if (s.invNo && !skipWhy[s.invNo]) skipWhy[s.invNo] = s.reason; });

  var repaired = [], refused = [], vatStamped = 0;
  Object.keys(want).forEach(function (invNo) {
    var r = byInv[invNo];
    if (!r) return refused.push({ invNo: invNo, reason: skipWhy[invNo] || 'not in the preview — nothing to repair' });
    if (r.ambiguous && !p.confirmAmbiguous) {
      return refused.push({ invNo: invNo, reason: r.ambiguousWhy.join('; ') + ' — confirm to repair it anyway' });
    }
    if (!p.skipJournal) {
      var jr = _rows('Journal').filter(function (j) {
        return String(j['Source']) === 'INV' && String(j['Source No']) === invNo;
      });
      if (!jr.length) return refused.push({ invNo: invNo, reason: 'no journal entry to correct' });
      var by = {};
      jr.forEach(function (j) {
        var k = String(j['Account Code']);
        by[k] = by[k] || { dr: 0, cr: 0 };
        by[k].dr += _num(j['Debit']); by[k].cr += _num(j['Credit']);
      });
      var arDr = (by[ACC.AR] || { dr: 0 }).dr;
      if (Math.abs(arDr - r.amountNow) > 0.005) {
        return refused.push({ invNo: invNo, reason: 'the journal debits ' + arDr.toFixed(2) +
          ' to receivables but the invoice is ' + r.amountNow.toFixed(2) + ' — corrected by hand, leave it alone' });
      }
      var g = function (code, side) { return (by[code] || { dr: 0, cr: 0 })[side]; };
      /* _postJournal REPLACES every line for (INV, invNo). Rebuilding the entry from the invoice
         alone would silently DELETE its COGS and inventory legs — and the goods/service split is not
         recoverable from InvoiceItems, which has no charge-kind column. So the existing entry IS the
         record of the split: carry every other leg through untouched and add only the VAT. The
         original balanced, so "add vat to both sides" balances too, by construction.
         Dated to the INVOICE's own date — _now() would move revenue between months. */
      _postJournal('INV', invNo, r.date || _now(), 'PHP', [
        { account: ACC.AR,         debit:  Math.round((arDr + r.imputedVat) * 100) / 100, memo: 'Invoice ' + invNo + ' — ' + r.customer },
        { account: ACC.SALES,      credit: g(ACC.SALES, 'cr'),    memo: 'Sales ' + invNo },
        { account: ACC.SERVICE,    credit: g(ACC.SERVICE, 'cr'),  memo: 'Service revenue ' + invNo },
        { account: ACC.OUTPUT_VAT, credit: r.imputedVat,          memo: 'Output VAT ' + rate + '% on ' + invNo + ' (A278 repair)' },
        { account: ACC.DEPOSITS,   credit: g(ACC.DEPOSITS, 'cr'), memo: 'Refundable deposit held — ' + invNo },
        { account: ACC.COGS,       debit:  g(ACC.COGS, 'dr'),     memo: 'COGS ' + invNo },
        { account: ACC.INV,        credit: g(ACC.INV, 'cr'),      memo: 'Inventory issued ' + invNo }
      ]);
    }
    ...
    // The one call that owns Collected / Status / the ARCOLL entry — which is what turns a stale
    // "Paid with a negative residue" row back into an honest Paid or Partial.
    var re = _arRecomputeFromCollections(r.arNo, null);
    repaired.push({ invNo: invNo, arNo: r.arNo, category: r.category, vat: r.imputedVat,
      amountBefore: r.amountNow, amountAfter: r.amountAfter, status: re.status });
  });
  return { success: true, rate: rate, repaired: repaired, refused: refused,
    vatStamped: Math.round(vatStamped * 100) / 100, refNo: repaired.map(function (x) { return x.invNo; }).join(', '),
    message: 'Repaired ' + repaired.length + ' invoice(s) at ' + rate + '% — receivables raised by ' +
      (Math.round(vatStamped * 100) / 100).toFixed(2) + ' in total.' +
      (refused.length ? ' Refused ' + refused.length + ': ' +
        refused.map(function (x) { return x.invNo + ' (' + x.reason + ')'; }).join('; ') + '.' : '') };
}
...
function _shipAutoDerive(soNo) {
  ...
  var pos = _rowsMemo('PurchaseOrders').filter(function (r) { return String(r['SO No']) === String(soNo); });
  if (pos.length) {
    d.po_created = true;
    ...
    var aps = _rowsMemo('APAging').filter(function (r) { return poNos[String(r['PO No'])]; });
    ...
    var prsForPo = _rowsMemo('PaymentRequests').filter(function (r) {
    ...
    var mrs = _rowsMemo('MaterialsReceiving').filter(function (r) { return poNos[String(r['PO No'])]; });
  ...
  var invs = _rowsMemo('Invoices').filter(function (r) { return String(r['SO No']) === String(soNo); });
  if (invs.length) d.invoiced = true;
  var ars = _rowsMemo('ARAging').filter(function (r) { return String(r['SO No']) === String(soNo); });
  if (ars.length) {
    d.ar_open = true;
...
function getShipments() {
  _ROWS_MEMO = {};                      // A270 — one read per sheet for the whole call, not per row
  try {
  ...
  } finally { _ROWS_MEMO = null; }      // never leave it open — writes must see their own appends
}
...
function getShipmentTimeline(p) {
  ...
  _ROWS_MEMO = {};                      // A270 — read-only, same 8 sheets; see _rowsMemo
  try {
  var s = _shipMap(r);
...
function _deleteMigratedInvoiceForSO(soNo) {
  ...
  /* A249 — NEVER TOUCH A VOIDED INVOICE. voidInvoice flags 'Voided' and deliberately leaves
     'Created By' alone (it is still the record of who raised it), so a voided migrated invoice
     matched this filter and was DELETED — taking the void reason, and the entire audit trail of the
     reversal, with it. Saving SO costs on such an order silently destroyed the evidence that a
     document had been withdrawn. A void is a decision somebody recorded; regeneration is not
     entitled to erase it. */
  _rows('Invoices').filter(function (v) { return String(v['SO No']) === String(soNo)
      && String(v['Created By']) === 'Migrated (legacy)'
      && String(v['Voided'] || '') !== 'true'; })
...
function _writeMigratedRecordsForSO(cd, force) {
  ...
  if (force || (!hasReal && !hasMigInv)) {
    /* A249 — REGENERATION KEEPS THE INVOICE NUMBER.
     *
     * This used to delete the migrated invoice and mint a brand-new number on every save, and
     * saveSOCostDetails calls it with force on EVERY save from the SO cost editor. Three consequences,
     * all silent:
     *   · any ARAging row pointing at the old number became a permanent orphan, and the next backfill
     *     then created a SECOND receivable for the same sale;
     *   · the invoice number a customer was given changed underneath them;
     *   · it burned a number from the monthly sequence on every edit.
     * On the live book 9 invoices dated AFTER the AR ledger began are tagged 'Migrated (legacy)' with
     * no receivable — they were not inherited from the migration, they were manufactured here.
     *
     * The row is now UPDATED IN PLACE. Not delete-then-re-append with the same number: if
     * PropertiesService throws, _nextNumber falls back to the sheet maximum alone, and a row absent
     * during that window could see its number reissued to somebody else.
     *
     * Deliberately NOT creating an AR row here. Four migration scripts call saveSOCostDetails in a
     * loop over the whole legacy book; doing it here would mass-create receivables across 85
     * pre-baseline invoices the architecture excludes on purpose. That belongs in backfillMissingAR,
     * scoped and secured, where it is an explicit act. */
    var existing = _rows('Invoices').filter(function (v) {
      return String(v['SO No']) === soNo && String(v['Created By']) === 'Migrated (legacy)'
        && String(v['Voided'] || '') !== 'true';
    })[0];
    if (force && existing) {
      var eNo = String(existing['INV No']);
      var iSh = _sheet('Invoices'), iCols = SCHEMA.Invoices;
      iSh.getRange(existing.rowIndex, iCols.indexOf('Date') + 1, 1, 1).setValues([[date]]);
      iSh.getRange(existing.rowIndex, iCols.indexOf('Customer') + 1, 1, 1).setValues([[customer]]);
      iSh.getRange(existing.rowIndex, iCols.indexOf('Total Sales') + 1, 1, 1).setValues([[sales]]);
      iSh.getRange(existing.rowIndex, iCols.indexOf('Total COGS') + 1, 1, 1).setValues([[cogs]]);
      // The single migrated line mirrors the header, so it is rewritten rather than accumulated.
      var itSh = _sheet('InvoiceItems');
      _rows('InvoiceItems').filter(function (r) { return String(r['INV No']) === eNo; })
        .sort(function (a, b) { return b.rowIndex - a.rowIndex; })
        .forEach(function (r) { itSh.deleteRow(r.rowIndex); });
      itSh.appendRow([eNo, '(migrated)', 'Migrated legacy sales', 1, sales, sales, cogs, cogs, '', '', '', '']);  // A282 width
      wroteInv = true;
    } else {
...
function backfillMigratedRecords(p) {
  ...
  cds.forEach(function (c) {
    ...
    if (!invBySo[soNo]) {
      ...
      invItemSh.appendRow([invNo, '(migrated)', 'Migrated legacy sales', 1, sales, sales, cogs, cogs, '', '', '', '']);  // A282 width
      invBySo[soNo] = true;
      invoicesCreated++;
    }
...
function saveSOCostDetails(p) {
  ...
  try {
    _writeMigratedRecordsForSO({
      ...
      /* A249 — 'COGS Type' was NEVER PASSED, so the writer read undefined, defaulted to 'local' and
         computed intl=false on every single save. The A220 guard below it — written precisely to keep
         this in step with saveSOCostDetails — was dead code on this path. For an International order
         the regenerated receiving silently dropped duties, local charges, both bank charges and
         shipping, while saveSOCostDetails had already folded them into Total COGS, so the receiving
         and the cost record could never reconcile. 46 of 107 live sales orders are International. */
      'COGS Type': cogsType,
...
function _soForDoc(module, refNo) {
  ...
  if (m === 'Pricing Request') {
    /* A242 — this used to be _memoFind, i.e. the FIRST quotation carrying the PR No, while
       _quoPickForPR (A227) answers the same question with the LAST LIVE one. Two resolvers, two
       answers, and the disagreement decided which sales order's folder a request's PDF was filed
       under. It only ever mattered once a request had more than one quotation, which no live request
       did — partial quoting makes that ordinary, so they are reconciled on the A227 rule now. */
    var q = _quoPickForPR(_rows('Quotations').filter(function (r) {
      return String(r['PR No'] || '') === ref;
    }));
    if (!q) return '';
...
/* A267 — the quotation chain is now admin → DIRECTOR → management, with management last. Those two
   tiers have to be told apart, which _isMgmtTier deliberately does not do (it stays as-is because
   POs, itineraries and pricing all still treat the pair as one). */
function _isDirectorRole(role) { return role === 'director'; }
function _isMgmtOnly(role) { return role === 'management'; }
...
function submitQuotationApproval(p) {
  ...
  // A271 — admin-created quotations go straight to management; sales-created go to admin first.
...
function approveQuotation(p) {
  ...
  /* A271 — the quotation chain is admin → management again; the director no longer approves
     quotations at all. 'Pending Director' is still ACCEPTED here on purpose: two quotations were
     sitting at that status when this shipped, and dropping the branch outright would have stranded
     them at a stage no code path could clear. Management signs them off like any other. */
  if (st === 'Pending Management' || st === 'Pending Director') {
    if (!_isMgmtOnly(role)) return { success: false, message: 'Management gives the final approval at this stage.' };
...
function rejectQuotation(p) {
  ...
  var canReject = (st === 'Pending Admin' && _isAdminTier(role))
    // A271 — 'Pending Director' is legacy for quotations; management clears those too.
    || ((st === 'Pending Management' || st === 'Pending Director') && _isMgmtOnly(role));
...
// ═══════════════════════════════════════════════════════════════════════════════════════════════
//  A277 — LEAD GENERATION  (six sheets · one store · one counter · one follow-up reader)
// ═══════════════════════════════════════════════════════════════════════════════════════════════
/* The lead-generation role is measured in eight numbers a day — plants researched, contacts
   verified, intro emails, follow-up emails, cold calls, follow-up calls, leads handed off, meetings
   booked — and everything here exists so those numbers are TRUE. Four rules follow from that:

   1. EVERY COUNTED THING IS A SERVER-STAMPED EVENT DATE, IN MANILA, AS A STRING. `Researched On`,
      `Verified On`, `Sent On`, `Handed Off On`, `Booked On`, `Replied On` are written by _lgDay()
      at the moment the thing happens — never typed, never a Date object, never derived in the
      browser. Counting is string equality against that column. The activity log is a timeline
      and is never counted: it records every edit and every retry as a row.

   2. STAMPS FIRE ON THE TRANSITION, ONCE. A contact re-saved as Pattern a week later is not
      verified twice; a lead returned and re-handed stamps `Rehanded On` and is shown as re-handed,
      not counted again. A stored stamp is never overwritten.

   3. DELETES ARE SOFT. `Deleted On` is stamped and the row is excluded from every read and count.
      The marketing store hard-deletes by a rowIndex the browser cached sixty seconds ago, which
      deletes the neighbour when anything above it has moved. Not repeated here.

   4. THE SERVER REFUSES WHAT WOULD INFLATE A COUNT. Closed lists are enforced here, not only in
      the dropdown; `Created By` is the session actor whatever the payload says; an email batch's
      `Count` is derived from the contacts it lists and a client-sent count is discarded (the A229
      rule); a typed date may not be in the future nor more than a day back. The four writes are
      _SECURED, so the actor is stamped by Flask — these rows are one person's performance record.

   The follow-up cadence (day 3 / 7 / 14 after the intro, then cold for three months) is DERIVED on
   every read from `Intro Sent` and LgTouches. Nothing stores "due" or "cold": a flag written on
   Monday is wrong by Tuesday and nothing ever comes to correct it. */

var LEADGEN = {
  plants:   { sheet: 'LgPlants',     prefix: 'PLT', event: 'Researched On' },
  contacts: { sheet: 'LgContacts',   prefix: 'CTC', event: 'Verified On' },
  batches:  { sheet: 'LgEmailBatch', prefix: 'BCH', event: 'Sent On' },
  leads:    { sheet: 'LgLeads',      prefix: 'QLD', event: 'Handed Off On' },
  accred:   { sheet: 'LgAccred',     prefix: 'ACR', event: 'Started On' },
  suppliers:{ sheet: 'LgSuppliers',  prefix: 'SUP', event: 'Researched On' },   // A279
  touches:  { sheet: 'LgTouches',    prefix: 'TCH', internal: true }   // written by the fan-outs and the LinkedIn logger
};

/* The closed lists. A dropdown in the browser is a convenience; this is the rule. */
var _LG_ENUM = {
  ...
  // A279
  reached:       ['Decision-maker', 'Gatekeeper', 'Voicemail / no answer'],
  linkedinKind:  ['Connection request', 'Message'],
  supplierStatus:['Researching', 'Qualified', 'Handed Off', 'Rejected']
};
var _LG_RULES = {
  plants:   { required: ['company', 'sector', 'territory'],
              enums: { sector: 'sector', territory: 'territory', status: 'plantStatus' },
              defaults: { status: 'Active' } },
  contacts: { required: ['plantNo', 'name'],
              enums: { role: 'contactRole', emailVerified: 'emailVerified', status: 'contactStatus' },
              defaults: { role: 'Other', emailVerified: 'Unverified', status: 'New' } },
  batches:  { required: ['kind'], enums: { kind: 'batchKind', sector: 'sector' }, defaults: { sector: '' } },
  leads:    { required: ['plantNo'], enums: { territory: 'territory', status: 'leadStatus' },
              defaults: { territory: '', status: 'Handed Off' } },
  accred:   { required: ['plantNo'], enums: { status: 'accredStatus' }, defaults: { status: 'Submitted' } },
  suppliers:{ required: ['company'], enums: { status: 'supplierStatus' }, defaults: { status: 'Researching' } }
};
/* Where a lead may go from where it is. 'Returned' is the rep's move; 'Returned' → 'Handed Off' is a
   re-hand, stamped separately so it is never counted as a second lead. */
var _LG_TRANSITIONS = {
  'Handed Off':          ['Presentation Booked', 'Quoted', 'Won', 'Lost', 'Returned'],
  'Presentation Booked': ['Quoted', 'Won', 'Lost', 'Returned'],
  'Quoted':              ['Won', 'Lost', 'Returned'],
  'Won': [], 'Lost': [],
  'Returned':            ['Handed Off']
};
var _LG_FLAGS = { 'Right Person': 1, 'Own Maintenance': 1, 'Flanged Or Hydraulic': 1, 'Said Yes': 1, 'PhilGEPS': 1, 'Docs Sent': 1 };
var _LG_QUAL = [['rightPerson', 'Right Person', 'the right person'], ['ownMaintenance', 'Own Maintenance', 'runs its own maintenance'],
                ['flangedOrHydraulic', 'Flanged Or Hydraulic', 'has flanged or hydraulic work'], ['saidYes', 'Said Yes', 'said yes to a presentation or asked for a quote']];
var _LG_WRITE_ROLES = ['leadgen', 'director', 'management'];
/* A279 — the nine daily items of the job description, in its order. `attempts` is calls + emails +
   LinkedIn touches combined; `conversations` is calls that reached the decision-maker; `crm` is
   "was anything logged" (records touched today); `eod` is the day's report submitted; `scheduled` is
   follow-up calls booked plus presentations booked. */
var _LG_COUNT_KEYS = ['attempts', 'conversations', 'emails', 'linkedin', 'suppliers', 'accounts', 'crm', 'eod', 'scheduled'];
var _LG_QUOTA_KEY = { attempts: 'lgQuotaAttempts', conversations: 'lgQuotaConversations', emails: 'lgQuotaEmails',
  linkedin: 'lgQuotaLinkedin', suppliers: 'lgQuotaSuppliers', accounts: 'lgQuotaAccounts', crm: 'lgQuotaCrm',
  eod: 'lgQuotaEod', scheduled: 'lgQuotaScheduled' };
var _LG_WEEK_KEY = { activeAccounts: 'lgWeekActiveAccounts', leads: 'lgWeekLeads', suppliersHandedOff: 'lgWeekSuppliers' };
var _LG_DOW = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
var _LG_STEPS = [3, 7, 14];          // working days after the intro
var _LG_REVISIT_MONTHS = 3;

// ── Dates: Manila, as strings, arithmetic on parts ────────────────────────────────────────────
/* Formatted in 'Asia/Manila' EXPLICITLY, the way the commission code does — the script timezone is
   not trusted, because A276 found _addTermDays a day early west of UTC when it was. */
function _lgDay(d)  { return Utilities.formatDate(d || _now(), 'Asia/Manila', 'yyyy-MM-dd'); }
function _lgHHmm(d) { return Utilities.formatDate(d || _now(), 'Asia/Manila', 'HH:mm'); }
/** Any cell — a Date, an ISO string, a typed 'yyyy-MM-dd', a blank — to 'yyyy-MM-dd' or ''. */
function _lgDayOf(v) {
  if (v === null || v === undefined || v === '') return '';
  if (v && typeof v.getTime === 'function') return isNaN(v.getTime()) ? '' : _lgDay(v);
  var s = String(v).trim();
  if (/^\d{4}-\d{2}-\d{2}$/.test(s)) return s;
  if (/^\d{4}-\d{2}-\d{2}T/.test(s)) { var d = new Date(s); return isNaN(d.getTime()) ? '' : _lgDay(d); }
  return /^\d{4}-\d{2}-\d{2}/.test(s) ? s.substring(0, 10) : '';
}
function _lgParts(ymd) {
  var m = /^(\d{4})-(\d{2})-(\d{2})$/.exec(String(ymd || ''));
  return m ? [parseInt(m[1], 10), parseInt(m[2], 10), parseInt(m[3], 10)] : null;
}
function _lgFmtUTC(d) {
  return d.getUTCFullYear() + '-' + ('0' + (d.getUTCMonth() + 1)).slice(-2) + '-' + ('0' + d.getUTCDate()).slice(-2);
}
/* Parts-based, like _hireAddDays: a UTC-midnight construction with UTC read-back cannot cross a day
   boundary whatever the host timezone is. */
function _lgAddDays(ymd, n) { var p = _lgParts(ymd); return p ? _lgFmtUTC(new Date(Date.UTC(p[0], p[1] - 1, p[2] + n))) : ''; }
function _lgAddMonths(ymd, n) {
  var p = _lgParts(ymd); if (!p) return '';
  var last = new Date(Date.UTC(p[0], p[1] - 1 + n + 1, 0)).getUTCDate();      // days in the target month
  return _lgFmtUTC(new Date(Date.UTC(p[0], p[1] - 1 + n, Math.min(p[2], last))));
}
function _lgDow(ymd) { var p = _lgParts(ymd); return p ? _LG_DOW[new Date(Date.UTC(p[0], p[1] - 1, p[2])).getUTCDay()] : ''; }
function _lgDaysBetween(a, b) {
  var pa = _lgParts(a), pb = _lgParts(b); if (!pa || !pb) return 0;
  return Math.round((Date.UTC(pb[0], pb[1] - 1, pb[2]) - Date.UTC(pa[0], pa[1] - 1, pa[2])) / 86400000);
}
function _lgMonday(ymd) { var i = _LG_DOW.indexOf(_lgDow(ymd)); return i < 0 ? '' : _lgAddDays(ymd, -((i + 6) % 7)); }
/** ISO-8601 week number — the week containing the Thursday. */
function _lgIsoWeek(ymd) {
  var p = _lgParts(ymd); if (!p) return 0;
...
// ── Settings: the working week, the quotas, the reps ──────────────────────────────────────────
function _lgSettings() { return getFlowSettings().data || {}; }
function _lgList(v) {
  if (Array.isArray(v)) return v;
  if (v && typeof v.getTime === 'function') return [v];       // a lone holiday Sheets turned into a Date
  return String(v === null || v === undefined ? '' : v).split(',').map(function (s) { return s.trim(); }).filter(Boolean);
}
function _lgWeekCfg(settings) {
  var days = _lgList(settings.lgWorkingDays || 'Mon,Tue,Wed,Thu,Fri').filter(function (d) { return _LG_DOW.indexOf(d) !== -1; });
  if (!days.length) days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'];
  var hol = {};
  _lgList(settings.lgHolidays).forEach(function (d) { var y = _lgDayOf(d); if (y) hol[y] = 1; });
  return { days: days, holidays: hol };
}
function _lgIsWorking(ymd, cfg) { return cfg.days.indexOf(_lgDow(ymd)) !== -1 && !cfg.holidays[ymd]; }
/** The next working day on or after `ymd`. An intro on Thursday is not overdue on Monday because
 *  day 3 fell on a Sunday. */
function _lgRollForward(ymd, cfg) {
  var d = ymd, i = 0;
  while (d && !_lgIsWorking(d, cfg) && i < 21) { d = _lgAddDays(d, 1); i++; }
  return d;
}
function _lgSectorOfWeek(monday, settings) {
  var cycle = _lgList(settings.lgSectorCycle || 'Cement,Mining,Power,Water');
  if (!cycle.length) return { name: '', index: 0, of: 0 };
  var i = _lgIsoWeek(monday) % cycle.length;
  return { name: cycle[i], index: i + 1, of: cycle.length };
}

// ── Reading ───────────────────────────────────────────────────────────────────────────────────
function _lgRows(entity) {
  return _rows(LEADGEN[entity].sheet).filter(function (r) { return !r['Deleted On']; });
}
function _lgIsDateHeader(h) {
  return / On$| Date$/.test(h) || ['Date', 'Intro Sent', 'Submitted', 'Approved', 'Expiry'].indexOf(h) !== -1;
}
function _lgDto(entity, r) {
  var fields = SCHEMA[LEADGEN[entity].sheet], o = { rowIndex: r.rowIndex };
  fields.forEach(function (h) {
    var v = r[h];
    if (h === 'Created At' || h === 'Updated At') v = v || '';
    else if (_lgIsDateHeader(h)) v = _lgDayOf(v);
    else if (_LG_FLAGS[h]) v = _lgYes(v);
    else v = (v === undefined || v === null) ? '' : v;
    o[_camel(h)] = v;
  });
  return o;
}
function _lgYes(v) { return v === true || /^(yes|true|1|y)$/i.test(String(v || '').trim()); }

/** getLeadgen({entity?, plantNo?, handedTo?, status?}) — every live row, or one entity, filtered
 *  server-side so the rep's "leads for you" is not a five-sheet read on every dashboard load. */
function getLeadgen(p) {
  ...
  Object.keys(LEADGEN).forEach(function (k) {
    ...
    if (only && k !== only) return;
    var rows = _lgRows(k).map(function (r) { return _lgDto(k, r); });
...
/* Contacts, leads and accreditations carry their plant's company/site/sector, and a lead its
   contact's name and details, so the rep's "leads for you" panel, the lead sheet and the daily
   report's evidence are ONE read rather than three. The maps are built lazily, once per call. */
function _lgMaps() {
  var pm = null, cm = null;
  return {
    plant: function (no) { if (!pm) { pm = {}; _lgRows('plants').forEach(function (r) { pm[String(r['Plant No'])] = r; }); } return pm[String(no)] || null; },
    contact: function (no) { if (!cm) { cm = {}; _lgRows('contacts').forEach(function (r) { cm[String(r['Contact No'])] = r; }); } return cm[String(no)] || null; }
  };
}
function _lgEnrich(k, rows, maps) {
  if (k === 'contacts' || k === 'leads' || k === 'accred') {
    rows.forEach(function (r) {
      var pl = maps.plant(r.plantNo);
      if (!pl) return;
  ...
  if (k === 'leads') {
    rows.forEach(function (r) {
      var ct = r.contactNo ? maps.contact(r.contactNo) : null;
...
/** getLeadgenDay({date?, user?}) — THE EVIDENCE behind one day's eight numbers, for the daily
 *  report. Not counts: the plants themselves, the contacts verified, each email batch with the
 *  contacts it went to (the page checks each against the mailbox), each call with who and what,
 *  the leads handed off and the presentations booked. Same stamps getLeadgenCounts counts on, so
 *  the list under a number is exactly the rows that made it. */
function getLeadgenDay(p) {
  p = p || {};
  var today = _lgDay(), date = _lgDayOf(p.date) || today;
  var user = String(p.user || '').trim();
  var mine = function (h) { return function (r) { return !user || String(r[h] || '') === user; }; };
  var on = function (rows, h) { return rows.filter(function (r) { return _lgDayOf(r[h]) === date; }); };
  var maps = _lgMaps();
  var dto = function (k) { return function (r) { return _lgDto(k, r); }; };
  var plants = on(_lgRows('plants').filter(mine('Created By')), 'Researched On').map(dto('plants'));
  var contacts = _lgEnrich('contacts', on(_lgRows('contacts').filter(mine('Created By')), 'Verified On').map(dto('contacts')), maps);
  var batches = on(_lgRows('batches').filter(mine('Created By')), 'Sent On').map(function (b) {
    var d = _lgDto('batches', b);
    d.contacts = _lgNos(b['Contact Nos']).map(function (no) {
      var c = maps.contact(no), pl = c ? maps.plant(c['Plant No']) : null;
      return { contactNo: no, name: c ? String(c['Name'] || '') : '(removed)', role: c ? String(c['Role'] || '') : '',
               email: c ? String(c['Email'] || '') : '', company: pl ? String(pl['Company'] || '') : '', plantSite: pl ? String(pl['Plant / Site'] || '') : '' };
    });
    return d;
  });
  var calls = _rows('SalesCalls').filter(function (r) { return r['Kind']; }).filter(mine('User'))
    .filter(function (r) { return _lgDayOf(r['Date']) === date; }).map(function (r) {
      var c = r['Contact No'] ? maps.contact(r['Contact No']) : null, pl = c ? maps.plant(c['Plant No']) : null;
      return { callNo: r['Call No'], kind: r['Kind'], contact: r['Contact'], company: r['Company'] || (pl ? pl['Company'] : ''),
               plantSite: pl ? String(pl['Plant / Site'] || '') : '', role: c ? String(c['Role'] || '') : '',
               outcome: r['Outcome'], reached: r['Reached'] || '', notes: r['Notes'], createdAt: r['Created At'] || '', contactNo: r['Contact No'] || '' };
    });
  var leadRows = _lgRows('leads').filter(mine('Created By'));
  var byKind = function (k) { return function (b) { return b.kind === k; }; };
  // A279 — the rest of the nine
  var suppliers = on(_lgRows('suppliers').filter(mine('Created By')), 'Researched On').map(dto('suppliers'));
  var withContact = function (t) {
    var c = maps.contact(t['Contact No']), pl = c ? maps.plant(c['Plant No']) : null;
    return { touchNo: t['Touch No'], contactNo: t['Contact No'], kind: t['Kind'], sentOn: _lgDayOf(t['Sent On']),
             name: c ? String(c['Name'] || '') : '(removed)', role: c ? String(c['Role'] || '') : '',
             company: pl ? String(pl['Company'] || '') : '', plantSite: pl ? String(pl['Plant / Site'] || '') : '',
             linkedin: c ? String(c['LinkedIn'] || '') : '' };
  };
  var dead = {};
  _rows('LgEmailBatch').forEach(function (b) { if (b['Deleted On']) dead[String(b['Batch No'])] = 1; });
  var touches = on(_rows('LgTouches').filter(mine('Created By')).filter(function (t) { return !dead[String(t['Ref No'])]; }), 'Sent On');
  var linkedin = touches.filter(function (t) { return t['Channel'] === 'LinkedIn'; }).map(withContact);
  var scheduledCalls = _lgEnrich('contacts', on(_lgRows('contacts').filter(mine('Created By')), 'Scheduled On').map(dto('contacts')), maps);
  var report = _rows('DailyReports').filter(function (r) {
    return String(r['Role'] || '').toLowerCase() === 'leadgen' && (!user || String(r['User'] || '').trim() === user) && _lgDayOf(r['Date']) === date;
  })[0] || null;
  var allPlants = _lgRows('plants').filter(mine('Created By'));
  var crmTouched = on(allPlants, 'Updated At').length + on(_lgRows('contacts').filter(mine('Created By')), 'Updated At').length +
                   on(_lgRows('suppliers').filter(mine('Created By')), 'Updated At').length + on(leadRows, 'Updated At').length +
                   calls.length + touches.length;
  return {
    success: true, today: today, date: date,
    accounts: plants, plants: plants, contacts: contacts, suppliers: suppliers,
    introBatches: batches.filter(byKind('Intro')), followupBatches: batches.filter(byKind('Follow-up')),
    calls: calls,
    conversations: calls.filter(function (c) { return String(c.reached) === 'Decision-maker'; }),
    coldCalls: calls.filter(function (c) { return c.kind === 'Cold'; }), followupCalls: calls.filter(function (c) { return c.kind === 'Follow-up'; }),
    linkedin: linkedin,
    scheduledCalls: scheduledCalls,
    intelUpdates: on(allPlants, 'Intel Updated On').map(dto('plants')),
    suppliersHandedOff: on(_lgRows('suppliers').filter(mine('Created By')), 'Handed Off On').map(dto('suppliers')),
    crm: crmTouched,
    eod: report ? { reportNo: report['Report No'], status: report['Status'], submittedAt: report['Submitted At'] || '',
                    submitCount: _num(report['Submit Count']), reviewedBy: report['Reviewed By'] || '' } : null,
    leads: _lgEnrich('leads', on(leadRows, 'Handed Off On').map(dto('leads')), maps),
    meetings: _lgEnrich('leads', on(leadRows, 'Booked On').map(dto('leads')), maps)
  };
}

// ── Writing ───────────────────────────────────────────────────────────────────────────────────
function _lgRole(p) { return String((p && p.actorRole) || '').toLowerCase(); }
function _lgNo(msg) { return { success: false, message: msg }; }
/** A typed event date: today when blank; never the future; at most a day back unless the director. */
function _lgTypedDate(v, today, isDirector) {
  if (v === undefined || v === null || String(v).trim() === '') return { day: today };
  var d = _lgDayOf(v);
  if (!d) return { error: '"' + v + '" is not a date.' };
  if (d > today) return { error: 'The date cannot be in the future (' + d + ').' };
  if (!isDirector && _lgDaysBetween(d, today) > 1) return { error: 'Only the director can log something more than a day back (' + d + ').' };
  return { day: d };
}
function _lgNos(v) {
...
function _lgHeaderFor(fields, camel) {
  for (var i = 0; i < fields.length; i++) if (_camel(fields[i]) === camel) return fields[i];
  return '';
}
/** Required fields present, every enum a member of its list, defaults filled. Returns the first
 *  problem as a sentence, or ''. */
function _lgValidate(entity, rec, ex) {
  var rule = _LG_RULES[entity], fields = SCHEMA[LEADGEN[entity].sheet];
  var i, k, h, v;
  for (i = 0; i < rule.required.length; i++) {
    k = rule.required[i]; h = _lgHeaderFor(fields, k);
    v = (rec[k] !== undefined && rec[k] !== null) ? rec[k] : (ex ? ex[h] : '');
    if (String(v === undefined || v === null ? '' : v).trim() === '') return h + ' is required.';
  }
  var keys = Object.keys(rule.enums);
  for (i = 0; i < keys.length; i++) {
    k = keys[i]; h = _lgHeaderFor(fields, k);
    var list = _LG_ENUM[rule.enums[k]];
    v = (rec[k] !== undefined && rec[k] !== null) ? String(rec[k]).trim() : undefined;
    if (v === undefined) { if (ex && ex[h]) continue; v = ''; }
    if (v === '') {
      if (rule.defaults.hasOwnProperty(k)) { rec[k] = rule.defaults[k]; continue; }
      return h + ' is required.';
    }
    if (list.indexOf(v) === -1) return h + ' must be one of: ' + list.join(' · ') + '.';
    rec[k] = v;
  }
  return '';
}
function _lgMsg(entity, rec, extra) {
  var m = { plants: ['Plant', (rec.company || '') + (rec.plantSite ? ' — ' + rec.plantSite : '')],
            contacts: ['Contact', rec.name || ''], batches: ['Email batch', (rec.kind || '') + (extra ? ' × ' + extra : '')],
            leads: ['Lead', extra || ''], accred: ['Accreditation', rec.company || ''] }[entity];
  return m[0] + (m[1] ? ' · ' + m[1] : '');
}

/* ── The hand-off, as a transaction ──────────────────────────────────────────────────────────
   saveClient rewrites all twelve columns from `p.x || ''` and matches by lowercased name, so calling
   it from here would blank a paying customer's payment terms — which createInvoice reads to set the
   AR due date. This fills only what is blank, resolves the company through the alias registry so
   two spellings are one client, and sets Stage = Prospect only on a row it CREATES. A customer is
   never downgraded to a prospect. Idempotent, so the clientRef dedupe above it makes a retry safe. */
function _upsertClientFields(name, patch, actor) {
  var raw = String(name || '').trim();
  if (!raw) return { created: false, customer: '' };
  var key = _canonKey(raw), rk = _rawKey(raw);
  var fields = SCHEMA.Clients, sh = _sheet('Clients');
  var ex = _rows('Clients').filter(function (r) {
    var n = r['Customer']; return _rawKey(n) === rk || (key && _canonKey(n) === key);
  })[0];
  var map = { address: 'Address', contactPerson: 'Contact Person', designation: 'Designation', email: 'Email',
              phone: 'Phone', rfqRef: 'RFQ Ref', paymentTerms: 'Payment Terms', notes: 'Notes' };
  var vals = fields.map(function (h) { return ex && ex[h] !== undefined ? ex[h] : ''; });
  var changed = false;
  Object.keys(map).forEach(function (k) {
    var i = fields.indexOf(map[k]);
    if (i < 0 || !patch[k]) return;
    if (String(vals[i] === null || vals[i] === undefined ? '' : vals[i]).trim() === '') { vals[i] = patch[k]; changed = true; }
  });
  if (ex) {
    if (changed) {
      vals[fields.indexOf('Updated By')] = actor || '';
      vals[fields.indexOf('Updated At')] = _now();
      sh.getRange(ex.rowIndex, 1, 1, fields.length).setValues([vals]);
    }
    return { created: false, customer: String(ex['Customer']), changed: changed };
  }
  var c = _canonClient(raw);
  var display = (c && c.key && c.display && c.display !== _FLOW_UNKNOWN_CLIENT) ? c.display : raw;
  vals[0] = display;
  vals[fields.indexOf('Stage')] = 'Prospect';
  vals[fields.indexOf('Updated By')] = actor || '';
  vals[fields.indexOf('Updated At')] = _now();
  _append('Clients', vals);
  return { created: true, customer: display, changed: true };
}

/* A279 — a LinkedIn connection request or message is the third logged channel. One touch row per
   act, so it counts toward attempts and advances the follow-up cadence exactly as an email does. */
function _lgLogLinkedin(p) {
  var role = _lgRole(p);
  if (_LG_WRITE_ROLES.indexOf(role) === -1) return _lgNo('Only the lead-generation user can record this.');
  var rec = {};
  try { rec = JSON.parse(p.record || '{}'); } catch (e) { return _lgNo('Invalid record JSON.'); }
  var dup = _refSeen('lg_linkedin', p.clientRef);
  if (dup) return { success: true, entity: 'linkedin', id: dup, refNo: dup, replayed: true, message: 'LinkedIn · already logged' };
  var no = String(rec.contactNo || '').trim();
  var c = no ? _lgRows('contacts').filter(function (x) { return String(x['Contact No']) === no; })[0] : null;
  if (!c) return _lgNo('Pick the contact this went to.');
  var kind = String(rec.kind || '').trim();
  if (_LG_ENUM.linkedinKind.indexOf(kind) === -1) return _lgNo('Kind must be one of: ' + _LG_ENUM.linkedinKind.join(' · ') + '.');
  var td = _lgTypedDate(rec.date, _lgDay(), role === 'director');
  if (td.error) return _lgNo(td.error);
  var id = _nextNumber('LgTouches', 1, 'LI');
  _append('LgTouches', [id, no, 'LinkedIn', kind, id, td.day, String(p.actorName || ''), _now()]);
  _refStore('lg_linkedin', p.clientRef, id);
  return { success: true, entity: 'linkedin', id: id, refNo: id, contactNo: no, today: _lgDay(),
    message: 'LinkedIn · ' + kind + ' — ' + String(c['Name'] || '') };
}

/* A279 — the supplier hand-off, the mirror of _upsertClientFields: a qualified local supplier goes
   into the Suppliers master procurement already works from, filling only blank cells so a supplier
   accounting has banked details for is never overwritten by research notes. */
function _upsertSupplierFields(name, patch, actor) {
  var raw = String(name || '').trim();
  if (!raw) return { created: false, supplier: '' };
  var rk = _rawKey(raw), fields = SCHEMA.Suppliers, sh = _sheet('Suppliers');
  var ex = _rows('Suppliers').filter(function (r) { return _rawKey(r['Supplier']) === rk; })[0];
  var vals = fields.map(function (h) { return ex && ex[h] !== undefined ? ex[h] : ''; });
  var map = { address: 'Address', notes: 'Notes' }, changed = false;
  Object.keys(map).forEach(function (k) {
    var i = fields.indexOf(map[k]);
    if (i < 0 || !patch[k]) return;
    if (String(vals[i] === null || vals[i] === undefined ? '' : vals[i]).trim() === '') { vals[i] = patch[k]; changed = true; }
  ...
  if (ex) {
    if (changed) {
      vals[fields.indexOf('Updated By')] = actor || ''; vals[fields.indexOf('Updated At')] = _now();
      sh.getRange(ex.rowIndex, 1, 1, fields.length).setValues([vals]);
    }
    return { created: false, supplier: String(ex['Supplier']), changed: changed };
  }
  vals[0] = raw; vals[fields.indexOf('Updated By')] = actor || ''; vals[fields.indexOf('Updated At')] = _now();
  _append('Suppliers', vals);
  return { created: true, supplier: raw, changed: true };
}

/** saveLeadgenRecord({entity, record: JSON, clientRef}) — create or update one row, with the
 *  server owning every stamp, every count and every closed list. */
function saveLeadgenRecord(p) {
  var entity = String(p.entity || ''), cfg = LEADGEN[entity];
  if (entity === 'linkedin') return _lgLogLinkedin(p);                  // A279 — a touch, not a row
  if (!cfg || cfg.internal) return _lgNo('Unknown lead-gen entity: ' + entity);
  var rec = {};
  try { rec = JSON.parse(p.record || '{}'); } catch (e) { return _lgNo('Invalid record JSON.'); }
  if (!rec || typeof rec !== 'object') return _lgNo('Invalid record.');

  var role = _lgRole(p), isDirector = role === 'director';
  var salesReturn = role === 'sales' && entity === 'leads';
  if (_LG_WRITE_ROLES.indexOf(role) === -1 && !salesReturn) {
    return _lgNo('Only the lead-generation user can record this' + (role ? ' — you are signed in as ' + role : '') + '.');
  }
  var fields = SCHEMA[cfg.sheet], sh = _sheet(cfg.sheet), idHeader = fields[0], idKey = _camel(idHeader);
  var today = _lgDay(), now = _now(), actor = String(p.actorName || '');
  var rows = _rows(cfg.sheet), ex = null;

  if (rec.rowIndex) {
    var ri = parseInt(rec.rowIndex, 10);
    ex = rows.filter(function (r) { return r.rowIndex === ri; })[0];
    if (!ex) return _lgNo('Record not found — reload and try again.');
    if (rec[idKey] && String(ex[idHeader]) !== String(rec[idKey])) return _lgNo('The list has moved since it was loaded — reload and try again.');
    if (ex['Deleted On']) return _lgNo(ex[idHeader] + ' was deleted.');
  } else {
    var dup = _refSeen('lg_' + entity, p.clientRef);
    if (dup) return { success: true, entity: entity, id: dup, refNo: dup, replayed: true, message: _lgMsg(entity, rec) + ' (already saved)' };
  }

  /* The rep's one move: give a lead back, with a reason. Nothing else on the row is theirs. */
  if (salesReturn) {
    if (!ex) return _lgNo('A rep cannot create a lead.');
    if (String(ex['Handed To'] || '') !== String(p.actorUsername || '')) return _lgNo('This lead was not handed to you.');
    if (String(rec.status || '') !== 'Returned') return _lgNo('A rep can only return a lead.');
    if ((_LG_TRANSITIONS[String(ex['Status'])] || []).indexOf('Returned') === -1) return _lgNo(ex['Status'] + ' leads cannot be returned.');
    var why = String(rec.returnReason || '').trim();
    if (!why) return _lgNo('Say why the lead is coming back.');
    var col = function (h) { return fields.indexOf(h) + 1; };
    sh.getRange(ex.rowIndex, col('Status'), 1, 1).setValues([['Returned']]);
    sh.getRange(ex.rowIndex, col('Returned On'), 1, 1).setValues([[today]]);
    sh.getRange(ex.rowIndex, col('Return Reason'), 1, 1).setValues([[why]]);
    sh.getRange(ex.rowIndex, col('Updated At'), 1, 1).setValues([[now]]);
    return { success: true, entity: entity, id: ex[idHeader], refNo: ex[idHeader], rowIndex: ex.rowIndex,
             message: 'Lead · returned by ' + actor + ' — ' + why };
  }

  var bad = _lgValidate(entity, rec, ex);
  if (bad) return _lgNo(bad);

  var stamp = {};                                 // header → value the server decides
  var settings = null, plant = null, fanout = null, handoff = null, supHandoff = null;
  var plantOf = function (no) {
    return _lgRows('plants').filter(function (r) { return String(r['Plant No']) === String(no); })[0] || null;
  };

  if (entity === 'plants') {
    var company = String(rec.company !== undefined ? rec.company : ex['Company']).trim();
    var site = String(rec.plantSite !== undefined ? rec.plantSite : (ex ? ex['Plant / Site'] : '')).trim();
    var key = _rawKey(company) + '|' + _rawKey(site);
    var clash = rows.filter(function (r) {
      return !r['Deleted On'] && (!ex || r.rowIndex !== ex.rowIndex) &&
             _rawKey(r['Company']) + '|' + _rawKey(r['Plant / Site']) === key;
    })[0];
    if (clash) return _lgNo('Already listed as ' + clash['Plant No'] + ': ' + clash['Company'] + (site ? ' — ' + clash['Plant / Site'] : '') + '.');
    stamp['Researched On'] = ex ? _lgDayOf(ex['Researched On']) : today;
    /* A279 — "update target account research with new intelligence": stamped when something that
       IS research changes on an existing account, not on every save of it. */
    var intel = ex ? _lgDayOf(ex['Intel Updated On']) : '';
    if (ex) {
      var changed = ['equipment', 'notes', 'nextAction', 'source', 'status', 'philgeps'].some(function (k) {
        if (rec[k] === undefined) return false;
        var h = _lgHeaderFor(fields, k), was = _LG_FLAGS[h] ? _lgYes(ex[h]) : String(ex[h] == null ? '' : ex[h]).trim();
        var now_ = _LG_FLAGS[h] ? _lgYes(rec[k]) : String(rec[k] == null ? '' : rec[k]).trim();
        return was !== now_;
      });
      if (changed) intel = today;
    }
    stamp['Intel Updated On'] = intel;
  }

  if (entity === 'contacts') {
    var pn = rec.plantNo !== undefined ? rec.plantNo : ex['Plant No'];
    plant = plantOf(pn);
    if (!plant) return _lgNo('Plant ' + pn + ' is not on the list.');
    stamp['Intro Sent'] = ex ? _lgDayOf(ex['Intro Sent']) : '';            // the batch fan-out owns this
    var wasV = ex ? String(ex['Email Verified'] || 'Unverified') : 'Unverified';
    var nowV = String(rec.emailVerified !== undefined ? rec.emailVerified : wasV);
    var verified = function (v) { return v === 'Pattern' || v === 'Switchboard'; };
    stamp['Verified On'] = (ex && ex['Verified On']) ? _lgDayOf(ex['Verified On'])
                         : (verified(nowV) && !verified(wasV) ? today : '');
    var wasS = ex ? String(ex['Status'] || 'New') : 'New';
    var nowS = String(rec.status !== undefined ? rec.status : wasS);
    stamp['Replied On'] = (ex && ex['Replied On']) ? _lgDayOf(ex['Replied On'])
                        : (nowS === 'Replied' && wasS !== 'Replied' ? today : '');
    /* A279 — "schedule and confirm follow-up calls for the following days". Setting (or moving) the
       next-call date is the act that counts, on the day it was done; the date itself is the plan. */
    var wasNC = ex ? _lgDayOf(ex['Next Call Date']) : '';
    var nowNC = rec.nextCallDate !== undefined ? _lgDayOf(rec.nextCallDate) : wasNC;
    stamp['Next Call Date'] = nowNC;
    stamp['Scheduled On'] = (nowNC && nowNC !== wasNC) ? today : (ex ? _lgDayOf(ex['Scheduled On']) : '');
  }

  if (entity === 'suppliers') {
    var sco = String(rec.company !== undefined ? rec.company : ex['Company']).trim();
    var sk = _rawKey(sco);
    var sclash = rows.filter(function (r) {
      return !r['Deleted On'] && (!ex || r.rowIndex !== ex.rowIndex) && _rawKey(r['Company']) === sk;
    })[0];
    if (sclash) return _lgNo('Already listed as ' + sclash['Supplier No'] + ': ' + sclash['Company'] + '.');
    stamp['Researched On'] = ex ? _lgDayOf(ex['Researched On']) : today;
    var sFrom = ex ? String(ex['Status'] || 'Researching') : '';
    var sTo = String(rec.status !== undefined ? rec.status : (ex ? sFrom : 'Researching'));
    var isQual = function (v) { return v === 'Qualified' || v === 'Handed Off'; };
    stamp['Qualified On'] = (ex && ex['Qualified On']) ? _lgDayOf(ex['Qualified On']) : (isQual(sTo) && !isQual(sFrom) ? today : '');
    stamp['Handed Off On'] = (ex && ex['Handed Off On']) ? _lgDayOf(ex['Handed Off On']) : (sTo === 'Handed Off' && sFrom !== 'Handed Off' ? today : '');
    if (sTo === 'Handed Off' && sFrom !== 'Handed Off') {
      var loc = rec.location !== undefined ? rec.location : (ex ? ex['Location'] : '');
      var cat = rec.category !== undefined ? rec.category : (ex ? ex['Category'] : '');
      var who = rec.contact !== undefined ? rec.contact : (ex ? ex['Contact'] : '');
      supHandoff = _upsertSupplierFields(sco, { address: loc,
        notes: 'Lead-gen prospect' + (cat ? ' — ' + cat : '') + (who ? ' · contact: ' + who : '') }, actor);
    }
  }

  if (entity === 'batches') {
    settings = _lgSettings();
    if (ex) {
      var keep = ['Date', 'Sent On', 'Contact Nos', 'Count'];
      if (rec.contactNos !== undefined && _lgNos(rec.contactNos).join(', ') !== _lgNos(ex['Contact Nos']).join(', ')) {
        return _lgNo('The contacts on a logged batch cannot change — delete it and log it again.');
      }
      keep.forEach(function (h) { stamp[h] = h === 'Count' ? _num(ex[h]) : (h === 'Contact Nos' ? ex[h] : _lgDayOf(ex[h])); });
    } else {
      var nos = _lgNos(rec.contactNos);
      if (!nos.length) return _lgNo('Pick the contacts this batch went to.');
      var max = _num(settings.lgMaxBatch) || 60;
      if (nos.length > max) return _lgNo('A batch is at most ' + max + ' emails; this one lists ' + nos.length + '.');
      var byNo = {};
      _lgRows('contacts').forEach(function (c) { byNo[String(c['Contact No'])] = c; });
      for (var i = 0; i < nos.length; i++) {
        var c = byNo[nos[i]];
        if (!c) return _lgNo('Contact ' + nos[i] + ' is not on the list.');
        if (!String(c['Email'] || '').trim()) return _lgNo(c['Name'] + ' (' + nos[i] + ') has no email address.');
        if (String(c['Status']) === 'Do Not Contact') return _lgNo(c['Name'] + ' is marked Do Not Contact.');
      }
      var td = _lgTypedDate(rec.date, today, isDirector);
      if (td.error) return _lgNo(td.error);
      stamp['Date'] = td.day; stamp['Sent On'] = td.day;
      stamp['Contact Nos'] = nos.join(', '); stamp['Count'] = nos.length;
      fanout = { nos: nos, byNo: byNo, day: td.day, kind: rec.kind };
    }
  }

  if (entity === 'leads') {
    settings = _lgSettings();
    var lpn = rec.plantNo !== undefined ? rec.plantNo : ex['Plant No'];
    plant = plantOf(lpn);
    if (!plant) return _lgNo('Plant ' + lpn + ' is not on the list.');
    var cn = rec.contactNo !== undefined ? rec.contactNo : (ex ? ex['Contact No'] : '');
    var contact = cn ? _lgRows('contacts').filter(function (c) { return String(c['Contact No']) === String(cn); })[0] : null;
    if (cn && !contact) return _lgNo('Contact ' + cn + ' is not on the list.');
    var from = ex ? String(ex['Status'] || 'Handed Off') : '', to = ex ? String(rec.status || from) : 'Handed Off';
    if (ex && to !== from && (_LG_TRANSITIONS[from] || []).indexOf(to) === -1) {
      return _lgNo(from + ' → ' + to + ' is not a move this lead can make.');
    }
    var handing = !ex || (from === 'Returned' && to === 'Handed Off');
    if (handing) {
      var missing = _LG_QUAL.filter(function (q) { return !_lgYes(rec[q[0]] !== undefined ? rec[q[0]] : (ex ? ex[q[1]] : '')); });
      if (missing.length) return _lgNo('A lead is handed off only when all four conditions are met — missing: ' +
                                       missing.map(function (q) { return q[2]; }).join('; ') + '.');
      var territory = String(rec.territory || (ex && ex['Territory']) || plant['Territory'] || '');
      if (_LG_ENUM.territory.indexOf(territory) === -1) return _lgNo('Territory must be Luzon or VisMin.');
      var rep = String(rec.handedTo || (territory === 'VisMin' ? settings.lgRepVisMin : settings.lgRepLuzon) || '').trim();
      var up = _upsertClientFields(plant['Company'], {
        contactPerson: contact ? contact['Name'] : '', designation: contact ? contact['Role'] : '',
        email: contact ? contact['Email'] : '', phone: contact ? contact['Mobile'] : '',
        address: plant['Province'] || '',
        notes: 'Lead-gen prospect' + (plant['Plant / Site'] ? ' — ' + plant['Plant / Site'] : '')
      }, actor);
      stamp['Territory'] = territory; stamp['Handed To'] = rep;
      stamp['Client Created'] = up.created ? 'Yes' : 'Existing';
      stamp['Status'] = 'Handed Off';
      if (ex) { stamp['Handed Off On'] = _lgDayOf(ex['Handed Off On']); stamp['Rehanded On'] = today; }
      else stamp['Handed Off On'] = today;
      handoff = { rep: rep, territory: territory, client: up.customer, clientCreated: up.created };
    } else {
      ['Handed Off On', 'Rehanded On', 'Returned On', 'Booked On'].forEach(function (h) { stamp[h] = _lgDayOf(ex[h]); });
      stamp['Client Created'] = ex['Client Created'] || '';
      stamp['Handed To'] = rec.handedTo !== undefined ? String(rec.handedTo) : (ex['Handed To'] || '');
      stamp['Status'] = to;
      if (to === 'Presentation Booked' && !ex['Booked On']) stamp['Booked On'] = today;
      if (to === 'Returned' && !ex['Returned On']) stamp['Returned On'] = today;
    }
  }

  if (entity === 'accred') {
    var apn = rec.plantNo !== undefined ? rec.plantNo : ex['Plant No'];
    plant = plantOf(apn);
    if (!plant) return _lgNo('Plant ' + apn + ' is not on the list.');
    if (!String(rec.company || (ex && ex['Company']) || '').trim()) rec.company = plant['Company'];
    stamp['Started On'] = ex ? _lgDayOf(ex['Started On']) : today;
  }

  var id = ex ? ex[idHeader] : _nextNumber(cfg.sheet, 1, cfg.prefix);
  var values = fields.map(function (h) {
    if (stamp.hasOwnProperty(h)) return stamp[h];
    if (h === idHeader) return id;
    if (h === 'Created At') return ex ? ex[h] : now;
    if (h === 'Updated At') return now;
    if (h === 'Created By') return ex ? (ex[h] || actor) : actor;     // the session, not the payload
    if (h === 'Deleted On') return ex ? (ex[h] || '') : '';
    var ck = _camel(h);
    var v = (rec[ck] !== undefined && rec[ck] !== null) ? rec[ck] : (ex ? ex[h] : '');
    if (_LG_FLAGS[h]) return _lgYes(v) ? 'Yes' : '';
    if (_lgIsDateHeader(h)) return _lgDayOf(v);
    return v === undefined || v === null ? '' : v;
  });
  var rowIndex;
  if (ex) { sh.getRange(ex.rowIndex, 1, 1, fields.length).setValues([values]); rowIndex = ex.rowIndex; }
  else { sh.appendRow(values); rowIndex = sh.getLastRow(); _refStore('lg_' + entity, p.clientRef, id); }

  /* An email batch is the quota; the touches are the cadence. One touch row per contact, keyed on
     the batch so a deleted batch takes its touches out of the follow-up maths; `Intro Sent` is set
     where blank only — a second intro to the same person does not restart their clock. */
  if (fanout) {
    var csh = _sheet('LgContacts'), introCol = SCHEMA.LgContacts.indexOf('Intro Sent') + 1;
    fanout.nos.forEach(function (no, i) {
      var c = fanout.byNo[no];
      if (fanout.kind === 'Intro' && !c['Intro Sent']) csh.getRange(c.rowIndex, introCol, 1, 1).setValues([[fanout.day]]);
      _append('LgTouches', [id + '#' + (i + 1), no, 'Email', fanout.kind, id, fanout.day, actor, now]);
    });
  }

  var msg = entity === 'suppliers' ? ('Supplier · ' + String(rec.company || (ex && ex['Company']) || '') + (supHandoff ? ' → handed to procurement' + (supHandoff.created ? ' (new in Suppliers)' : '') : ' · ' + stamp['Status'] || ''))
          : entity === 'batches' ? _lgMsg(entity, rec, stamp['Count'])
          : entity === 'leads' ? _lgMsg(entity, rec, plant['Company'] + (handoff ? (handoff.rep ? ' → ' + handoff.rep : ' — no rep set for ' + handoff.territory) : ' · ' + stamp['Status']))
          : _lgMsg(entity, rec);
  var out = { success: true, entity: entity, id: id, refNo: id, rowIndex: rowIndex, message: msg, today: today };
  if (handoff) { out.handedTo = handoff.rep; out.territory = handoff.territory; out.client = handoff.client; out.clientCreated = handoff.clientCreated; }
  if (supHandoff) { out.supplier = supHandoff.supplier; out.supplierCreated = supHandoff.created; }
  return out;
}

/** deleteLeadgenRecord({entity, id, rowIndex}) — soft. The id must match the row, and a record
 *  whose counted day is not today needs the director: yesterday's number was already reported. */
function deleteLeadgenRecord(p) {
  var entity = String(p.entity || ''), cfg = LEADGEN[entity];
  if (!cfg || cfg.internal) return _lgNo('Unknown lead-gen entity: ' + entity);
  var role = _lgRole(p);
  if (_LG_WRITE_ROLES.indexOf(role) === -1) return _lgNo('Only the lead-generation user can remove this.');
  var ri = parseInt(p.rowIndex, 10), id = String(p.id || '');
  if (!ri || !id) return _lgNo('id and rowIndex are required.');
  var fields = SCHEMA[cfg.sheet], sh = _sheet(cfg.sheet);
  var row = _rows(cfg.sheet).filter(function (r) { return r.rowIndex === ri; })[0];
  if (!row) return _lgNo('Record not found — reload and try again.');
  if (String(row[fields[0]]) !== id) return _lgNo('The list has moved since it was loaded — reload and try again.');
  if (row['Deleted On']) return { success: true, entity: entity, id: id, refNo: id, message: id + ' was already removed.' };
  var today = _lgDay();
  var eventDay = _lgDayOf(row[cfg.event]) || _lgDayOf(row['Created At']);
  if (eventDay && eventDay !== today && role !== 'director') {
    return _lgNo(id + ' was counted on ' + eventDay + ' — only the director can remove a record from another day.');
  }
  sh.getRange(ri, fields.indexOf('Deleted On') + 1, 1, 1).setValues([[today]]);
  return { success: true, entity: entity, id: id, refNo: id, message: _lgMsg(entity, _lgDto(entity, row)) + ' removed' };
}

// ── The counter ───────────────────────────────────────────────────────────────────────────────
/** getLeadgenCounts({date?, weekStart?, user?}) — the eight numbers for one day and its week, from
 *  the stamp columns. The single source for the tiles, the week panel, the Friday PDF and the
 *  daily report; the page draws these, it never counts. */
function getLeadgenCounts(p) {
  p = p || {};
  var settings = _lgSettings(), cfg = _lgWeekCfg(settings);
  var today = _lgDay(), date = _lgDayOf(p.date) || today;
  var weekStart = _lgMonday(_lgDayOf(p.weekStart) || date);
  var user = String(p.user || '').trim();
  var mine = function (h) { return function (r) { return !user || String(r[h] || '') === user; }; };
  var plants = _lgRows('plants').filter(mine('Created By')), contacts = _lgRows('contacts').filter(mine('Created By')),
      suppliers = _lgRows('suppliers').filter(mine('Created By')), leads = _lgRows('leads').filter(mine('Created By')),
      calls = _rows('SalesCalls').filter(function (r) { return r['Kind']; }).filter(mine('User'));
  /* A279 — emails are counted as TOUCHES (one per contact, minus a removed batch's), so an email, a
     call and a LinkedIn message are the same unit and add up to "attempts" honestly. */
  var dead = {};
  _rows('LgEmailBatch').forEach(function (b) { if (b['Deleted On']) dead[String(b['Batch No'])] = 1; });
  var touches = _rows('LgTouches').filter(mine('Created By')).filter(function (t) { return !dead[String(t['Ref No'])]; });
  var reports = _rows('DailyReports').filter(function (r) {
    return String(r['Role'] || '').toLowerCase() === 'leadgen' && (!user || String(r['User'] || '').trim() === user) &&
           ['Submitted', 'Reviewed'].indexOf(String(r['Status'])) !== -1;
  });
  var on = function (rows, h, d) { return rows.filter(function (r) { return _lgDayOf(r[h]) === d; }); };
  var dayCounts = function (d) {
    var c = on(calls, 'Date', d), t = on(touches, 'Sent On', d);
    var em = t.filter(function (x) { return x['Channel'] === 'Email'; }).length;
    var li = t.filter(function (x) { return x['Channel'] === 'LinkedIn'; }).length;
    return {
      attempts: c.length + em + li,
      conversations: c.filter(function (r) { return String(r['Reached']) === 'Decision-maker'; }).length,
      emails: em,
      linkedin: li,
      suppliers: on(suppliers, 'Researched On', d).length,
      accounts: on(plants, 'Researched On', d).length,
      // "CRM updated": anything touched today — rows created or edited, calls and touches logged.
      crm: on(plants, 'Updated At', d).length + on(contacts, 'Updated At', d).length + on(suppliers, 'Updated At', d).length +
           on(leads, 'Updated At', d).length + c.length + t.length,
      eod: on(reports, 'Date', d).length ? 1 : 0,
      scheduled: on(contacts, 'Scheduled On', d).length + on(leads, 'Booked On', d).length,
      // the weekly figures and the reply rate, per day, so the week can be summed from here
      leads: on(leads, 'Handed Off On', d).length,
      meetings: on(leads, 'Booked On', d).length,
      suppliersHandedOff: on(suppliers, 'Handed Off On', d).length,
      intelUpdates: on(plants, 'Intel Updated On', d).length,
      introEmails: t.filter(function (x) { return x['Channel'] === 'Email' && x['Kind'] === 'Intro'; }).length,
      replies: on(contacts, 'Replied On', d).length,
      working: _lgIsWorking(d, cfg)
    };
  };
  var EXTRA = ['leads', 'meetings', 'suppliersHandedOff', 'intelUpdates', 'introEmails', 'replies'];
  var quotas = {}, quotasMax = {};
  _LG_COUNT_KEYS.forEach(function (k) {
    quotas[k] = _num(settings[_LG_QUOTA_KEY[k]]);
    quotasMax[k] = Math.max(quotas[k], _num(settings[_LG_QUOTA_KEY[k] + 'Max']));
  });
  var days = [], byDay = {}, working = [], totals = {};
  _LG_COUNT_KEYS.concat(EXTRA).forEach(function (k) { totals[k] = 0; });
  for (var i = 0; i < 7; i++) {
    var d = _lgAddDays(weekStart, i), c = dayCounts(d);
    days.push(d); byDay[d] = c;
    // A Saturday or a holiday is drawn on the strip but counts toward nothing: the week's target
    // is daily × working days, and a number earned outside them would make the ratio a lie.
    if (!c.working) continue;
    working.push(d);
    _LG_COUNT_KEYS.concat(EXTRA).forEach(function (k) { totals[k] += c[k]; });
  }
  var targets = {}, targetsMax = {};
  _LG_COUNT_KEYS.forEach(function (k) {
    // the two once-a-day items are once a day, not "five by Friday"
    targets[k] = quotas[k] * working.length; targetsMax[k] = quotasMax[k] * working.length;
  });
  /* The weekly items of the job description, as ranges. Active accounts is a STATE, not a sum. */
  var weekly = {
    activeAccounts: { value: plants.filter(function (r) { return String(r['Status'] || 'Active') === 'Active'; }).length,
                      min: _num(settings.lgWeekActiveAccounts), max: _num(settings.lgWeekActiveAccountsMax) },
    leads: { value: totals.leads, min: _num(settings.lgWeekLeads), max: _num(settings.lgWeekLeadsMax) },
    suppliersHandedOff: { value: totals.suppliersHandedOff, min: _num(settings.lgWeekSuppliers), max: _num(settings.lgWeekSuppliersMax) },
    intelUpdates: { value: totals.intelUpdates, min: 0, max: 0 },
    meetings: { value: totals.meetings, min: 0, max: 0 }
  };
  var day = dayCounts(date);
  return {
    success: true, today: today, hour: _lgHHmm(), date: date, day: day, quotas: quotas, quotasMax: quotasMax,
    week: { start: weekStart, end: days[6], days: days, workingDays: working, targets: targets, targetsMax: targetsMax,
            totals: totals, byDay: byDay, weekly: weekly, replies: totals.replies,
            replyRate: totals.introEmails ? Math.round(totals.replies / totals.introEmails * 1000) / 10 : null,
            replyRateAim: _num(settings.lgReplyRateAim) },
    sector: _lgSectorOfWeek(weekStart, settings),
    reps: { Luzon: String(settings.lgRepLuzon || ''), VisMin: String(settings.lgRepVisMin || '') },
    workingDays: cfg.days, holidays: Object.keys(cfg.holidays), maxBatch: _num(settings.lgMaxBatch) || 60
  };
}

// ── The follow-up reader ──────────────────────────────────────────────────────────────────────
/** getLeadgenFollowups({date?}) — who is due today, derived from `Intro Sent` and the touches.
 *  Day 3 / 7 / 14 working days after the intro with no touch since; a reply or a wrong person
 *  ends it; three touches with no reply go cold and come back three months on. Nothing is stored. */
function getLeadgenFollowups(p) {
  p = p || {};
  var cfg = _lgWeekCfg(_lgSettings());
  var today = _lgDay(), date = _lgDayOf(p.date) || today;
  var plants = {};
  _lgRows('plants').forEach(function (r) { plants[String(r['Plant No'])] = r; });
  var dead = {};
  _rows('LgEmailBatch').forEach(function (r) { if (r['Deleted On']) dead[String(r['Batch No'])] = 1; });
  var touches = {};
  _rows('LgTouches').forEach(function (t) {
    if (dead[String(t['Ref No'])]) return;
    var day = _lgDayOf(t['Sent On']);
    if (!day || day > date) return;
    (touches[String(t['Contact No'])] = touches[String(t['Contact No'])] || []).push({ day: day, channel: String(t['Channel']), kind: String(t['Kind']) });
  });
  var out = [];
  _lgRows('contacts').forEach(function (c) {
    var st = String(c['Status'] || 'New');
    if (st === 'Wrong Person' || st === 'Do Not Contact') return;
    var plant = plants[String(c['Plant No'])];
    if (plant && (plant['Status'] === 'Do Not Contact' || plant['Status'] === 'Customer')) return;
    var row = { contactNo: c['Contact No'], name: c['Name'], role: c['Role'], email: c['Email'], mobile: c['Mobile'],
                plantNo: c['Plant No'], company: plant ? plant['Company'] : '', plantSite: plant ? plant['Plant / Site'] : '',
                sector: plant ? plant['Sector'] : '', territory: plant ? plant['Territory'] : '', introSent: _lgDayOf(c['Intro Sent']) };
    var ts = (touches[String(c['Contact No'])] || []).slice().sort(function (a, b) { return a.day < b.day ? -1 : a.day > b.day ? 1 : 0; });
    var push = function (stage, due, action) {
      row.stage = stage; row.due = due; row.action = action; row.overdue = _lgDaysBetween(due, date); out.push(row);
    };
    /* A279 — a call booked for today (or missed) comes first, whatever the cadence says. */
    var ncd = _lgDayOf(c['Next Call Date']);
    if (ncd && ncd <= date) {
      var calledSince = ts.some(function (t) { return t.channel === 'Call' && t.day >= ncd; });
      if (!calledSince) { row.nextCallDate = ncd; push('Scheduled call', ncd, 'Follow-up call, as scheduled'); return; }
    }
    if (st === 'Replied') {
      var rep = _lgDayOf(c['Replied On']) || date;
      var called = ts.some(function (t) { return t.channel === 'Call' && t.day >= rep; });
      if (!called) push('Replied', rep, 'Call back');
      return;
    }
    var intro = row.introSent;
    if (!intro) return;
    var fus = ts.filter(function (t) { return t.kind !== 'Intro' && t.day >= intro; });
    var n = fus.length;
    if (n >= _LG_STEPS.length) {
      var revisit = _lgRollForward(_lgAddMonths(fus[n - 1].day, _LG_REVISIT_MONTHS), cfg);
      var since = ts.some(function (t) { return t.day >= revisit; });
      if (date >= revisit && !since) push('Revisit', revisit, 'Cold for ' + _LG_REVISIT_MONTHS + ' months — try again');
      return;
    }
    var due = _lgRollForward(_lgAddDays(intro, _LG_STEPS[n]), cfg);
    if (date >= due) push('Day ' + _LG_STEPS[n], due, n === 0 ? 'Follow-up email' : n === 1 ? 'Follow-up call' : 'Last follow-up');
  });
  out.sort(function (a, b) { return b.overdue - a.overdue || String(a.company).localeCompare(String(b.company)); });
  return { success: true, today: today, date: date, data: out };
}
...
function getSalesCalls(p) {
  ...
  return { success: true, data: rows.map(function (r) {
    return {
      ...
      kind: r['Kind'] || '', contactNo: r['Contact No'] || '', reached: r['Reached'] || '',
      rowIndex: r.rowIndex
    };
  }) };
}
...
function logSalesCall(p) {
  var contact = String(p.contact || '').trim(), company = String(p.company || '').trim();
  var kind = String(p.kind || '').trim(), contactNo = String(p.contactNo || '').trim();
  var lg = null, date;
  if (kind || contactNo) {
    /* A277 — the lead-gen path. A call with a Kind is one of the two call quotas, so the closed
       lists are enforced and the date is a Manila string; the rep's ordinary call below is
       untouched. */
    if (_LG_ENUM.callKind.indexOf(kind) === -1) return _lgNo('Call kind must be Cold or Follow-up.');
    if (_LG_ENUM.callOutcome.indexOf(String(p.outcome || '')) === -1) return _lgNo('Outcome must be one of: ' + _LG_ENUM.callOutcome.join(' · ') + '.');
    // A279 — a "real conversation" is one that reached the decision-maker; the outcome cannot say.
    if (_LG_ENUM.reached.indexOf(String(p.reached || '').trim()) === -1) return _lgNo('Say who you reached: ' + _LG_ENUM.reached.join(' · ') + '.');
    if (contactNo) {
      lg = _lgRows('contacts').filter(function (c) { return String(c['Contact No']) === contactNo; })[0];
      if (!lg) return _lgNo('Contact ' + contactNo + ' is not on the list.');
      if (!contact) contact = String(lg['Name'] || '');
      if (!company) {
        var pl = _lgRows('plants').filter(function (r) { return String(r['Plant No']) === String(lg['Plant No']); })[0];
        company = pl ? String(pl['Company'] || '') : '';
      }
    }
    var td = _lgTypedDate(p.date, _lgDay(), _lgRole(p) === 'director');
    if (td.error) return _lgNo(td.error);
    date = td.day;
  } else {
    date = p.date ? _dateStr(p.date) : _dateStr(_now());
  }
  if (!contact && !company) return { success: false, message: 'Contact or company is required.' };
  var no = _nextNumber('SalesCalls', 1, 'CALL');
  _append('SalesCalls', [no, date, p.actorName || '', contact, company, p.outcome || '',
    p.notes || '', _now(), kind, contactNo, kind ? String(p.reached || '').trim() : '']);   // 11 values (A279: Reached)
  if (lg) {
    _append('LgTouches', [no + '#1', contactNo, 'Call', kind, no, date, p.actorName || '', _now()]);
    // "Wrong person" is a fact about the contact, and it ends their cadence. Only a fresh contact
    // is moved: one who has replied is not un-replied by a later mis-dial.
    if (String(p.outcome) === 'Wrong person' && String(lg['Status'] || 'New') === 'New') {
      _setCellByKey('LgContacts', 'Contact No', contactNo, 'Status', 'Wrong Person');
    }
  }
  return { success: true, callNo: no, refNo: contact || company, kind: kind, contactNo: contactNo, date: date,
...
function deleteSalesCall(p) {
  var ri = parseInt(p.rowIndex, 10);
  if (!ri) return { success: false, message: 'rowIndex required.' };
  /* A277 — the row at a cached rowIndex is whatever is there NOW. With a callNo to check against,
     a stale list is refused instead of deleting the neighbour; without one (the rep's page, which
     predates this) it behaves as before. */
  var row = _rows('SalesCalls').filter(function (r) { return r.rowIndex === ri; })[0];
  if (!row) return { success: false, message: 'Call not found — reload the list.' };
  if (p.callNo && String(row['Call No']) !== String(p.callNo)) {
    return { success: false, message: 'The list has moved since it was loaded — reload before removing.' };
  }
  var callNo = String(row['Call No'] || '');
  if (row['Kind'] || row['Contact No']) {
    // Its touch goes with it, or the follow-up maths would count a call that never happened.
    var tsh = _sheet('LgTouches');
    _rows('LgTouches').filter(function (t) { return String(t['Ref No']) === callNo; })
      .sort(function (a, b) { return b.rowIndex - a.rowIndex; })
      .forEach(function (t) { tsh.deleteRow(t.rowIndex); });
  }
  _sheet('SalesCalls').deleteRow(ri);
  return { success: true, refNo: callNo, message: 'Call removed.' };
}
...
function linkQuotationEmail(p) {
  ...
  if (earliest && (!stored || isNaN(stored) || earliest < stored)) {
    patch['Sent At'] = earliest;
    /* A230 — the date is now a FACT, so the "estimated" caption must go with it. A215's backfill
       writes 'Sent At Basis' to mark a guessed date, and the screens draw an "Estimated from…" pill
       off that column alone; without this the rep attaches a real email, watches the date change,
       and is still told it is an estimate.
       ONLY INSIDE THIS BRANCH, and that asymmetry is the whole subtlety: when the STORED date wins
       (the else path, no patch), the basis must STAY, because what is on screen is still the guess.
       Clearing it there would turn an estimate into an assertion. */
    patch['Sent At Basis'] = '';
  }
...
/* A230 — resolve by Link ID, then fall back to the (quotation, message) PAIR.
 *
 * This used to read p.linkId and nothing else, and the sales mailbox posted {quotationNo, messageId}
 * — so it looked up the string "undefined", matched nothing, and answered "Link not found." about a
 * link named on screen. Every Detach from the mailbox since A217 failed that way. The caller now
 * sends the id it always had; this fallback is for the case that motivated the old message being
 * wrong in the first place — a row deleted from the sheet by hand, or a client that has the pair but
 * not the id.
 *
 * THE PAIR, NEVER THE MESSAGE ALONE. One email can legitimately carry two quotations, as two rows
 * with two Link IDs. Matching on messageId by itself would detach it from both, silently, from a
 * button that named only one.
 *
 * _qeNormId IS MANDATORY on that fallback. Message-IDs arrive bracketed from some clients and bare
 * from others; a plain string compare would reproduce "Link not found." from the opposite direction
 * the first time a bracketed id came through. dismissQuotationEmail already normalises — this is the
 * same rule, applied in the same place.
 *
 * EXPLICIT NON-GOAL: this does NOT recompute Quotations['Sent At']. Detaching the last link leaves
 * the back-dated send date standing, deliberately — the mailbox says so out loud ("The quotation
 * keeps its send date; only the link goes"). Recomputing it here would move a follow-up clock, and
 * every badge and nudge that reads it, on a single click. */
function unlinkQuotationEmail(p) {
  var rows = _rows('QuotationEmails');
  var r = null;
  if (p && p.linkId) {
    r = rows.filter(function (x) { return String(x['Link ID']) === String(p.linkId); })[0] || null;
  }
  if (!r && p && p.quotationNo && p.messageId) {
    var wantQ = String(p.quotationNo), wantM = _qeNormId(p.messageId);
    r = rows.filter(function (x) {
      return String(x['Quotation No']) === wantQ && _qeNormId(x['Message ID']) === wantM;
    })[0] || null;
  }
  if (!r) {
    /* Say what was looked for. The old message asserted a fact about the sheet when the real fact
       was that the handler had been given nothing to look up. */
    return { success: false, message: 'No link found for ' +
      (p && p.linkId ? 'id ' + p.linkId : '') +
      (p && p.quotationNo ? (p.linkId ? ', nor for ' : '') + p.quotationNo + ' + that message' : '') +
      (!(p && (p.linkId || p.quotationNo)) ? '(nothing was sent to look up)' : '') + '.' };
  }
  var st = String(r['Status'] || _QE_ACTIVE);
  if (st !== _QE_ACTIVE) {
    return { success: false, message: st === 'Dismissed'
      ? 'That email was dismissed for ' + r['Quotation No'] + ', not attached to it — there is nothing to detach.'
      : 'That link is already detached.' };
  }
  _qeSet(r['Link ID'], { 'Status': 'Unlinked', 'Note': String((p && p.reason) || r['Note'] || '') });
  return { success: true, refNo: r['Quotation No'], linkId: String(r['Link ID'] || ''),
    message: 'Email unlinked.' };
}

/** A217 — record what the mailbox found when it looked for a reply.
 *
 *  THIS IS THE MISSING HALF OF A208. `Reply At` / `Reply From` / `Reply Checked At` have been in the
 *  schema since that change (SCHEMA:233), are read by _qeMap, and were written by NOTHING — they are
 *  stamped '' at link time and never touched again. So flowFollowUp's `replied` branch
 *  (flow-api.js:1067) has never once fired in production, the worklist's top-priority step "They
 *  replied" was unreachable, and every link renders "reply not checked". The system could not answer
 *  the single most important question about a sent quotation.
 *
 *  The matching itself is Flask's: /api/email/quotation-threads walks References / In-Reply-To with a
 *  normalised-subject fallback (blueprints/email_log.py:943). It was finished and callerless. This
 *  handler only writes down what it found.
 *
 *  `checkedAt` is stamped EVEN WHEN NO REPLY WAS FOUND, and that is the point: "we looked on Tuesday
 *  and there was nothing" is a different fact from "nobody has ever looked", and flowFollowUp already
 *  distinguishes them. Writing only the hits would leave every un-replied quotation permanently
 *  indistinguishable from an unchecked one.
 *
 *  A reply is never overwritten by a later empty check — the client did reply; a subsequent sweep
 *  that fails to see it (a moved message, a re-indexed mailbox) must not erase that.
 *
 *  NOT _SECURED, deliberately, and this is a change of mind worth recording. The plan for A217 said
 *  it should be, on the grounds that reply state decides whether a deal looks alive. But its three
 *  A208 siblings — linkQuotationEmail, unlinkQuotationEmail, dismissQuotationEmail — are all
 *  unsecured, and linkQuotationEmail is strictly more powerful than this one: it BACK-DATES
 *  Quotations['Sent At']. Securing one of four would not raise the floor, it would only make the
 *  group inconsistent and give a false impression that the boundary had moved. The whole
 *  QuotationEmails write group should be secured together, as its own change, alongside the other
 *  unsecured writes already on the list (addDocument, setFlowSettings). */
function setQuotationEmailReply(p) {
  p = p || {};
  var replies = p.replies;
  if (typeof replies === 'string') { try { replies = JSON.parse(replies); } catch (e) { replies = null; } }
  if (!replies || typeof replies !== 'object') return { success: false, message: 'replies required.' };
  var checkedAt = String(p.checkedAt || _now());

  var wrote = 0, found = 0, skipped = 0;
  _rows('QuotationEmails').forEach(function (r) {
    if (String(r['Status'] || _QE_ACTIVE) !== _QE_ACTIVE) return;
    var id = _qeNormId(r['Message ID']);
    if (!id) return;
    // The caller keys by the raw Message-ID; compare on the same normalisation the table uses.
    var hit = null;
    Object.keys(replies).forEach(function (k) { if (_qeNormId(k) === id) hit = replies[k]; });
    if (hit === null || hit === undefined) return;

    var patch = { 'Reply Checked At': checkedAt };
    var at = hit && hit.repliedAt ? String(hit.repliedAt) : '';
    if (at) {
      patch['Reply At'] = at;
      patch['Reply From'] = String((hit && hit.repliedFrom) || '');
      found++;
    } else if (r['Reply At']) {
      skipped++;                                  // already replied: keep it, only the check date moves
    }
    _qeSet(r['Link ID'], patch);
    wrote++;
  });

  return { success: true, checked: wrote, replies: found, keptExisting: skipped,
    message: wrote ? (found + ' repl' + (found === 1 ? 'y' : 'ies') + ' found across ' + wrote +
                      ' linked message' + (wrote === 1 ? '' : 's') + '.')
                   : 'No linked messages to check.' };
}
...
function dismissQuotationEmail(p) {
  ...
  if (existing) {
    /* A230 — REFUSE TO DEMOTE A LIVE LINK.
     *
     * This branch matched across every status, so an ACTIVE row was flipped straight to Dismissed —
     * a detach, performed by the handler that is not the detach. It skipped every guard
     * unlinkQuotationEmail has, left the back-dated Quotations['Sent At'] standing, and wrote an
     * audit row reading 'Email Dismissed' when what had happened was a detach. The audit lying is
     * the part no UI change can repair, which is why the refusal lives here and not only in the
     * screen that used to offer the button.
     *
     * An Unlinked row still flips: "that was wrong, and stop suggesting it" is a real transition,
     * and it keeps one row per (quotation, message) pair. */
    var exSt = String(existing['Status'] || _QE_ACTIVE);
    if (exSt === _QE_ACTIVE) {
      return { success: false, message: 'That email is currently attached to ' + p.quotationNo +
        '. Detach it first — dismissing is for suggestions you have not accepted.' };
    }
...
/* ── A215: park a quotation off the worklist until a date ────────────────────────────────────────
   NOT the same thing as Follow Up Days, and the difference matters. A threshold says "this client
   always takes three weeks, stop nagging me at seven days" — it is a property of the deal. Parking
   says "not now, ask me again in October" — it is a decision the rep just made, with a reason.
   Folding them together would mean a rep who parks one deal quietly changes how every future
   follow-up on it is judged. */
function snoozeQuotation(p) {
  p = p || {};
  if (!p.quotationNo) return { success: false, message: 'quotationNo required.' };
  var q = _quotationRow(p.quotationNo);
  if (!q) return { success: false, message: 'Quotation not found.' };

  var until = _dateStr(p.until || '');
  if (!until) {   // no date = un-park, which must always be one action away
    _setQuotationCells(p.quotationNo, { 'Snooze Until': '', 'Snooze Reason': '' });
    return { success: true, refNo: p.quotationNo, until: '',
      message: 'Back on your list.' };
  }
  if (!/^\d{4}-\d{2}-\d{2}$/.test(until)) {
    return { success: false, message: 'Give a date as YYYY-MM-DD.' };
  }
  if (until <= _dateStr(_now())) {
    return { success: false, message: 'Park it until a date in the future — ' + until + ' has passed.' };
  }
  /* A reason is required. Six weeks later "why is this parked?" is the only question anybody asks,
     and a blank answer makes the park indistinguishable from a deal somebody forgot. */
  var why = String(p.reason || '').trim();
  if (!why) return { success: false, message: 'Say why you are parking it — future you will want to know.' };

  _setQuotationCells(p.quotationNo, { 'Snooze Until': until, 'Snooze Reason': why });
  return { success: true, refNo: p.quotationNo, until: until, reason: why,
    message: 'Parked until ' + until + '.' };
}

/* ── A215: the send-date backfill ────────────────────────────────────────────────────────────────
   sendQuotation stamps 'Sent At' only going forward, so every quotation marked Sent before that
   existed has none — 60 of 85 live rows. The follow-up engine returns 'unknown' for all of them and
   goes quiet, which means the worklist is blind to almost the whole pipeline.

   The estimate is deliberately crude and deliberately LABELLED. 'Approved At' is the best proxy
   there is: a rep sends a quotation when it is approved, usually the same day. But an estimate that
   looks identical to a recorded date is how a follow-up clock starts lying, so every row written
   here carries its basis, and every surface that shows the date says "estimated".

   Preview and apply are separate handlers, the A193 pattern: nothing writes until somebody has read
   the list of what would change. */
var _QSA_BASIS = { approved: 'Estimated from approval', created: 'Estimated from quotation date' };
/* Mirrors FLOW_Q_SENT_STATUSES in dashboard/js/flow-api.js — the statuses that mean "it is with the
   client", and therefore the only ones a send date belongs on. */
var _QSA_SENT_STATUSES = { 'Sent': 1 };

/** What would the backfill do? Read-only, writes nothing. */
function previewQuotationSentAt(p) {
  p = p || {};
  var rows = [], skipped = 0, noBasis = 0;
  _rows('Quotations').forEach(function (q) {
    if (!_QSA_SENT_STATUSES[String(q['Status'] || '')]) return;
    if (String(q['Sent At'] || '').trim()) { skipped++; return; }        // already recorded — never touched
    var basis = '', src = q['Approved At'] || '';
    if (src) basis = _QSA_BASIS.approved;
    else { src = q['Date'] || ''; basis = _QSA_BASIS.created; }
    var est = _dateStr(src);
    if (!est) { noBasis++; return; }                                     // nothing to estimate FROM
    rows.push({ quotationNo: String(q['Quotation No']), customer: String(q['Customer'] || ''),
      createdBy: String(q['Created By'] || ''), status: String(q['Status'] || ''),
      estimatedSentAt: est, basis: basis, ageDays: _qsaAge(est) });
  });
  rows.sort(function (a, b) { return b.ageDays - a.ageDays; });
  return { success: true, data: rows, count: rows.length,
    alreadyRecorded: skipped, cannotEstimate: noBasis,
    message: rows.length
      ? rows.length + ' quotation(s) would get an ESTIMATED send date. ' + skipped +
        ' already have a real one and are left alone.'
      : 'Nothing to estimate — every sent quotation already has a date.' };
}
function _qsaAge(ymd) {
  var d = new Date(_dateStr(ymd) + 'T00:00:00'), t = new Date(_dateStr(_now()) + 'T00:00:00');
 ...
 *  this twice is a no-op and a later correction by hand is never undone. */
/* A243 — CORRECTING ONE QUOTATION'S OWNER.
 *
 * The pricing-request side has had setPricingRequestSalesperson since A226; the quotation side had
 * nothing, and previewQuotationOwners deliberately skips any row that already carries a recorded
 * owner ("idempotent: a recorded owner is never touched"). That is right for a backfill — it must
 * never overwrite a human decision — but it left a real gap: a quotation whose Salesperson was
 * recorded WRONGLY could not be corrected through the app at all, only by editing the sheet.
 *
 * Found from a live case: both rows numbered 2026-457 were typed by Kimberlyn Blones and carried the
 * owner "Neil" — a bare first name matching no account. getQuotations scopes a rep's list by OWNER,
 * so those quotations were invisible to every sales rep INCLUDING Neil Estur, and only oversight
 * roles (which send no filter) could see them at all. Nothing warned anybody.
 *
 * Same shape and the same role gate as its pricing-request twin, deliberately: reattribution decides
 * whose tracker a deal appears in and whose commission it feeds, so it is _SECURED and the role has
 * to come from the session rather than the browser. Blank clears it, which falls the owner back to
 * the initials in the number and then to the typist — the A218 chain.
 */
function setQuotationSalesperson(p) {
  if (!p || !p.quotationNo) return { success: false, message: 'quotationNo required.' };
  var role = String(p.actorRole || '').toLowerCase();
  if (['director', 'management', 'admin', 'accounting'].indexOf(role) < 0) {
    return { success: false, message: 'Only oversight roles can change who a quotation belongs to.' };
  }
  var q = _quotationRow(p.quotationNo);
  if (!q) return { success: false, message: 'Quotation ' + p.quotationNo + ' not found.' };
  var who = String(p.salesperson == null ? '' : p.salesperson).trim();
  var was = _quoOwner(q);
  _setQuotationCells(p.quotationNo, { 'Salesperson': who });
  return { success: true, quotationNo: String(p.quotationNo), salesperson: who, previous: was,
    message: who ? (p.quotationNo + ' now belongs to ' + who + ' (was ' + (was || 'nobody') + ').')
                 : (p.quotationNo + ' no longer names a salesperson; it falls back to the initials in '
                    + 'its number and then to whoever typed it (' + was + ').') };
}
...
function runQuotationSentAtBackfill(p) {
  ...
  var role = String(p.actorRole || '').toLowerCase();
  if (['director', 'management', 'admin', 'accounting'].indexOf(role) < 0) {
    return { success: false, message: 'Only management, the director or admin can backfill send dates.' };
  }
  var only = null;
  if (p.quotationNos) {
    try { only = {}; (JSON.parse(p.quotationNos) || []).forEach(function (n) { only[String(n)] = 1; }); }
  ...
  var plan = previewQuotationSentAt(p);
  var done = [], failed = 0;
  plan.data.forEach(function (r) {
    if (only && !only[r.quotationNo]) return;
    try {
      _setQuotationCells(r.quotationNo, { 'Sent At': r.estimatedSentAt, 'Sent At Basis': r.basis });
      done.push(r.quotationNo);
    } catch (e) { failed++; }
  });
  return { success: true, updated: done.length, failed: failed, refNo: done[0] || '',
    message: 'Estimated a send date for ' + done.length + ' quotation(s)' +
      (failed ? ', ' + failed + ' failed' : '') + '. Each is marked as an estimate, not a record.' };
}
...
var _FLOW_SETTING_DEFAULTS = {
  ...
  /* A277 — the lead-generation quotas, per working day, and the working week itself. Director or
     management adjust them here after the first month; nothing on the page is a target of its own.
     lgRepLuzon / lgRepVisMin hold the rep's LOGIN USERNAME, matched to session.username — never a
     display name, which is the joined-by-a-typed-string failure the scan found twice already. */
  /* A279 — every quota is a RANGE, as the job description writes them ("40-50"). Met at the minimum;
     the maximum is the stretch shown beside it. The weekly figures are per week, not per day. */
  lgQuotaAttempts: 40, lgQuotaAttemptsMax: 50, lgQuotaConversations: 5, lgQuotaConversationsMax: 10,
  lgQuotaEmails: 25, lgQuotaEmailsMax: 30, lgQuotaLinkedin: 5, lgQuotaLinkedinMax: 10,
  lgQuotaSuppliers: 5, lgQuotaSuppliersMax: 10, lgQuotaAccounts: 10, lgQuotaAccountsMax: 15,
  lgQuotaCrm: 1, lgQuotaCrmMax: 1, lgQuotaEod: 1, lgQuotaEodMax: 1, lgQuotaScheduled: 1, lgQuotaScheduledMax: 3,
  lgWeekActiveAccounts: 20, lgWeekActiveAccountsMax: 30, lgWeekLeads: 10, lgWeekLeadsMax: 20,
  lgWeekSuppliers: 25, lgWeekSuppliersMax: 50,
  lgMaxBatch: 60, lgReplyRateAim: 8,
  lgWorkingDays: 'Mon,Tue,Wed,Thu,Fri', lgHolidays: '',
  lgSectorCycle: 'Cement,Mining,Power,Water',
  lgRepLuzon: '', lgRepVisMin: ''
};
function getFlowSettings() {
  ...
  try {
    _rows('FlowSettings').forEach(function (r) {
      ...
      if (!k) return;
      var v = r['Value'];
      /* A277 — coerce only a WHOLLY numeric value. parseFloat('2026-12-25') is 2026, which turned a
         holiday into a year; Number('2026-12-25') is NaN and leaves the string alone. */
      var t = (v === '' || v === null || v === undefined || typeof v === 'object') ? NaN : Number(String(v).trim());
      out[k] = (String(v === null || v === undefined ? '' : v).trim() !== '' && isFinite(t)) ? t : v;
    });
...
/* A234 — how complete an order must be before its commission may be SUBMITTED. Not 100: the client
   withholds 1% EWT, so the cash that can ever arrive is _commExpectedCash(), not the order's face
   value, and half a peso of rounding on a seven-figure order must not hold somebody's pay. This is
   the number _commCoverageNote has always printed PARTIAL below; A234 promoted it from a sentence
   to a rule, which is why it is a named constant now rather than a literal in two places. */
var _COMM_FULL_PCT      = 99.5;
...
/* A234 — HOW COMPLETE IS THIS ORDER? One function, two consumers.
 *
 * _commCoverageNote (the sentence a rep and an approver read) and the submit gate in
 * submitCommissionRequest (the rule that stops them) both call this. That is the entire reason it
 * exists as a function: before A234 the 99.5% test lived inside the note as a literal, and putting a
 * second copy in the gate would have let the screen say "PARTIAL" while the server allowed a submit,
 * or the reverse — a disagreement about pay that nobody could explain.
 *
 * `pct` is deliberately the ROUNDED figure, because the note prints it. A claim at 99.46% prints
 * "(99.5%)" and prints no PARTIAL warning; if the gate tested the unrounded value it would refuse a
 * claim the screen had just called complete.
 *
 * CASH IS COMPARED TO CASH — see the A211 note below, which is the trap this inherits.
 *
 * @returns {{valued:boolean, cashDue:number, claimedAfter:number, pct:number,
 *            full:boolean, shortfall:number, over:boolean}}
 *          `valued` false when the order carries no value on record: coverage is then UNKNOWABLE, not
 *          zero, and the caller must not read `full` as a judgement about collection.
 */
function _commCoverage(soTotal, prior, thisBase) {
  var total = _num(soTotal);
  var claimedAfter = _num(prior) + _num(thisBase);
  if (total <= 0) {
    return { valued: false, cashDue: 0, claimedAfter: claimedAfter, pct: 0,
             full: false, shortfall: 0, over: false };
  }
  var cashDue = _commExpectedCash(total);
  var pct = Math.round(claimedAfter / cashDue * 1000) / 10;
  return {
    valued: true, cashDue: cashDue, claimedAfter: claimedAfter, pct: pct,
    full: pct >= _COMM_FULL_PCT,
    shortfall: Math.max(0, Math.round((cashDue - claimedAfter) * 100) / 100),
    over: claimedAfter > cashDue + 0.005
  };
}
...
function _commCoverageNote(soTotal, invoiced, prior, thisBase) {
  var total = _num(soTotal);
  var cov = _commCoverage(soTotal, prior, thisBase);
  var claimedAfter = cov.claimedAfter;
  if (!cov.valued) {
...
function _commDerive(soNo, collectionNos, excludeCommNo, ctx) {
  ...
  /* A234 — the same object the note was built from, returned so the SUBMIT GATE and the browser can
     both read the decision instead of each re-deriving it from soTotal/prior/base. Three copies of
     one arithmetic is how the screen and the server end up disagreeing about somebody's pay. */
  var coverage = _commCoverage(soTotal, prior, money.base);
  ...
  return {
    ...
    coverageNote: note,
    coverage: coverage,           // A234 — {valued, cashDue, claimedAfter, pct, full, shortfall, over}
    items: items
  };
}
...
/** A239 — WHY CAN A REP NOT CLAIM THIS ORDER? Read-only; writes nothing, decides nothing.
 *
 *  Commission money reaches a rep down one chain, and it can break at any of four links:
 *
 *      Collection → (its AR row, or its invoice) → Sales Order → Quotation → owner
 *
 *  getCommissionClaimable already walks exactly that and already builds a reason string for each
 *  failure — but it drops those rows into `unresolved` / `unattributed` buckets aggregated across the
 *  whole book, so the question people actually ask ("where is Gerald's Mincon money?") could only be
 *  answered by reading the sheets by hand. This asks it per order.
 *
 *  It reuses _commContext, _commSoForCollection and _commSalesperson rather than re-deriving any of
 *  it. That is the point: a diagnostic that computes attribution its own way would eventually
 *  disagree with the list it is supposed to explain, and then it would be worse than nothing.
 *
 *  Pass nothing for the whole book; pass quotationNo or soNo to answer for one order. */function previewCommissionAttribution(p) {
  p = p || {};   // hand-callable from the Apps Script editor; _dispatch always passes an object
  var wantQuo = String(p.quotationNo || '').trim();
  var wantSo = String(p.soNo || '').trim();

  var ctx = _commContext();
  var out = { resolved: [], unresolved: [], unattributed: [],
              totals: { resolved: 0, unresolved: 0, unattributed: 0, collections: 0 } };

  ctx.collections.forEach(function (c) {
    var net = _num(c['Amount (PHP)']) - _num(c['EWT (PHP)']);
    var res = _commSoForCollection(c, ctx.arByNo, ctx.invByNo);
    var so = res.soNo ? ctx.soByNo[res.soNo] : null;
    var quoNo = so ? String(so['Quotation No'] || '').trim() : '';

    if (wantSo && String(res.soNo) !== wantSo) return;
    if (wantQuo && quoNo !== wantQuo) return;
    ...
    var line = {
      ...
      date: c['Date'], netCash: net, soNo: res.soNo, via: res.via, quotationNo: quoNo
    };

    if (!res.soNo) {
      out.totals.unresolved += net;
      out.unresolved.push(_commExtend(line, { link: 'sales order',
        reason: 'No sales order can be resolved from this collection, its AR row, or its invoice.' }));
      return;
    }
    if (!so) {
      out.totals.unresolved += net;
      out.unresolved.push(_commExtend(line, { link: 'sales order',
        reason: 'Sales order ' + res.soNo + ' does not exist.' }));
      return;
    }
    var who = _commSalesperson(so, ctx.quoteByNo);
    if (!who.name) {
      out.totals.unattributed += net;
      out.unattributed.push(_commExtend(line, { link: quoNo ? 'owner' : 'quotation',
                                                reason: who.reason }));
      return;
    }
    out.totals.resolved += net;
    out.resolved.push(_commExtend(line, {
      salesperson: who.name, ownerBasis: _commOwnerBasis(ctx.quoteByNo[quoNo], quoNo)
    }));
  });

  ['resolved', 'unresolved', 'unattributed'].forEach(function (k) {
    out.totals[k] = _commPeso(out.totals[k]);
  });

  /* ASKED ABOUT ONE ORDER, ANSWER ABOUT THE ORDER — not only about whatever collections happen to
     reach it. The commonest real case is a quotation whose sales order exists and which nothing has
     paid yet: filtering collections would return an empty list, and an empty list is indistinguishable
     from "no such order". This walks the chain from the top and names the FIRST link that is missing,
     which is the actual answer to "why can this not be claimed". */
  if (wantQuo || wantSo) out.order = _commOrderChain(wantQuo, wantSo, ctx, out);

  out.success = true;
  out.scope = wantQuo ? ('quotation ' + wantQuo) : (wantSo ? ('sales order ' + wantSo) : 'every collection');
  out.message = out.order
    ? out.order.summary
    : (out.totals.collections + ' collection(s) examined \u00b7 ' + out.resolved.length +
       ' attributable, ' + out.unresolved.length + ' with no sales order, ' +
       out.unattributed.length + ' with nobody to pay.');
  return out;
}

/** WHICH RULE named the owner — the stored column, the initials in the number, or merely the typist.
 *  A wrong name has to be traceable to the rule that produced it, or a bad rule looks like bad data. */
function _commOwnerBasis(quo, quoNo) {
  if (!quo) return '';
  if (String(quo['Salesperson'] || '').trim()) return 'Salesperson column';
  if (_QUO_INITIALS[_quoInitials(quoNo)]) return 'initials in the quotation number';
  return 'assumed from who created it';
}

/** The chain for ONE order, top down, stopping at the first broken link. */
function _commOrderChain(wantQuo, wantSo, ctx, walked) {
  var o = { quotationNo: wantQuo, soNo: wantSo, quotationExists: false, soExists: false,
            collections: walked.totals.collections, collectedCash: walked.totals.resolved +
              walked.totals.unresolved + walked.totals.unattributed,
            salesperson: '', ownerBasis: '', blockedAt: '', summary: '' };

  var quo = wantQuo ? ctx.quoteByNo[wantQuo] : null;
  if (wantSo && !quo) {
    var soRow = ctx.soByNo[wantSo];
    if (soRow) { o.quotationNo = String(soRow['Quotation No'] || '').trim();
                 quo = o.quotationNo ? ctx.quoteByNo[o.quotationNo] : null; }
  }
  o.quotationExists = !!quo;

  // Which sales order carries it — the link the rep cannot see and cannot fix themselves.
  if (!o.soNo && o.quotationNo) {
    Object.keys(ctx.soByNo).forEach(function (k) {
      if (String(ctx.soByNo[k]['Quotation No'] || '').trim() === o.quotationNo) o.soNo = k;
    });
  }
  o.soExists = !!(o.soNo && ctx.soByNo[o.soNo]);

  if (quo) {
    var who = _commSalesperson({ 'Quotation No': o.quotationNo }, ctx.quoteByNo);
    o.salesperson = who.name;
    o.ownerBasis = _commOwnerBasis(quo, o.quotationNo);
  }

  if (!o.quotationExists) {
    o.blockedAt = 'quotation';
    o.summary = 'Quotation ' + (o.quotationNo || '(none)') + ' is not in the book, so nothing can be attributed.';
  } else if (!o.soExists) {
    o.blockedAt = 'sales order';
    o.summary = 'Quotation ' + o.quotationNo + ' has no sales order, so no collection can ever reach it.';
  } else if (!o.salesperson) {
    o.blockedAt = 'owner';
    o.summary = 'Sales order ' + o.soNo + ' resolves to no salesperson, so there is nobody to pay.';
  } else if (!o.collections) {
    o.blockedAt = 'collection';
    o.summary = 'Sales order ' + o.soNo + ' belongs to ' + o.salesperson +
                ', but no collection has been recorded against it yet — there is no cash to claim.';
  } else {
    o.blockedAt = '';
    o.summary = o.collections + ' collection(s) totalling ' + _commPeso(o.collectedCash) +
                ' on sales order ' + o.soNo + ', attributable to ' + o.salesperson + '.';
  }
  return o;
}
...
function submitCommissionRequest(p) {
   ...
   * actually been collected. Drafting stays open on purpose: the rep builds the claim, saves it, and
   * watches it close the gap. Only this door is shut, so nothing part-collected ever reaches the
   * director/management queue and no approver has to judge whether a partial claim is reasonable.
   *
   * THIS ENDS PROGRESSIVE CLAIMING, and that is a real loss, not an oversight. coverage.claimedAfter
   * is prior + this claim, so clearing _COMM_FULL_PCT means claiming every collection on the order —
   * a rep can no longer bank a slice as each instalment lands. A210's ladder was built to make part
   * claims arithmetically sound, and it still is; what changed is a policy decision that a commission
   * is earned when the ORDER is complete, not as the cash trickles in.
   *
   * The threshold is not 100 and must not be: the client withholds 1% EWT, so the collectible cash is
   * _commExpectedCash(), never the order's face value. Testing against the face value would refuse
   * every real VAT order forever — see the A211 note on _commCoverageNote for the same trap caught
   * from the other side.
   *
   * An UNVALUED order is refused too, and deliberately says something different. Coverage there is
   * unknowable rather than incomplete, so telling the rep "not fully collected" would be a guess
   * dressed as a fact; the honest answer names the missing data and points at who can fix it. */
  var cov = d.coverage;
  if (!cov.valued) {
    return { success: false, message:
      'Sales order ' + r['SO No'] + ' carries no value on record, so there is no way to tell whether ' +
      'it has been fully collected — and a commission may only be submitted once it has. Ask ' +
      'accounting to put the order total on the sales order, then submit this again.' };
  }
  if (!cov.full) {
    return { success: false, message:
      'This order is not fully collected yet, so the commission cannot be submitted. ' +
      _commMoney(cov.claimedAfter) + ' of ' + _commMoney(cov.cashDue) + ' collectible has been ' +
      'claimed (' + cov.pct + '%) — ' + _commMoney(cov.shortfall) + ' still to collect. ' +
      'Keep the draft: it can be submitted as soon as the balance lands.' };
  }
...
function seedCommissionDemo(p) {
  ...
  _append('Invoices', [
    // A278: 13 values. The VAT is stated, so d.gross is now DERIVABLE from this row
    // (38,607.93 + 4,632.95 = 43,240.88) instead of being a free-standing constant beside it.
    d.invNo, d.soNo, today, d.customer, d.exVat, 0, who, now, '', '', 0, 12, 4632.95
  ]);
  _append('ARAging', [
...
function submitTravelReplenishment(p) {
  ...
  /* A238 — LEGS THAT EVIDENCE NOTHING. Since A237 a leg carrying 'Has Receipt' = Yes with no receipt
     filed against it prints on neither page: not the itinerary unless it is Transport, and never the
     certificate, which is reserved for expenses that CANNOT produce a receipt. Its money still lands
     in Total Spent on the cover, so an approver signs for an amount with nothing behind it.
     Waivable by an approver, and by exactly the same rule as the itinerary below — never by the
     traveller, whatever their role. */
  /* ASK THE SAME QUESTION getTravelReceipts ASKS. 'Receipt Doc ID' is NOT the answer: the column is
     written back in a second pass that flow-travel.js explicitly allows to fail quietly, precisely
     because the durable link is the Drive FILE NAME (receipt-<seq>.jpg) and the id is an
     optimisation. Gating on the column would refuse a rep whose photo uploaded perfectly well but
     whose write-back missed — which is the exact case that comment exists to describe. */
  var filedSeqs = {};
  _rows('Documents').forEach(function (d) {
    if (String(d['Module']) !== _TRAV_DOC_MODULE) return;
    if (String(d['Ref No']) !== String(p.travNo)) return;
    if (!d['File ID']) return;
    var s = _travReceiptSeq(d['File Name']);
    if (s) filedSeqs[s] = true;
  });
  var noProof = items.filter(function (i) {
    if (String(i['Has Receipt'] || '') !== 'Yes') return false;   // on the certificate; evidenced by it
    if (!(_num(i['Amount']) > 0)) return false;
    if (String(i['Receipt Doc ID'] || '').trim()) return false;   // the id when it did get written
    return !filedSeqs[_num(i['Seq'])];                            // otherwise the file itself
  });
  var canWaiveAny = _travMayActForAll(p.actorRole) &&
                    String(p.actorName || '').trim() !== String(r['User'] || '').trim();
  if (noProof.length && !(String(p.waiverReason || '').trim() && canWaiveAny)) {
    var owed = 0;
    noProof.forEach(function (i) { owed += _num(i['Amount']); });
    return { success: false, needsReceiptWaiver: true, legs: noProof.length,
      amount: _travPeso(owed),
      message: noProof.length + ' leg' + (noProof.length === 1 ? '' : 's') + ' expect a receipt and ' +
               'have none attached — ' + _travPeso(owed) + '. They print on neither the itinerary nor ' +
               'the certificate, so nothing on the pack evidences them. Attach the photos, or set ' +
               'those legs to the certificate if no receipt exists for them.' };
  }

  var itin = _travItineraryFor(r['User'], r['Week Start']);
  var itinStatus = itin ? String(itin['Status'] || '') : '';
  var waiverBy = '', waiverReason = '';
  if (noProof.length) {                     // waived above; record who and why on the same pair
    waiverBy = String(p.actorName || '');
    waiverReason = String(p.waiverReason || '').trim();
  }
  if (itinStatus !== 'Approved') {
    var canWaive = canWaiveAny;
...
function _travPostExpense(row, p) {
  ...
  _travItems(no).forEach(function (i) {
    ...
    var means = String(i['Means'] || '').trim().toLowerCase();
    if (kind === 'Parking/Toll') buckets.toll += amt;
    else if (kind === 'Meals') buckets.meals += amt;
    else if (kind === 'Load') buckets.load += amt;
    /* A238 — `fuel` was initialised here and never once incremented, so the Fuel column on the
       expense report was always zero and every litre of travel fuel hid inside Other. Latent until
       A237 put Fuel in the rep's Transport dropdown, which made it reachable and routine.
       Bucketed on MEANS, not on a new kind: adding 'Fuel' to _TRAV_KINDS would have to be mirrored
       in TV_KINDS, and it would drop fuel off the Travel Itinerary for every leg already saved as
       Transport. The means is what the rep actually chose. */
    else if (kind === 'Transport' && means === 'fuel') buckets.fuel += amt;
...
/** A238 — file the three-page pack to Drive, the way every other approval chain files its document.
 *
 *  This was the one gap the A238 scan turned up that was not a defect so much as a half-built
 *  feature: TravelReplenishments has carried a 'PDF Link' column since A212, _travMap reads it into
 *  the DTO, and NOTHING has ever written it. Quotations, pricing requests, POs and payment requests
 *  all have a save*PDF; travel did not, so the pack accounting and the director signed existed only
 *  as a blob in whichever browser rendered it, and an approved week left no artefact at all.
 *
 *  Deliberately NOT in _SECURED. It writes a document for a record whose own guards already decided
 *  who may act, it moves no money, and adding it would widen the three-list mirror that A238 is
 *  explicitly not touching. It follows saveQuotationPDF exactly, including _registerDocument, so the
 *  file lands under the same _Internal/Travel Allowance tree A212-7 built for the receipts. */
function saveTravelPDF(p) {
  p = p || {};   // hand-callable from the Apps Script editor; _dispatch always passes an object
  if (!p.pdfBase64) return { success: false, message: 'pdfBase64 required.' };
  var no = String(p.travNo || '').trim();
  if (!no) return { success: false, message: 'travNo required.' };
  var row = _travRow(no);
  if (!row) return { success: false, message: 'Travel report not found.' };

  var name = p.fileName || ('Travel_Allowance_' + no + '.pdf');
  var saved = _saveFileToDrive(p.pdfBase64, name, 'application/pdf',
    _docFolder(_TRAV_DOC_MODULE, no, _GENERATED_DOC_TYPE, row['Week Start'] || _now()));
  var link = saved.url;
  _setCellByKey('TravelReplenishments', 'Trav No', no, 'PDF Link', link);
  _travSet(no, { 'Updated At': _now() });
  _registerDocument(_TRAV_DOC_MODULE, no, name, link, saved.id, p.actorName);
  return { success: true, link: link, travNo: no, refNo: no,
           message: 'Travel allowance pack saved to Drive.' };
}
...
var _MODULE_MAP = {
  ...
  saveLeadgenRecord: ['Lead Gen', 'Saved'], deleteLeadgenRecord: ['Lead Gen', 'Removed'],   // A277
  ...
  setQuotationEmailReply: ['Quotation', 'Reply Checked'],
  ...
  setQuotationSalesperson: ['Quotation', 'Reattributed'],
  ...
  saveTravelPDF: ['Travel Allowance', 'Document filed'],                        // A238
  ...
  backfillMissingAR: ['AR Aging', 'Backfilled'],
  applyInvoiceVatRepair: ['AR Aging', 'VAT Repaired'],   // A278
  setFlowDriveFolder: ['Balance Sheet', 'Config Updated'],
  // A193
 ...
 *
 *  A216 — IT RETURNS 'HH:mm', NOT '3:30 PM', AND THAT IS LOAD-BEARING. The rep's planner renders
 *  this value into <input type="time"> (weekly-itinerary.js:156), which accepts only 'HH:mm'. Given
 *  anything else the browser silently blanks the control — and the next save writes the blank back,
 *  so opening the plan and pressing Save destroyed every planned time on it. A215's '3:30 PM' was
 *  readable and still broke that round trip. Whatever a human reads is formatted at the point of
 *  display instead (iwTime12 in dashboard/js/itinerary-week.js).
 *
 ...
 *  what they meant, and the input control was going to reject it either way. */
...
function _logAmount(params) {
  try {
    if (params.items) {
      ...
      its.forEach(function (it) { t += _lineAmount(it); });
      if (t) return t;
    }
  } catch (e) {}
    ...
    var refNo = (action === 'updateAPAging')
      ...
      : (result.quotationNo || result.soNo || result.poNo || result.mrNo || result.invNo
         ...
         /* A277 — the generic stores (marketing, lead-gen) return `id`, not `refNo`. Without this
            every marketing save logged a blank Ref No, and marketing-daily-report.js — which
            buckets by the id prefix — has counted zero of everything since it shipped. */
         || result.id || params.refNo || params.prNo || params.poNo
...
function getPricingRequests(p) {
  ...
  /* A242 — ALL of them, not just the pick. A request may now become more than one quotation, and
     `quotationNo` above answers a question 16 surfaces ask ("which ONE best represents this") that
     is still worth answering — so it does not move. `quotationNos` is the new, complete answer,
     added beside it rather than instead of it. */
  var quoNosByPR = {}, quoByNo = {};
  _rows('Quotations').forEach(function (q) { quoByNo[String(q['Quotation No'])] = q; });
  Object.keys(quoCands).forEach(function (k) {
    quoNosByPR[k] = _quoAllForPR(k, quoCands[k]).map(function (q) { return String(q['Quotation No']); });
  });
  ...
  return { success: true, data: headers.map(function (h) {
    ...
    return {
      ...
      /* A242: every quotation this request became, live ones first. `quotationNo` above is still the
         single pick and still what most surfaces read; this is what a partly-quoted request needs to
         show honestly. Empty for the 297 requests that became nothing. */
      quotationNos: quoNosByPR[String(h['PR No'])] || [],
      ...
      items: its.map(function (r) {
        return {
          ...
          /* A242 — which quotation carried this line, and whether it is still available.
             `quotable` is answered HERE and not in the browser on purpose: deciding it needs the
             quotations' statuses (gone or Cancelled frees the line again) and qcLoadFromPR fetches
             only the requests, so a browser asked to work it out would have to guess. */
          quotedOn: String(r['Quoted On'] || '').trim(),
          quotable: _prLineQuotable(r, quoByNo)
        };
      })
    };
  }) };
...
/* ── A242 — PARTIAL QUOTATIONS ────────────────────────────────────────────────────────────────────
 *
 * A request may now become MORE THAN ONE quotation: the 3 lines management has priced go out today,
 * the other 2 stay on the request and go out later. _quoPickForPR above still answers "which ONE
 * quotation best represents this request" for every surface that wants a single answer, and its own
 * note records that no live request had two — that stops being true from here, so anything that must
 * see them all uses _quoAllForPR instead of picking.
 *
 * Every helper below is PURE apart from _rows, and none of them writes. */

/** Every quotation carrying this PR No, live ones first, newest last within each half. */
function _quoAllForPR(prNo, all) {
  var cands = (all || _rows('Quotations')).filter(function (q) {
    return String(q['PR No'] || '') === String(prNo);
  });
  var live = [], retired = [];
  cands.forEach(function (q) {
    (_QUOTE_CLOSED.indexOf(String(q['Status'] || '')) === -1 ? live : retired).push(q);
  });
  return live.concat(retired);
}

function _quotationNosForPR(prNo, all) {
  return _quoAllForPR(prNo, all).map(function (q) { return String(q['Quotation No']); });
}

/* CAN THIS LINE BE QUOTED?
 *
 * 'Quoted On' is never cleared, so this is DERIVED rather than read. A line is available when it
 * names nothing, or when the quotation it names has gone away or was CANCELLED — the distinction
 * pr-worklist.js already draws at its `quotation-void` step: Cancelled means WE withdrew the
 * document and the work is live again, while Lost and Not Pursued are the client deciding and are
 * real endings. Deriving also means the one thing Apps Script cannot do — write two cells
 * atomically — stops mattering: a stamp naming a quotation that was never created reads as quotable,
 * so a half-finished create heals itself instead of stranding a line nobody can quote.
 *
 * @param it      a PricingRequestItems row
 * @param quoByNo {quotationNo: Quotations row}; a number missing from it is a quotation that no
 *                longer exists, which is exactly the `quotation-gone` case
 */
function _prLineQuotable(it, quoByNo) {
  var on = String((it && it['Quoted On']) || '').trim();
  if (!on) return true;
  var q = (quoByNo || {})[on];
  if (!q) return true;                                        // renamed or deleted — the line is free
  return String(q['Status'] || '') === 'Cancelled';           // withdrawn by us, not decided by them
}

/* HAS MANAGEMENT PRICED THIS LINE?
 *
 * Not the same question as "is Final Price > 0", and the difference is a real quotation: a genuine
 * ₱0 freebie IS priced and must be quotable, while a line management deferred is not priced and
 * would print on a client's document at ₱0.00 if it were treated as one. The header's breakdown
 * (col 15) answers it exactly — _mergePricedItems keys it 'L:'+line and MERGES rather than replaces,
 * so a line priced in an earlier run is still recorded after a later one. Rows written before the
 * breakdown existed have none, so those fall back to the old signal.
 *
 * @return {{has: boolean, lines: Object}} `has` false = no breakdown at all, use the fallback
 */
function _prPricedLines(hdr) {
  var out = { has: false, lines: {} };
  var rows = [];
  try { rows = JSON.parse((hdr && hdr['Priced Items JSON']) || '[]') || []; } catch (e) { rows = []; }
  rows.forEach(function (r) {
    if (r && r.line != null && r.line !== '') { out.has = true; out.lines[String(_num(r.line))] = 1; }
  });
  return out;
}

function _prLinePriced(it, priced) {
  if (priced && priced.has) return !!priced.lines[String(_num(it['Line']))];
  return _num(it['Final Price']) > 0;                         // pre-breakdown rows: the old signal
}

/** The included lines of a request that are still available to quote, with why. */
function _prRemainingLines(prNo, opts) {
  opts = opts || {};
  var hdr = opts.hdr || _prHeaderRow(prNo);
  var quoByNo = opts.quoByNo;
  if (!quoByNo) {
    quoByNo = {};
    _rows('Quotations').forEach(function (q) { quoByNo[String(q['Quotation No'])] = q; });
  }
  var priced = _prPricedLines(hdr);
  var items = (opts.items || _rows('PricingRequestItems')).filter(function (r) {
    return String(r['PR No']) === String(prNo)
      && (r['Included'] === true || String(r['Included']) === 'true');
  });
  var open = [], ready = [], unpriced = [], quoted = [];
  items.forEach(function (r) {
    if (!_prLineQuotable(r, quoByNo)) { quoted.push(r); return; }
    open.push(r);
    (_prLinePriced(r, priced) ? ready : unpriced).push(r);
  });
  return { included: items, open: open, ready: ready, unpriced: unpriced, quoted: quoted };
}
...
function createPricingRequest(p) {
  ...
  items.forEach(function (it, i) {
    sh.appendRow([no, i + 1, it.itemNo, it.itemName, _num(it.qty), it.uom || '', it.remarks || '',
      ...
      it.itemId || '', '']);
      // trailing: Orig Item No/Name / Supplier Price VAT / A159 Item ID / A242 Quoted On (blank — a
      // brand-new line has been quoted on nothing)
  });
...
function updatePRSourcing(p) {
  ...
  JSON.parse(p.items || '[]').forEach(function (u) {
    ...
    /* A256 — CORRECTING THE REQUESTED TEXT, not just capturing it.
       Cols 15/16 were written once, first-change-wins, and no code path could ever change them
       again. That text is not internal: flow_quotation_pdf prints it as the bold heading above
       OUR OFFER — the line the client asked for. So a wrong capture was permanent and visible to
       the customer. PR-202608-011 is the live case: the two lines' requested descriptions were
       transposed, so item 01 asked for a 30-ton puller and was offered an 8-ton kit.
       Deliberately AFTER the auto-capture above, so an explicit correction wins over it. Sent and
       non-blank overwrites; absent or blank leaves the stored value alone, per A174. */
    if (u.origItemNo !== undefined && String(u.origItemNo).trim() !== '') {
      sh.getRange(row.rowIndex, 15, 1, 1).setValues([[String(u.origItemNo).trim()]]);
    }
    if (u.origItemName !== undefined && String(u.origItemName).trim() !== '') {
      sh.getRange(row.rowIndex, 16, 1, 1).setValues([[String(u.origItemName).trim()]]);
    }
  ...
  /* A242 — 'Partly Quoted' joins the list that may advance. Without it the second half of a split
     request could never be sourced: the admin would fill in the supplier price for the two leftover
     lines, the status would stay 'Partly Quoted', and submitForPricing would have nothing to move.
     A request whose remainder is being sourced IS in sourcing; what has already gone out is recorded
     per line in 'Quoted On', not in the status. */
  var curStatus = String((_prHeaderRow(p.prNo) || {})['Status'] || '');
  if (curStatus === 'Requested' || curStatus === 'Sourcing' || curStatus === ''
      || curStatus === 'Partly Quoted') {
...
function _sourcingGaps(prNo) {
  ...
  /* A242 — WHAT "SOURCED ENOUGH TO FORWARD" MEANS CHANGED, and this is the admin half of partial
     quoting. It used to demand a supplier price, principal and currency on EVERY included line, so a
     request where 3 of 5 items had come back from the supplier could not be forwarded at all — the
     whole request waited on its slowest line, and the only way to move was to un-tick the other two,
     which drops them from the deal for good. Now at least one line must be complete and the rest
     simply do not go: a line the admin has not sourced yet is a line management is not asked to
     price. That needs no "forwarded" flag anywhere — sourcing completeness already says it.
     Lines already out on a live quotation are ignored entirely; they are finished, not pending. */
  var open = 0, ready = 0;
  var quoByNo = {};
  _rows('Quotations').forEach(function (q) { quoByNo[String(q['Quotation No'])] = q; });
  _rows('PricingRequestItems').forEach(function (r) {
    if (String(r['PR No']) !== String(prNo)) return;
    if (!(r['Included'] === true || String(r['Included']) === 'true')) return;  // A147: stringified checkbox
    if (!_prLineQuotable(r, quoByNo)) return;
    open++;
    if (_num(r['Supplier Price (FC)']) > 0 && String(r['Principal'] || '').trim()
        && String(r['Currency'] || '').trim()) ready++;
  });
  if (open && !ready) {
    gaps.push('a supplier price, principal and currency on at least one included item');
  }
  return gaps;
}

/** A242 — how many lines a forward would actually carry, for a message that names the split. */
function _sourcingSplit(prNo) {
  var quoByNo = {};
  _rows('Quotations').forEach(function (q) { quoByNo[String(q['Quotation No'])] = q; });
  var ready = 0, waiting = 0;
  _rows('PricingRequestItems').forEach(function (r) {
    if (String(r['PR No']) !== String(prNo)) return;
    if (!(r['Included'] === true || String(r['Included']) === 'true')) return;
    if (!_prLineQuotable(r, quoByNo)) return;
    if (_num(r['Supplier Price (FC)']) > 0 && String(r['Principal'] || '').trim()
        && String(r['Currency'] || '').trim()) ready++; else waiting++;
  });
...
function submitForPricing(p) {
  ...
  /* A147 stage gate: only a request still being sourced can be forwarded.
     A242 adds 'Partly Quoted'. THIS IS THE BLOCKER THAT MADE THE WHOLE FEATURE A DEAD END: a request
     that had sent 3 of its 5 lines out on a quotation could never send the other 2 anywhere, because
     no status transition led back here. The remainder re-enters the ordinary cycle from this door. */
  var st = _prStatus(p.prNo);
  if (st && st !== 'Requested' && st !== 'Sourcing' && st !== 'Partly Quoted') {
    return { success: false, message: 'This request is "' + st + '" — only a Requested/Sourcing/Partly Quoted request can be forwarded to management.' };
  }
  ...
  var split = _sourcingSplit(p.prNo);
  _setPRStatus(p.prNo, 'For Mgmt Pricing');
  /* Say what is going and what is being left behind. A partial forward is a deliberate act and the
     admin should see its shape confirmed, not discover later that two lines never went. */
  return { success: true, prNo: p.prNo, ready: split.ready, waiting: split.waiting,
    message: 'Forwarded to management for pricing' +
      (split.waiting ? ' — ' + split.ready + ' item(s) sent, ' + split.waiting +
                       ' still waiting on a supplier price and left on this request.' : '.') };
}
...
function setMgmtPricing(p) {
  ...
  if (prStatus !== 'For Mgmt Pricing') {
     ...
     *
     * A242 — THIS GUARD IS NOW PER LINE, and it had to be, twice over. It used to fire only on
     * status 'Quoted' and only against the ONE quotation _quotationNoForPR picks, which breaks in
     * both directions once a request can have two: a 'Partly Quoted' request skipped the check
     * altogether, and where there were two quotations it checked whichever one the picker preferred
     * — so a Draft second quotation could wave through a re-price of lines sitting on the first,
     * Sent one. That is exactly the failure A158 added this guard to prevent, reappearing.
     *
     * The rule now: a line whose own 'Quoted On' names an Approved or Sent quotation is frozen. A
     * line that has never been quoted is free, which is what lets management price the remainder of
     * a split request while the client holds the first half. */
    var priceLines = [];
    try { priceLines = JSON.parse(p.items || '[]') || []; } catch (e) { priceLines = []; }
    var wanted = {};
    priceLines.forEach(function (it) { if (it && it.line != null) wanted[String(_num(it.line))] = 1; });
    var frozen = {};
    _rows('PricingRequestItems').forEach(function (r) {
      if (String(r['PR No']) !== String(p.prNo)) return;
      if (!wanted[String(_num(r['Line']))]) return;
      var on = String(r['Quoted On'] || '').trim();
      if (!on) return;
      var qr = _quotationRow(on);
      var st = qr ? String(qr['Status'] || '') : '';
      if (qr && (st === 'Approved' || st === 'Sent')) (frozen[on] = frozen[on] || []).push(_num(r['Line']));
    });
    var frozenNos = Object.keys(frozen);
    if (frozenNos.length) {
      return { success: false, message: 'Item(s) ' +
        frozenNos.map(function (n) { return frozen[n].join(', ') + ' on quotation ' + n; }).join('; ') +
        ' — that quotation has already gone to the client. Revise it first, then re-price. ' +
        'Lines not yet quoted can be priced without touching it.' };
    }
  }
  ...
  JSON.parse(p.items || '[]').forEach(function (u) {
    ...
    /* A255 — MANAGEMENT'S RE-SPECIFICATION. The model number and description are editable on the
       pricing screen and landed only in Priced Items JSON, a history blob no downstream reader
       touches. Everything that reaches the client — the returned request, createQuotationFromPR,
       the quotation itself — is rebuilt from THIS row, so a re-specified line went out as the old
       model and description at the new price. PR-202608-011 line 1 is the live case: priced as
       PH82K "PULLER KIT, 8 TON, 9-21/32in SPREAD", quoted as PH83C "HYDR GEAR PULL, 8 TON".
       Written only when sent AND non-blank — an absent or empty field must leave the stored value
       alone, never blank a real item name (the A174 lesson). */
    if (u.itemNo !== undefined && String(u.itemNo).trim() !== '') {
      ish.getRange(row.rowIndex, 3, 1, 1).setValues([[String(u.itemNo).trim()]]);                        // Item No (col 3)
    }
    if (u.itemName !== undefined && String(u.itemName).trim() !== '') {
      ish.getRange(row.rowIndex, 4, 1, 1).setValues([[String(u.itemName).trim()]]);                      // Item Name (col 4)
    }
  });
...
function rejectMgmtPricing(p) {
  ...
  /* Clear the whole sourcing on every item: cols 9-14 (Supplier, Principal, Currency, Supplier Price
     ...
     A242 — EXCEPT the lines already out on a live quotation. This wiped every row of the request,
     which was harmless while a request could only ever have one quotation and rejecting the pricing
     meant nothing had been issued. Once 3 of 5 lines can already be on a document the client is
     holding, rejecting the OTHER two's pricing would zero the Final Price behind the first
     quotation — and both _quotationPrDeviation and the approval banner would then report that
     quotation as deviating from a pricing that no longer exists. A rejection is about the lines
     being rejected. */
  var ish = _sheet('PricingRequestItems');
  var vatCol = SCHEMA.PricingRequestItems.indexOf('Supplier Price VAT') + 1;
  var quoByNo = {};
  ...
  _rows('PricingRequestItems').forEach(function (r) {
    if (String(r['PR No']) !== String(p.prNo)) return;
    if (!_prLineQuotable(r, quoByNo)) { keptLines.push(_num(r['Line'])); return; }
  ...
  /* Drop management's stored breakdown (header col 15) so the engine's buy-price fallback can never
     re-show a stale figure on the next load after the price has been cleared — but keep the entries
     belonging to the lines above, for the same reason. */
  var hsh = _sheet('PricingRequests');
  var keepKeys = {};
  keptLines.forEach(function (l) { keepKeys['L:' + String(l)] = 1; });
  _rows('PricingRequests').forEach(function (h) {
    if (String(h['PR No']) !== String(p.prNo)) return;
    var keep = '';
    if (keptLines.length) {
      var stored = [];
      try { stored = JSON.parse(h['Priced Items JSON'] || '[]') || []; } catch (e) { stored = []; }
      var survivors = stored.filter(function (r) {
        return r && r.line != null && r.line !== '' && keepKeys['L:' + String(_num(r.line))];
      });
      if (survivors.length) keep = JSON.stringify(survivors);
    }
    hsh.getRange(h.rowIndex, 15, 1, 1).setValues([[keep]]);
  });
  ...
  return { success: true, prNo: p.prNo, status: 'Sourcing', keptLines: keptLines,
    message: 'Pricing rejected — the sourced prices were cleared and the request returned to admin for re-sourcing.' +
      (keptLines.length ? ' Item(s) ' + keptLines.join(', ') + ' were left untouched: they are already out on a quotation.' : '') };
}
...
function createQuotationFromPR(p) {
  ...
  /* A147 stage gate. A242 — 'one quotation per PR' is retired here, and it was never a business rule:
     it was a status guard doing duty as one, and it made a request with 5 items where only 3 were
     priced unquotable. A request may now become several quotations over time; what stops the SAME
     LINE going out twice is the per-line 'Quoted On' check below, which is the invariant that
     actually matters — a client must never receive two documents for one item. */
  var prStatus = String(hdr['Status'] || '');
  if (prStatus !== 'Returned to Sales' && prStatus !== 'Partly Quoted') {
    return { success: false, message: 'This request is "' + prStatus +
      '" — it must be Returned to Sales (or Partly Quoted, for the items still outstanding) before it can be quoted.' };
  }

  var quoByNo = {};
  _rows('Quotations').forEach(function (q) { quoByNo[String(q['Quotation No'])] = q; });
  var allItems = _rows('PricingRequestItems');
  var rem = _prRemainingLines(p.prNo, { hdr: hdr, quoByNo: quoByNo, items: allItems });

  if (!rem.included.length) return { success: false, message: 'No included items to quote.' };
  if (!rem.open.length) {
    /* Everything is out. Say so truthfully and name what it became — the old code said "already
       quoted" from a status that could not tell the difference between "all of it" and "some". */
    var nos = _quotationNosForPR(p.prNo);
    return { success: true, prNo: p.prNo, quotationNo: nos[0] || _quotationNoForPR(p.prNo),
      quotationNos: nos, duplicate: true,
      message: nos.length
        ? ('Every item on this request is already quoted (' + nos.join(', ') +
           ') — revise one of those rather than creating another.')
        : 'Every item on this request is already quoted.' };
  }

  /* WHICH LINES. `lines` is the rep's selection from the configurator; absent, this is the old
     behaviour of carrying everything still open.
     THE HOLE THAT ABSENCE OPENS, and why it is closed here: management can now DEFER a line rather
     than un-include it, so an included line with no price is an ordinary state instead of an
     anomaly. An older browser posts no `lines`, so it would sweep those deferred lines onto the
     document at ₱0.00 — which is precisely the A158 failure that `excludedLines` was invented to
     stop. So when there is no selection AND something is unpriced, refuse and name them. */
  var wanted = null;
  if (p.lines !== undefined && p.lines !== null && p.lines !== '') {
    try { wanted = JSON.parse(typeof p.lines === 'string' ? p.lines : JSON.stringify(p.lines)) || []; }
    catch (e) { return { success: false, message: 'Could not read the list of items to quote.' }; }
    if (!wanted.length) return { success: false, message: 'No items were selected to quote.' };
  } else if (rem.unpriced.length) {
    return { success: false, message: rem.unpriced.length + ' item(s) on this request have not been ' +
      'priced yet (' + rem.unpriced.map(function (r) { return r['Item Name'] || r['Item No']; }).join(', ') +
      '). Reload the page and choose which items to quote — quoting them now would print them as free.' };
  }

  var pick = rem.open;
  if (wanted) {
    var want = {};
    wanted.forEach(function (l) { want[String(_num(l))] = 1; });
    /* Refuse a line that is already out, by name. The rep's browser greys these out, but the browser
       may be stale and this is the guard that a client never gets the same item twice. */
    var taken = rem.quoted.filter(function (r) { return want[String(_num(r['Line']))]; });
    if (taken.length) {
      return { success: false, message: 'Item(s) ' +
        taken.map(function (r) { return (r['Item Name'] || r['Item No']) + ' (on ' + String(r['Quoted On']) + ')'; }).join(', ') +
        ' are already on a quotation. Refresh and select only the items still outstanding.' };
    }
    pick = rem.open.filter(function (r) { return want[String(_num(r['Line']))]; });
    if (!pick.length) return { success: false, message: 'None of the selected items are still available to quote.' };
  }

  // A147: carry ₱0 freebies/accessories — a 0 from the engine is a legitimate quoted price, and
  // filtering on Final Price > 0 silently dropped freebies the sales rep saw on screen.
  var qItems = pick.map(function (r) {
  ...
  var qres = createQuotation({ customer: hdr['Customer'], date: _now(), status: 'Draft',
    ...
    /* A158: the client's per-SUBMISSION token, not a permanent per-PR key. The old 'qfp_'+prNo key
       ...
       reached nobody. This token only protects against a transport retry of THIS submission.
       A242: the invariant it is NOT protecting is "one quotation per PR" — that was the status
       guard's job and is now the per-line 'Quoted On' check above. */
  ...
  /* A242 — STAMP THE LINES, then set the status from what is left.
   *
   * ORDER MATTERS, and not in the obvious direction. Apps Script cannot write two cells atomically,
   * so one of these is going to be the write that a timeout can land between. Stamping AFTER the
   * quotation exists leaves the dangerous failure: the document is real, the lines still read as
   * unquoted, and the rep re-quotes the same three items onto a second document the client also
   * receives. Stamping FIRST inverts it — a stamp naming a quotation that was never created is read
   * as quotable by _prLineQuotable ("names a quotation that no longer exists"), so the failure
   * heals itself on the next load. We cannot stamp before we know the number, so the stamp goes
   * immediately after the create and before anything else, and is cleared on the way out if it
   * cannot be completed. */
  var ish2 = _sheet('PricingRequestItems');
  var qonCol = SCHEMA.PricingRequestItems.indexOf('Quoted On') + 1;
  var stamped = [];
  if (qonCol > 0) {
    var byLine = {};
    pick.forEach(function (r) { byLine[String(_num(r['Line']))] = 1; });
    _rows('PricingRequestItems').forEach(function (r) {
      if (String(r['PR No']) !== String(p.prNo)) return;
      if (!byLine[String(_num(r['Line']))]) return;
      ish2.getRange(r.rowIndex, qonCol, 1, 1).setValues([[String(qres.quotationNo)]]);
      stamped.push(_num(r['Line']));
    });
  }

  /* What is left decides the status. 'Partly Quoted' is not a decoration: it is what keeps the
     request in the rep's worklist saying how many items are still outstanding, instead of reading
     'Quoted' while two lines have never been sent to anybody. */
  var after = _prRemainingLines(p.prNo, { hdr: hdr, quoByNo: null });
  var done = !after.open.length;

  /* The Notes link. _setPRStatus OVERWRITES col 10, and _quotationNoForPR still reads that free text
     as its fallback for the 41 legacy rows that have no 'PR No' column entry — so writing the second
     quotation's number over the first's would erase the only record some rows have. Append. */
  var prevNote = String(hdr['Notes'] || '');
  var note = (prevNote.indexOf('Quotation ' + qres.quotationNo) >= 0) ? prevNote
    : (/Quotation\s+\S+/i.test(prevNote) ? (prevNote + ' · Quotation ' + qres.quotationNo)
                                         : ('Quotation ' + qres.quotationNo));
  _setPRStatus(p.prNo, done ? 'Quoted' : 'Partly Quoted', note);

  return { success: true, prNo: p.prNo, quotationNo: qres.quotationNo,
    lines: stamped, remaining: after.open.length, status: done ? 'Quoted' : 'Partly Quoted',
    message: 'Quotation ' + qres.quotationNo + ' created from ' + p.prNo +
      ' with ' + qItems.length + ' item(s).' +
      (done ? '' : ' ' + after.open.length + ' item(s) are still to be quoted on this request.') };
}
...
function getClients() {
  return { success: true, data: _rows('Clients').map(function (r) {
    return { customer: r['Customer'], address: r['Address'] || '', contactPerson: r['Contact Person'] || '',
      ...
      updatedBy: r['Updated By'] || '', updatedAt: r['Updated At'] || '', stage: r['Stage'] || '',   // A277
      rowIndex: r.rowIndex };
  }) };
}
function saveClient(p) {
  ...
  /* A277 — twelve values for a twelve-column range: this literal MUST match SCHEMA.Clients or the
     setValues below throws. 'Stage' is kept from the row unless the caller sets it — a paying
     customer saved from the clients page does not become a prospect. */
  var row = [name, p.address || '', p.contactPerson || '', p.designation || '', p.email || '', p.phone || '',
    p.rfqRef || '', p.paymentTerms || '', p.notes || '', p.actorName || '', _now(),
    p.stage !== undefined ? String(p.stage || '') : (existing ? (existing['Stage'] || '') : '')];
  if (row.length !== n) throw new Error('saveClient writes ' + row.length + ' values into ' + n + ' columns.');
  if (existing) sh.getRange(existing.rowIndex, 1, 1, n).setValues([row]);
...
var HANDLERS = {
  ...
  renameSalesOrder: renameSalesOrder, renameInvoice: renameInvoice,
  ...
  getLeadgen: getLeadgen, getLeadgenCounts: getLeadgenCounts, getLeadgenFollowups: getLeadgenFollowups,   // A277
  getLeadgenDay: getLeadgenDay,   // A277-2 — the evidence behind the day's numbers, for the daily report
  saveLeadgenRecord: saveLeadgenRecord, deleteLeadgenRecord: deleteLeadgenRecord,
  ...
  setQuotationSalesperson: setQuotationSalesperson,
  ...
  saveTravelPDF: saveTravelPDF,                                                                     // A238
  ...
  /* A278 — the preview is HANDLERS-only on purpose: it writes nothing, and getInvoices/getARAging
     already expose the same rows, so securing it would buy nothing and cost the 60s read cache.
     It is deliberately NOT in _MODULE_MAP either — _logActivity only runs inside the MUTATIONS
     branch of _dispatch, so an audit row declared for a non-mutation can never actually be written. */
  previewInvoiceVatRepair: previewInvoiceVatRepair, applyInvoiceVatRepair: applyInvoiceVatRepair,
  // A193: Drive filing — client / sales order / document type
...
var MUTATIONS = {
  ...
  /* A243 — reverseReceiving was in HANDLERS, _SECURED and _MODULE_MAP but NOT here, and this list
     ...
     journal rows and then deleteRow()s from two sheets — ran unserialised, and the ActivityLog row
     it declares could never be written. The deleteRow is what makes the missing lock dangerous
     rather than untidy: deleting shifts every row index below it, so a concurrent writer holding an
     index it read a moment ago writes into the wrong record.
     Found by tests/audit/registration.js, which fails on precisely this if it is ever undone. */
  reverseReceiving: 1,
  voidCollection: 1, voidInvoice: 1,
  ...
  saveLeadgenRecord: 1, deleteLeadgenRecord: 1,   // A277 — the batch fan-out reads contacts and writes back
  saveSONotes: 1,   // A191
  ...
  linkQuotationEmail: 1, unlinkQuotationEmail: 1, dismissQuotationEmail: 1, setQuotationEmailReply: 1,
  ...
  /* A238 — writes the PDF Link cell and a Documents row, so it takes the lock like every other
     writer. Two tabs filing the same pack would otherwise leave two Drive files and two registry
     rows pointing at one record. */
  saveTravelPDF: 1,
  ...
  applyInvoiceVatRepair: 1,   // A278 — reads a row, decides from it and writes back; it takes the lock
