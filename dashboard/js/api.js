...
/** Fetch sent emails from the agent's GoDaddy mailbox for a date (default today). Oversight roles may
 *  pass a target `user` (activity-log name); `date` is YYYY-MM-DD (omit for today). */
async function apiFetchEmailLogToday(user, date) {
  const body = {};
  if (user) body.user = user;
  if (date) body.date = date;
  return _flaskFetch('/api/email/today', body);
}
...
/* A284 — sit one cutoff out, or put it back. `skip` false un-skips; the server refuses either way
   once that cutoff has been collected or approved. */
function apiSkipSalaryDeductionCutoff(deductionNo, period, skip) {
  return fetchFromAPI({ action: 'skipSalaryDeductionCutoff', deductionNo: deductionNo,
    period: period, skip: (skip === false ? 'false' : 'true') }, { noCache: true });
}
...
/* A229 — per-cutoff incentives. Both filters are optional and independent: by employee for the
   per-person history, by period for the payroll grid's totals. */
function apiGetPayrollIncentives(opts) {
  var o = opts || {};
  return fetchFromAPI({
    action: 'getPayrollIncentives',
    employee: o.employee || '',
    period: o.period || ''
  }, { noCache: true });
}

function apiSavePayrollIncentive(payload) {
  return fetchFromAPI({
    action: 'savePayrollIncentive',
...
/** Voids by ID — the row survives, so the history stays complete. */
function apiVoidPayrollIncentive(incentiveId, actorName) {
  return fetchFromAPI({
    action: 'voidPayrollIncentive',
