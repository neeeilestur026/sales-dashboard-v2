"""MRODocTemplate -- ReportLab BaseDocTemplate for Materials Receiving Reports.

Ported from the standalone app's CustomDocTemplate.  The header/footer callback
draws the watermark, title, logo, and two-column vendor / receiving-details
block on every page.  ``build_pdf()`` constructs the full story (items table,
special-notes box, signature block) and calls ``self.build()``.
"""
...
class MRODocTemplate(BaseDocTemplate):
    """Custom BaseDocTemplate for Materials Receiving Reports.
    ...
    filename : str
        Output PDF path.
    client_details : dict
        Keys used in header: vendor_name, vendor_address, vendor_contact_person,
        vendor_email, vendor_tin, sales_invoice, po_date, purchase_order_no,
        received_checked_name, received_checked_position.
        Keys used in story: special_notes, prepared_by_name, prepared_by_position.
    items : list[dict]
        Each dict must have: item_no, model_no, item_description, quantity,
        and optionally item_remarks.
    items_per_page : int
        Number of item rows per page (10 for short descriptions, 3 for long).
    """
    ...
    def add_header_footer(self, canvas, doc):
        ...
        draw_label(vx, vy, "VENDOR INFORMATION:")
        vy -= LINE
        ...
        # Row 1 labels
        draw_label(rx,  ry, "Sales Invoice:")
        draw_label(rx2, ry, "Receiving Date:")
        ry -= LINE
        ...
        # Row 2 labels
        draw_label(rx,  ry, "Supplier:")
        draw_label(rx2, ry, "Received & Checked by:")
        ry -= LINE
        ...
        # Row 3 labels
        draw_label(rx, ry, "Purchase Order No:")
        ry -= LINE
    ...
    # ── Build complete PDF ────────────────────────────────────────────────
    def build_pdf(self):
        """Construct the full story (items table, special notes, signature
        block) and call ``self.build()`` to emit the PDF."""
        ...
        def create_item_table(data_rows, per_page):
            """Build a table padded to *per_page* rows with a summary
            placeholder row appended (original behaviour)."""
            header_row = [
            ...
            BASE_COL_WIDTHS = [40, 80, 260, 60, 110]   # sums to 550
            ...
            # Summary placeholder row (original behaviour)
        ...
        # ── Special Notes footer box (shown on last page) ────────────────
        ...
        # ── Prepared by signatory block (last page, below special notes) ──
        ...
        # Inner table: spacer -> underline -> name -> position -> label
        sig_inner = Table(
            [
                [Spacer(1, 0.5 * inch)],    # space for handwritten signature
                [underline],
                [Spacer(1, 0.04 * inch)],
        ...
        # Place inner table flush-left in the frame
