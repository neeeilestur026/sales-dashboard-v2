...
# A241 — the GROUP BAND row. It carries the rep's own free-text title, no item index and no figures,
# so without this it lands in the `elif items:` branch and is concatenated onto the previous item's
# product name — the identical failure A213 hit with scope headings and A240 hit twice more. The
# title is arbitrary and unmatchable; the RANGE TAG at the end of the row is generated by us, so that
# is what identifies the row. Anchored at the END because the title precedes it.
_BAND_TAG_RE = re.compile(
    "(?:" + re.escape(_flat(QUO_GROUP_TAG_RANGE_FMT)).replace(re.escape("%S"), r"[^\W_]{1,6}")
    + "|" + re.escape(_flat(QUO_GROUP_TAG_ONE_FMT)).replace(re.escape("%S"), r"[^\W_]{1,6}") + ")$")
assert _BAND_TAG_RE.search(_flat("POWER AND CONTROL CABLES " + QUO_GROUP_TAG_RANGE_FMT % ("03", "08"))), (
    "the group-band matcher no longer matches the range tag the renderer prints")
assert _BAND_TAG_RE.search(_flat("TESTING AND COMMISSIONING " + QUO_GROUP_TAG_ONE_FMT % "11")), (
    "the group-band matcher no longer matches the single-item tag the renderer prints")
assert not _BAND_TAG_RE.search(_flat("Earthing conductor")), (
    "the group-band matcher is too loose — it would swallow an ordinary item line")
