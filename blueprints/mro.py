"""Materials Receiving (MRO) Blueprint – ports the standalone MRO app into the unified Flask application."""
...
mro_bp = Blueprint("mro_bp", __name__, template_folder="../templates")
...
def _upload_mro_pdf_to_drive(created_by: str, uk: str) -> str:
    """Upload stored MRO PDF to Google Drive and return the drive link."""
    ...
    try:
        ...
        payload = {
            "action": "saveMROPDF",
            "pdfBase64": pdf_b64,
            "fileName": user_pdf.get("filename", "mro.pdf"),
        ...
        if resp.status_code == 200:
            ...
            if result.get("success"):
                ...
                logger.warning("MRO Drive upload failed: %s", result.get("message", ""))
        else:
            logger.warning("MRO Drive upload: status %d", resp.status_code)
    except Exception as e:
        logger.error("MRO Drive upload error: %s", e)
    return ""


def submit_to_google_sheet(vendor_name, po_date, po_number, purchase_order_no,
                           items_list, remarks, received_by, drive_link=""):
    """Submit item rows to Google Sheets via Apps Script with redirect handling."""
    try:
        if not MRO_GOOGLE_APPS_SCRIPT_URL or not MRO_GOOGLE_APPS_SCRIPT_URL.startswith("https://"):
        ...
        for item in items_list:
            row = {
                ...
                "receiving_date":    po_date,
                "sales_invoice":     po_number,
...
@mro_bp.route("/")
def index():
    """Render the MRO generator page."""
    return render_template("mro/index.html")
...
def generate():
    ...
    client_details["ship_to"] = (
        "H.O ESTUR CORPORATION, Blk 90 Lot 2 & 4 Ph 1 University Heights, "
        "Brgy Kaypian, District 1, San Jose Del Monte, Bulacan, Philippines, 3023"
    )
    ...
    client_name = sanitize_filename(client_details.get("vendor_name", "UnknownVendor"))
    po_num = sanitize_filename(client_details.get("purchase_order_no", "NoPO"))
    pdf_filename = f"Materials_Receiving_{client_name}_{po_num}.pdf"
    ...
    try:
        ...
        log.append("Success: Materials Receiving Report generated.")
        ...
        if DASHBOARD_APPS_SCRIPT_URL:
            try:
                ...
                drive_link = _upload_mro_pdf_to_drive(created_by, uk)
                if drive_link:
                    _user_pdf[uk]["drive_link"] = drive_link
                    log.append("Uploaded to Google Drive.")
            except Exception as e:
                logger.warning("MRO Drive upload failed: %s", e)
...
def submit_to_sheets():
    ...
    try:
        ...
        purchase_order_no = data.get("purchase_order_no", "").strip()
        vendor_name     = data.get("vendor_name", "").strip()
        po_date         = data.get("po_date", "").strip()
        remarks         = data.get("remarks", "").strip()
        received_by     = data.get("received_by", "").strip()

        if not all([po_number, vendor_name, po_date, received_by]):
            return jsonify({"success": False, "message": "Missing required fields", "output_log": log}), 400
        if not items:
            return jsonify({"success": False, "message": "No items in purchase order", "output_log": log}), 400
        ...
        def _async_sheet_submit():
            try:
                success, message = submit_to_google_sheet(
                    vendor_name, po_date, po_number, purchase_order_no,
                    items, remarks, received_by, drive_link=drive_link
                )
                _user_submission[uk] = {
